...
use solana_escrow_wire::{
    AssetKind, EscrowInstructionV1, EscrowMaintenanceInstructionV2, EscrowStateV1, EscrowStatus, InitializeParamsV1,
...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    if data.starts_with(EscrowMaintenanceInstructionV2::MAGIC) {
        EscrowMaintenanceInstructionV2::decode(data)
            .map_err(|_| ProgramError::from(Error::InvalidInstruction))?;
        return sweep_terminal_excess(program_id, accounts);
    }
...
fn fund(program_id: &Pubkey, accounts: &[AccountInfo<'_>]) -> ProgramResult {
    ...
    match state.asset_kind {
        AssetKind::NativeSol => {
            ...
            if funder.key == vault.key {
                return Err(Error::InvalidAccounts.into());
            }
            before.checked_add(state.amount).ok_or(Error::Arithmetic)?;
            invoke(
                &system_instruction::transfer(funder.key, vault.key, state.amount),
                &[funder.clone(), vault.clone(), system.clone()],
            )?;
            require_exact_increase(before, vault.lamports(), state.amount)?;
        }
        AssetKind::LegacySpl => {
            ...
            let updated = TokenAccount::unpack(&vault.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?;
            require_exact_increase(before, updated.amount, state.amount)?;
        }
    }
    ...
    match state.asset_kind {
        AssetKind::NativeSol => {
            ...
            let rent_floor = Rent::get()
                .map_err(|_| Error::RentUnavailable)?
                .minimum_balance(0);
            let required = rent_floor.checked_add(state.amount).ok_or(Error::Arithmetic)?;
            if before < required {
                return Err(Error::InvalidAmount.into());
            }
            move_lamports(vault, destination, state.amount)?;
            require_exact_decrease(before, vault.lamports(), state.amount)?;
        }
        AssetKind::LegacySpl => {
            ...
            validate_vault_token_identity(vault, authority, mint, token_program, state)?;
            if vault.key == destination.key {
                return Err(Error::InvalidAccounts.into());
            }
            let before = TokenAccount::unpack(&vault.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?.amount;
            if before < state.amount {
                return Err(Error::InvalidAmount.into());
            }
            ...
            let updated = TokenAccount::unpack(&vault.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?;
            require_exact_decrease(before, updated.amount, state.amount)?;
        }
    }
    Ok(())
}
...
fn close(program_id: &Pubkey, accounts: &[AccountInfo<'_>]) -> ProgramResult {
    ...
    Err(Error::StateRetentionRequired.into())
}

// Permissionless maintenance can only pay the original funder. The economic
// result, reveal, terminal slot and rent-bearing receipt are never changed.
fn sweep_terminal_excess(program_id: &Pubkey, accounts: &[AccountInfo<'_>]) -> ProgramResult {
    let mut iterator = accounts.iter();
    let state_account = next_account_info(&mut iterator)?;
    let state = load_state(program_id, state_account)?;
    require_state_pdas(program_id, state_account, &state)?;
    ...
    match state.asset_kind {
        AssetKind::NativeSol => {
            ...
            require_key(funder, &Pubkey::new_from_array(state.funder))?;
            validate_native_vault(program_id, state_account, vault, &state)?;
            if vault.key == funder.key { return Err(Error::InvalidAccounts.into()); }
            let floor = Rent::get().map_err(|_| Error::RentUnavailable)?.minimum_balance(0);
            let before = vault.lamports();
            let excess = before.checked_sub(floor).ok_or(Error::Arithmetic)?;
            if excess != 0 { move_lamports(vault, funder, excess)?; }
            require_exact_decrease(before, vault.lamports(), excess)
        }
        AssetKind::LegacySpl => {
            let authority = next_account_info(&mut iterator)?;
            let vault = next_account_info(&mut iterator)?;
            let funder = next_account_info(&mut iterator)?;
            let destination = next_account_info(&mut iterator)?;
            let mint = next_account_info(&mut iterator)?;
            let token_program = next_account_info(&mut iterator)?;
            ensure_no_extra(iterator)?;
            require_writable(vault)?;
            ...
            validate_vault_authority(program_id, state_account, authority, &state)?;
            validate_vault_token_identity(vault, authority, mint, token_program, &state)?;
            if destination.owner != token_program.key || destination.key == vault.key {
                return Err(Error::TokenAccountRejected.into());
            }
            let recipient = TokenAccount::unpack(&destination.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?;
            if recipient.owner != *funder.key || recipient.mint != *mint.key {
                return Err(Error::TokenAccountRejected.into());
            }
            let excess = TokenAccount::unpack(&vault.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?.amount;
            if excess != 0 {
                let transfer = spl_token::instruction::transfer_checked(
                    token_program.key, vault.key, mint.key, destination.key,
                    authority.key, &[], excess, state.token_decimals,
                )?;
                let bump = [state.authority_bump];
                let seeds: &[&[u8]] = &[VAULT_AUTHORITY_SEED, state_account.key.as_ref(), &bump];
                invoke_signed(&transfer, &[vault.clone(), mint.clone(), destination.clone(),
                    authority.clone(), token_program.clone()], &[seeds])?;
            }
            let after = TokenAccount::unpack(&vault.try_borrow_data()?)
                .map_err(|_| Error::TokenAccountRejected)?.amount;
            require_exact_decrease(excess, after, excess)
        }
    }
}

// Compare transfer deltas: unsolicited deposits are never swap principal.
fn require_exact_increase(before: u64, after: u64, amount: u64) -> ProgramResult {
    if after != before.checked_add(amount).ok_or(Error::Arithmetic)? {
        return Err(Error::InvalidAmount.into());
    }
    Ok(())
}

fn require_exact_decrease(before: u64, after: u64, amount: u64) -> ProgramResult {
    if after != before.checked_sub(amount).ok_or(Error::Arithmetic)? {
        return Err(Error::InvalidAmount.into());
    }
    Ok(())
}
...
fn validate_vault_token_identity(
    vault: &AccountInfo<'_>,
    authority: &AccountInfo<'_>,
    mint: &AccountInfo<'_>,
    token_program: &AccountInfo<'_>,
    ...
    let token =
        TokenAccount::unpack(&vault.try_borrow_data()?).map_err(|_| Error::TokenAccountRejected)?;
    if token.owner != *authority.key || token.mint != *mint.key {
        return Err(Error::TokenAccountRejected.into());
    }
    Ok(())
}
