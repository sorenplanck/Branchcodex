#!/usr/bin/env python3
"""Run the selected real-daemon DOM<->SOL scenario serially with fail-closed evidence.

Linux-only: a private session plus child-subreaper ownership also catches
orphaned descendants that create another process group/session, including the
owned solana-test-validator. No global process-name matching or signals to
unverified numeric PIDs are used.
"""
import argparse
import ctypes
import hashlib
...
PREFIX = ("production_contracts_bootstrap::producer_v13::native_ceremony_tests::"
          "sol_coldstart_v23::daemon_scenario_v23::")
SCENARIOS = (
    "native_real_daemon_two_sol_claims_survive_original_store_reopen_v23",
)
...
# (variable, must be executable). The escrow program is data, never executed.
DEPENDENCIES = (
    ("DOM_INTEROP_REAL_BINARY_V23", True),
    ("DOM_SOL_TEST_VALIDATOR_V23", True),
    ("DOM_SOL_ESCROW_PROGRAM_V23", False),
)
# The validator ledger is large, reset per run and not replay evidence.
EXCLUDED_FIXTURE_PREFIXES = ("sol-ledger-",)
CANCELLED = False
...
def private_synthetic_tmp():
    ...
    Unix-domain peer-signer and HSM sockets live below this root. Validate the
    complete HOME ancestry before creating our owned, reusable base and
    validate the new case directory before handing it to a child process.
    """
    home = Path.home()
    ...
    # Leave room in sockaddr_un for actor, peer-signer and socket suffixes.
...
def start_test_command_v24(identifier, *, cwd, env, stdout):
    ...
    A new selection or changed command must be reviewed here; changing the
    allowlist alone cannot execute an unreviewed command. No shell is used.
    """
    expected = command(identifier)
    if identifier == "native_real_daemon_two_sol_claims_survive_original_store_reopen_v23":
        if expected != ["cargo","test","--locked","-p","dom-interopd","--no-default-features","--features","production","--lib","--profile","crypto-test","production_contracts_bootstrap::producer_v13::native_ceremony_tests::sol_coldstart_v23::daemon_scenario_v23::native_real_daemon_two_sol_claims_survive_original_store_reopen_v23","--","--ignored","--exact","--nocapture","--test-threads=1","--color","never"]:
...
def fingerprint_dependency(value, executable):
    """Record actual bytes; never execute a claimed dependency.

    The daemon and validator must be executable. The escrow `.so` is loaded by
    the validator as data, so it is fingerprinted without requiring X_OK.
    Every dependency must be a canonical regular file, not a symlink.
    """
    path = Path(value)
    if not path.is_absolute() or path.resolve() != path:
        raise ValueError("scenario dependency path must be absolute and canonical")
    ...
    with os.fdopen(descriptor, "rb") as source:
        ...
        if (not stat.S_ISREG(before.st_mode)
            ...
            raise ValueError("scenario dependency ownership, mode or size refused")
    ...
    if identity(before) != identity(after):
        raise RuntimeError("scenario dependency changed while fingerprinting")
    return {"path": str(path), "blake2b256": digest.hexdigest(),
            ...
            "ctime_ns": before.st_ctime_ns, "executable": executable}


def dependency_fingerprints(environment):
    """Bind results to the real daemon, the validator and the escrow program."""
    ...
    result = {name: fingerprint_dependency(environment[name], executable)
...
def excluded(root, path):
    """True for anything inside an owned validator ledger directory."""
    relative = path.relative_to(root)
    return any(part.startswith(EXCLUDED_FIXTURE_PREFIXES) for part in relative.parts)
...
def preserve_fixture(case, root):
    ...
    for directory, directories, files in os.walk(root, followlinks=False):
        # Prune validator ledgers before descending into them.
        directories[:] = [name for name in directories
                          if not excluded(root, Path(directory) / name)]
...
def run_one(root, evidence, name, timeout):
    ...
    env = dict(os.environ, CARGO_TERM_COLOR="never", RUST_TEST_THREADS="1",
               ...
               DOM_SOL_FAILED_FIXTURE_MARKER_V23=str(case / "failed-fixture-path.txt"))
    log = case / "test.log"
    ...
    try:
        ...
        with log.open("xb", buffering=0) as output, log.open("rb") as live:
            ...
            if dependency_fingerprints(env) != fingerprints:
                raise RuntimeError("scenario dependency identity changed during execution")
...
def main():
    ...
    campaign = {"schema": "DOM-SOL-REAL-DAEMON-CAMPAIGN-V23", "status": "not-run",
    ...
    try:
        ...
        for variable in ("DOM_INTEROP_REAL_BINARY_V23", "DOM_INTEROP_REAL_BINARY_BLAKE2B256_V23",
                         "DOM_SOL_TEST_VALIDATOR_V23", "DOM_SOL_ESCROW_PROGRAM_V23"):
