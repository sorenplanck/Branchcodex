...
fn load_config() -> Result<Config> {
    let listen = env::var("DOM_XMR_SIDECAR_LISTEN")
        ...
        .parse::<SocketAddr>()
        .context("invalid DOM_XMR_SIDECAR_LISTEN")?;
    let monerod_url =
        env::var("DOM_XMR_MONEROD_URL").unwrap_or_else(|_| "http://127.0.0.1:18081".to_owned());
    ...
    let key_hex =
        env::var("DOM_XMR_SIDECAR_AUTH_HEX").context("DOM_XMR_SIDECAR_AUTH_HEX is required")?;
    let key_vec = hex::decode(key_hex).context("sidecar auth key is not hex")?;
    let key: [u8; 32] = key_vec
        .try_into()
        .map_err(|_| anyhow::anyhow!("sidecar auth key must be 32 bytes"))?;
    ...
    let cache_dir = PathBuf::from(
        env::var("DOM_XMR_SIDECAR_CACHE_DIR")
    ...
    let uds_path = PathBuf::from(env::var("DOM_XMR_SIDECAR_UDS").context(
        "DOM_XMR_SIDECAR_UDS is required (absolute path outside world-writable directories)",
    )?);
...
async fn verify_funding_endpoint(
    ...
    let request: VerifyFundingRequestV2 = match serde_json::from_slice(body) {
        Ok(value) => value,
        Err(_) => {
            return write_error(stream, 400, "invalid_json", "invalid request", false)
                .await
                .map_err(Into::into);
        }
    };
    ...
    if config.auth.verify_funding(&request).is_err() {
        return write_error(stream, 401, "auth_failed", "authentication failed", false)
            .await
            .map_err(Into::into);
    }
...
async fn build_sweep_endpoint(stream: &mut TcpStream, config: &Config, body: &[u8]) -> Result<()> {
    ...
    match config.cache.load(&request.request_nonce, &request_hash) {
        ...
        Err(CacheError::Conflict) => {
            return write_error(
                ...
                409,
                "nonce_conflict",
                "request nonce reused with different bytes",
                false,
            )
            .await
            .map_err(Into::into);
    ...
    match result {
        Ok(response) => {
            if let Err(error) = config
                .cache
                .store(&request.request_nonce, request_hash, &response)
            {
                ...
                return write_error(
                    ...
                    status,
                    code,
                    "failed to persist exact signed transaction",
                    retryable,
                )
                .await
                .map_err(Into::into);
...
async fn monerod(
    config: &Config,
) -> Result<MoneroDaemon<SimpleRequestTransport>, SidecarOperationError> {
    SimpleRequestTransport::new(config.monerod_url.clone())
        .await
...
async fn verify_funding(
    ...
    if amount != Some(request.expected_amount_piconero) {
        return Err(SidecarOperationError::Rejected(
            "funding output amount/view pair mismatch".to_owned(),
        ));
    }
    // The output index is derived from canonical transaction bytes, never
    // assumed zero: Monero's native sender shuffles outputs.
    let transaction = monero_oxide_wallet::interface::ProvidesTransactions::transaction(
        &rpc,
        request.funding_tx_hash,
    )
    .await
    .map_err(|_| SidecarOperationError::Retryable)?;
    let view_bytes = Zeroizing::new(<[u8; 32]>::from(view));
    let verified = xmr_raw_tx_verify::verify_exact_raw_funding_v12(
        &transaction.serialize(),
        request.funding_tx_hash,
        request.expected_spend_public_key,
        &view_bytes,
        request.expected_amount_piconero,
        u64::MAX,
    )
    .map_err(|_| SidecarOperationError::Rejected("funding raw recipient mismatch".to_owned()))?;
    Ok(VerifyFundingResponseV2 {
        api_version: API_VERSION_V2,
        request_nonce: request.request_nonce,
        funding_tx_hash: request.funding_tx_hash,
        event_index: verified.output_index(),
...
async fn build_sweep(
    ...
    let view = request.view_scalar.expose(|bytes| parse_scalar(*bytes))?;
    let public = monero_wallet_ng::util::public_key(&spend)
        .compress()
        .to_bytes();
    if public != request.expected_spend_public_key {
        return Err(SidecarOperationError::Rejected(
            "private/public spend key mismatch".to_owned(),
        ));
    }
    let destination = monero_address::MoneroAddress::from_str(
    ...
    .await
    .map_err(|error| classify_sweep_error(&error))?;
    let raw_tx = transaction.serialize();
    if raw_tx.is_empty() || raw_tx.len() > MAX_RAW_TX_BYTES {
        return Err(SidecarOperationError::Rejected(
            "signed transaction exceeds bound".to_owned(),
        ));
    }
...
fn parse_scalar(bytes: [u8; 32]) -> Result<Scalar, SidecarOperationError> {
    ...
    if !input.is_empty() || scalar == Scalar::ZERO {
        return Err(SidecarOperationError::Rejected(
            "zero or trailing scalar bytes".to_owned(),
        ));
    }
    Ok(scalar)
}
...
fn parse_point(bytes: [u8; 32]) -> Result<Point, SidecarOperationError> {
    ...
    if !input.is_empty() {
        return Err(SidecarOperationError::Rejected(
            "trailing public key bytes".to_owned(),
        ));
    }
    compressed
        .decompress()
        .filter(|point| (*point).key_image().is_some())
        .ok_or_else(|| {
            SidecarOperationError::Rejected("non-prime-order public spend key".to_owned())
        })
}
...
#[cfg(unix)]
async fn handle_uds(mut stream: tokio::net::UnixStream, config: Arc<Config>) -> Result<()> {
    ...
    stream
        ...
        .await
        .context("read UDS hello")?;
    let hello =
        serde_json::from_slice::<SidecarHelloV1>(&hello_bytes).context("malformed UDS hello")?;
    hello
        .validate()
        .map_err(|_| anyhow::anyhow!("invalid UDS hello"))?;
    ...
    stream
        .write_all(&proof_length.to_be_bytes())
        .await
        .context("write hello proof length")?;
    stream
        .write_all(&proof_bytes)
        .await
        .context("write hello proof")?;

    let mut prefix = [0_u8; 4];
    stream
        .read_exact(&mut prefix)
        .await
        .context("read UDS frame length")?;
    let length = u32::from_be_bytes(prefix) as usize;
    if length == 0 || length > MAX_FRAME_BYTES {
        return write_uds_response(
            &mut stream,
    ...
    let response = match request {
            ...
            match build_proof_v23::build(&config, &request).await {
                ...
                Err(SidecarOperationError::Rejected(_)) => {
                    SidecarResponseV2::Error(SidecarErrorBody {
                        ...
                        message: "V23 build or durable scope rejected".to_owned(),
                        retryable: false,
                    })
                }
            }
            ...
            match input_proof_v23::prove_cached(&config, &request).await {
                ...
                Err(SidecarOperationError::Rejected(_)) => {
                    SidecarResponseV2::Error(SidecarErrorBody {
                        ...
                        message: "cached input proof rejected".to_owned(),
                        retryable: false,
                    })
                }
            }
        ...
        SidecarRequestV2::VerifyFunding(request) => {
            if request.validate_public_fields().is_err()
                || config.auth.verify_funding(&request).is_err()
            {
                SidecarResponseV2::Error(SidecarErrorBody {
        ...
        SidecarRequestV2::BuildSweep(request) => {
            if request.validate_public_fields().is_err()
                || config.auth.verify_build(&request).is_err()
            {
                SidecarResponseV2::Error(SidecarErrorBody {
                    code: "auth_or_request_failed".to_owned(),
                    message: "request rejected".to_owned(),
                ...
                match config.cache.load(&request.request_nonce, &request_hash) {
                    ...
                    Ok(None) => match build_sweep(&config, &request).await {
                        Ok(value) => {
                            match config
                                .cache
                                .store(&request.request_nonce, request_hash, &value)
                            {
...
async fn write_uds_response(
    ...
    stream
        ...
        .await
        .context("write UDS length")?;
    stream
        .write_all(&bytes)
        .await
        .context("write UDS response")?;
