...
pub trait ChainSourceV1 {
    ...
    /// Rewind a completed scan to a safe checkpoint without acknowledging
    /// observations newer than that scan. Revisioned feeds must override this.
    fn cursor_at_from_scan(
        &self,
        height: u64,
        _scanned_to: &ChainCursorV1,
    ) -> Result<ChainCursorV1, ChainSourceErrorV1> {
        self.cursor_at(height)
    }
...
impl<S: SettlementStore, C: ChainSourceV1, K: EffectSinkV1> SettlementEngine<S, C, K> {
    ...
    fn safe_cursor(
        ...
        if ceiling >= scanned_to.height {
            ...
            Ok(Some(self.chain.cursor_at_from_scan(ceiling, scanned_to)?))
        }
    }
