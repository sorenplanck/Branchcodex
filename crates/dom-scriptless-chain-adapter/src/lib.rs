...
struct BlockDto {
    ...
    #[serde(deserialize_with = "required_coinbase_projection")]
    coinbase: Option<CoinbaseDto>,
    total_fees_noms: u64,
    protocol_version: u32,
    range_proof_serialization_version: u8,
}

// Keep the field required on the wire; only its authenticated genesis value
// may be null. Missing data must not silently become an empty economic body.
fn required_coinbase_projection<'de, D: serde::Deserializer<'de>>(
    deserializer: D,
) -> Result<Option<CoinbaseDto>, D::Error> {
    Option::<CoinbaseDto>::deserialize(deserializer)
}
...
fn validate_scan_response(
    ...
    for block in dto.blocks {
        ...
        let header = BlockHeader::from_bytes(&header_bytes)
            .map_err(|_| ChainAdapterError::InvalidEvidence)?;
        let header_hash =
            *dom_chain::canonical_header_identifier(identity.network_magic, &header_bytes)
                .map_err(|_| ChainAdapterError::InvalidEvidence)?
                .as_bytes();
        ...
        if block.height == 0
            && (block_hash != identity.genesis_hash || previous_block_hash != [0; 32])
        {
            return Err(ChainAdapterError::InvalidEvidence);
        }
        let empty_mainnet_genesis =
            block.height == 0 && identity.network_magic == dom_core::NETWORK_MAGIC_MAINNET;
        match (&block.coinbase, empty_mainnet_genesis) {
            (None, true) if block.transactions.is_empty() && block.total_fees_noms == 0 => {}
            (Some(coinbase), false) => validate_coinbase(coinbase)?,
            _ => return Err(ChainAdapterError::InvalidEvidence),
        }
