...
#[path = "production_native_daemon_planning_v23_tests.rs"]
pub(crate) mod native_daemon_planning_v23;

use std::fs::File;
use std::io::Read;
use std::rc::Rc;

#[path = "production_xmr_enrollment_v23.rs"]
...
pub struct ProductionXmrRefundBundleV1 {
    ...
    /// Canonically committed refund recipient; legacy tag 1 carries none.
    pub(crate) refund_destination: Option<String>,
}
...
impl ProductionXmrRefundBundleV1 {
    /// Public, bounded refund bundle including its independent recipient.
    /// Proof and Store provenance are authenticated by admission/execution.
    pub fn new_v10(
        ...
        if template_hash == ZERO_DIGEST
            || executor_profile_hash == ZERO_DIGEST
            || deadline == 0
        ...
        Ok(Self {
            ...
            deadline,
            refund_destination: Some(refund_destination),
        })
    }

    /// Refund recipient committed by the canonical participant bundle.
    /// Legacy arms must be upgraded and re-admitted before XMR execution.
    pub fn refund_destination(&self) -> Option<&str> {
        self.refund_destination.as_deref()
    }
}

fn validate_xmr_destination_v10(destination: &str) -> Result<(), ProductionInputErrorV1> {
    if destination.is_empty()
        || destination.len() > xmr_live_sidecar_api::MAX_DESTINATION_BYTES
        || !destination.bytes().all(|byte| byte.is_ascii_alphanumeric())
    {
        return Err(ProductionInputErrorV1::InvalidParticipantBundle);
    }
    // Network, checksum and wallet semantics are checked by the native XMR
    // wallet before signing. This codec only enforces bounded canonical text.
    Ok(())
}
...
pub struct ProductionXmrLegSetupV1 {
    ...
    native_enrollment: Option<ProductionXmrEnrollmentBundleV23>,
}
...
impl ProductionXmrLegSetupV1 {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            binding,
            refund: None,
            native_enrollment: None,
        })
    }

    /// Bind the refund recipient inside the canonical participant bundle.
    /// The new tag 2 cannot be interpreted as a legacy tag 1 refund arm.
    pub fn with_refund_destination_v10(
        self,
        mut refund: ProductionXmrRefundBundleV1,
        destination: String,
    ) -> Result<Self, ProductionInputErrorV1> {
        validate_xmr_destination_v10(&destination)?;
        refund.refund_destination = Some(destination);
        self.with_refund(refund)
    }
    ...
    pub fn with_refund(
        ...
        if self.native_enrollment.is_some()
            || refund.template_hash == ZERO_DIGEST
        ...
        if let Some(destination) = &refund.refund_destination {
            validate_xmr_destination_v10(destination)?;
        }
    ...
    /// Enroll native shares without admitting an executable refund policy.
    pub fn with_native_enrollment_v23(
        mut self,
        enrollment: ProductionXmrEnrollmentBundleV23,
    ) -> Result<Self, ProductionInputErrorV1> {
        if self.refund.is_some() || self.native_enrollment.is_some() {
            return Err(ProductionInputErrorV1::InvalidParticipantBundle);
        }
        self.native_enrollment = Some(enrollment);
        Ok(self)
    }
    ...
    fn encode_into(&self, bytes: &mut Vec<u8>) -> Result<(), ProductionInputErrorV1> {
        ...
        if let Some(enrollment) = &self.native_enrollment {
            if self.refund.is_some() {
                return Err(ProductionInputErrorV1::InvalidParticipantBundle);
            }
            bytes.push(3);
            enrollment.encode_into(bytes)?;
            return Ok(());
        }
        match &self.refund {
            None => bytes.push(0),
            Some(refund) => {
                bytes.push(if refund.refund_destination.is_some() {
                    2
                } else {
                    1
                });
                ...
                if let Some(destination) = &refund.refund_destination {
                    let length = u16::try_from(destination.len())
                        .map_err(|_| ProductionInputErrorV1::InputBoundExceeded)?;
                    bytes.extend_from_slice(&length.to_be_bytes());
    ...
    fn decode_from(cursor: &mut InputCursorV1<'_>) -> Result<Self, ProductionInputErrorV1> {
        ...
        let refund_tag = cursor.u8()?;
        let native_enrollment = if refund_tag == 3 {
            Some(ProductionXmrEnrollmentBundleV23::decode_from(cursor)?)
        } else {
            None
        };
        let refund = match refund_tag {
            0 | 3 => None,
            tag @ (1 | 2) => {
                ...
                let refund_destination = if tag == 2 {
                    let length = usize::from(cursor.u16()?);
        ...
        if let Some(enrollment) = native_enrollment {
            return value.with_native_enrollment_v23(enrollment);
        }
        match refund {
            None => Ok(value),
...
/// Rebind refund metadata to the exact deadline enum in the authenticated
/// setup's frozen terms. This is not an execution or secret-release grant.
pub(crate) fn authenticate_xmr_refund_deadline_v23(
    terms: &SettlementTermsV1,
    setup: &ValidatedXmrSetup,
    artifact_deadline: u64,
) -> Result<kaystra_core::types::TimelockSpec, ProductionInputErrorV1> {
    ...
    // The frozen terms hash authenticates the discriminant as well as value.
    // This compares metadata only: no height is converted to Unix seconds
    // and no deadline alone authorizes a refund or a secret-bearing sweep.
    let deadline = terms.counterparty_leg.deadline;
    let value = match deadline {
        TimelockSpec::BlockHeight { value } | TimelockSpec::TimestampSeconds { value } => value,
        TimelockSpec::BtcTime512s { .. } => {
            return Err(ProductionInputErrorV1::InvalidParticipantBundle)
        }
    };
    if value == 0 || value != artifact_deadline {
        return Err(ProductionInputErrorV1::InvalidParticipantBundle);
    }
    Ok(deadline)
}

/// Authenticate the complementary share at admission, before any external
/// funding capability can be assembled. A proof of U alone does not prove
/// that the advertised Monero deposit is controlled by T + U.
fn authenticate_xmr_refund_share_v10(
    terms: &SettlementTermsV1,
    profile: &XmrAdapterProfileV1,
    setup: &ValidatedXmrSetup,
    refund: &ProductionXmrRefundBundleV1,
    ...
    let context = xmr_setup_profile::proof_context_hash(
        profile,
        &xmr_setup_profile::XmrProofContextV1 {
    ...
    .map_err(|_| ProductionInputErrorV1::InvalidParticipantBundle)?;
    let claim =
        xmr_refund_adaptor::verify_refund_bundle(&refund.proof, &terms.settlement_id.0, &context)
            .map_err(|_| ProductionInputErrorV1::InvalidParticipantBundle)?;
    let combined =
        xmr_crypto::combine_public_shares(setup.claim().ed_compressed, claim.ed_compressed)
            .map_err(|_| ProductionInputErrorV1::InvalidParticipantBundle)?;
    if claim.secp_compressed != refund.adaptor_point_sec1
        || claim.ed_compressed == setup.claim().ed_compressed
        || combined != setup.combined_spend_public_key()
        || xmr_refund_adaptor::DomRefundAdaptorExecutor::new(claim).profile_hash()
            != refund.executor_profile_hash
        || authenticate_xmr_refund_deadline_v23(terms, setup, refund.deadline).is_err()
    {
        return Err(ProductionInputErrorV1::InvalidParticipantBundle);
    }
    Ok(())
...
pub struct AuthenticatedXmrSessionBindingsV1 {
    ...
    native_enrollment: Option<xmr_session_init::PreparedXmrShareEnrollmentV23>,
    native_enrollment_bundle: Option<ProductionXmrEnrollmentBundleV23>,
}
...
impl AuthenticatedXmrSessionBindingsV1 {
    ...
    /// bounds are checked at decode. Admission verifies the role-2 proof and
    /// T + U deposit key; refund arming revalidates its own execution meaning.
    pub const fn refund_bundle(&self) -> Option<&ProductionXmrRefundBundleV1> {
        self.refund.as_ref()
    }

    /// Preparation-only capability; cannot authorize funding or sweeping.
    pub fn native_enrollment_v23(
        &self,
    ) -> Option<&xmr_session_init::PreparedXmrShareEnrollmentV23> {
        self.native_enrollment.as_ref()
    }

    /// Immutable constraints to recheck when the native refund is bound.
    pub fn native_enrollment_bundle_v23(&self) -> Option<&ProductionXmrEnrollmentBundleV23> {
        self.native_enrollment_bundle.as_ref()
    }
}
    ...
    .enumerate()
    {
        ...
        match chain.profile().kind {
            ...
            ChainKindV1::Monero { network } => {
                ...
                if leg.refund.is_some() && leg.native_enrollment.is_some() {
                    return Err(ProductionInputErrorV1::InvalidParticipantBundle);
                }
                let native_enrollment = leg
                    .native_enrollment
                    .as_ref()
                    .map(|enrollment| enrollment.authenticate(terms, &setup))
                    .transpose()?;
                monero_sessions[index] = Some(AuthenticatedXmrSessionBindingsV1 {
                    ...
                    native_enrollment,
                    native_enrollment_bundle: leg.native_enrollment.clone(),
                });
            }
        }
    }
...
mod tests {
    ...
    // evidence. Cryptographic enrollment is verified by the real input path.
    #[test]
    fn native_xmr_enrollment_codec_is_disjoint_from_executable_refund() {
        let leg = synthetic_monero_leg(ProductionRoutePositionV1::Upstream);
        let mut proof = leg.binding.dleq.clone();
        proof.role = 2;
        let point = proof.bundle.claim.secp_compressed;
        let enrollment = ProductionXmrEnrollmentBundleV23::new(
            proof.clone(),
            point,
            [0x61; 32],
            1_900_000_100,
            "5NativeRefundRecipientFixture".to_owned(),
        )
        .expect("structural enrollment");
        ...
        .expect("structural refund");
        let enrolled = leg
            .clone()
            .with_native_enrollment_v23(enrollment.clone())
            .expect("enrollment only");
        assert!(enrolled.refund.is_none());
        assert!(enrolled.clone().with_refund(refund.clone()).is_err());
        assert!(enrolled
            .clone()
            .with_native_enrollment_v23(enrollment.clone())
            .is_err());
        ...
        let decoded =
            ProductionParticipantBindingBundleV1::decode_canonical(&bytes).expect("decode");
        assert_eq!(decoded, bundle);
        assert!(decoded.monero_legs()[0].refund.is_none());
        assert!(decoded.monero_legs()[0].native_enrollment.as_ref() == Some(&enrollment));
        for length in [0, 1, bytes.len() - 1] {
            assert!(
                ProductionParticipantBindingBundleV1::decode_canonical(&bytes[..length]).is_err()
            );
        }
    ...
    fn monero_leg_refund_arm_round_trips_and_bounds_are_enforced() {
        ...
        let upgraded = synthetic_monero_leg(ProductionRoutePositionV1::Upstream)
            .with_refund_destination_v10(refund.clone(), "5RefundRecipientFixture".to_owned())
            .expect("V10 destination");
        let upgraded = ProductionParticipantBindingBundleV1::new_with_all_counterparty_bindings(
            ROUTE_ID,
            Vec::new(),
            Vec::new(),
            Vec::new(),
        ...
        .expect("bundle");
        let upgraded_bytes = upgraded.canonical_bytes().expect("V10 encode");
        assert_ne!(upgraded_bytes, bytes);
        assert_ne!(
            upgraded.bundle_digest().expect("digest"),
            bundle.bundle_digest().expect("legacy digest")
        );
        let decoded = ProductionParticipantBindingBundleV1::decode_canonical(&upgraded_bytes)
            .expect("V10 decode");
        assert_eq!(decoded, upgraded);
        assert_eq!(
            decoded.monero_legs()[0]
                .refund
                .as_ref()
                .expect("refund")
                .refund_destination(),
            Some("5RefundRecipientFixture")
        );
        for bad_destination in ["", "address with spaces", "address\n", "x/../../file"] {
            assert!(synthetic_monero_leg(ProductionRoutePositionV1::Upstream)
                .with_refund_destination_v10(refund.clone(), bad_destination.to_owned())
                .is_err());
        }
