...
pub(crate) fn bounded_rpc_timeouts(
    external_call_timeout_ms: u64,
...
impl ProductionChainServicesV2 {
    /// Migration of the old single EVM/BTC document into positional services.
    /// The V8 format itself does not require either family to be present.
    pub(crate) fn into_route_document_v8(
        self,
        topology: &crate::production_route_topology::ProductionRouteTopologyV4,
    ) -> Result<crate::production_route_services::RouteServicesV8, ProductionChainServicesErrorV1>
    {
        ...
        if self.solana.is_some() || self.xmr.is_some() {
            return Err(ProductionChainServicesErrorV1::InvalidEncoding);
        }
        let mut legs = Vec::with_capacity(2);
        for leg in topology.legs {
            let service = match leg.face {
                Face::Evm => ServiceV8::Evm {
                    endpoint: self.base.evm_rpc_endpoint.clone(),
                    refund_timeout_seconds: self.base.evm_timeout_seconds,
                },
                Face::Bitcoin => ServiceV8::Bitcoin {
                    endpoint: self.base.bitcoin_rpc_endpoint.clone(),
                    wallet: self.base.bitcoin_wallet_name.clone(),
                    cookie: self.base.bitcoin_cookie_path.clone(),
                },
                _ => return Err(ProductionChainServicesErrorV1::InvalidEncoding),
            };
            legs.push(LegServicesV8 {
                settlement_id: leg.settlement_id,
                chain_id: leg.chain_id,
                service,
            });
        }
        // V2 had only one resource per family and cannot express two owners.
        if topology.legs[0].face == topology.legs[1].face {
            return Err(ProductionChainServicesErrorV1::InvalidEncoding);
        }
        Ok(RouteServicesV8 {
            version: 8,
...
pub(crate) fn validate_endpoint_text(
    endpoint: &str,
...
pub(crate) fn validate_cookie_path(path: &Path) -> Result<(), ProductionChainServicesErrorV1> {
    let rendered = path
        .to_str()
        .ok_or(ProductionChainServicesErrorV1::InvalidBitcoinCookie)?;
    if !path.is_absolute()
...
pub(crate) fn bitcoin_network_identity(
