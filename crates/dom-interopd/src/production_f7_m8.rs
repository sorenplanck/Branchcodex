...
pub(crate) fn verify_and_issue_production_f7_m8_v2(
    ...
    let (contracts_authorization, bitcoin) =
        select_local_f7_authorization_v8(verified, local_bitcoin, leg)?;
...
/// Splits one fresh verifier result without issuing a second Contracts token.
/// This allows every M.8 phase to revalidate the same retained consumed owner.
pub(crate) fn select_local_f7_authorization_v8(
    verified: f7_anchor_authority::VerifiedF7RouteAnchorAuthorizationsV2,
    local_bitcoin: &ProductionBitcoinParticipantAuthorityV1<'_>,
    leg: LegIdV1,
) -> Result<
    (
        f7_anchor_authority::VerifiedF7AnchorAuthorizationV2,
        ProductionBitcoinM8ParticipantHandoffV2,
    ),
    ProductionF7M8ErrorV2,
> {
    ...
    Ok((contracts_authorization, bitcoin))
}
