...
#[path = "production_contracts_cancelled_bootstrap_v22.rs"]
pub(crate) mod xmr_cancelled_v22;
...
pub(crate) struct ProductionContractsSessionBootstrapRequestV1<'authority> {
    ...
    pub(crate) bootstrap_artifact_path: &'authority Path,
...
pub(crate) fn bootstrap_production_contracts_sessions_v1(
    ...
    let trusted_chain_id =
        authenticate_production_contracts_chain_v23(&dom_chain_adapter, authenticated_bootstrap)?;
    ...
    let mut private_bootstrap_v13 =
        crate::production_contracts_bootstrap::producer_v13::resume_completed_bootstrap_v13(
            bootstrap_artifact_path,
            authenticated_bootstrap,
            [
    ...
    if let Some(private) = private_bootstrap_v13.as_mut() {
        ...
        {
            match (
                private._cancelled_contracts[index].as_mut(),
                private._cancelled_shares[index].as_ref(),
            ) {
                (Some(contracts), Some(material)) => contracts
                    .reauthenticate(
                        chain_signers.dom_binding(leg),
                        &authenticated_bootstrap.legs()[index],
                        trusted_chain_id,
                        material,
                        identity.as_ref(),
                    )
                    .map_err(|_| ProductionContractsSessionBootstrapErrorV1::BootstrapRefused)?,
                (None, None) => {}
                _ => return Err(ProductionContractsSessionBootstrapErrorV1::BootstrapRefused),
            }
        }
    }
...
fn converge_leg_prefix(
    ...
    match store.authenticate_bootstrap_origin_v12(&material.initial) {
        Ok(_) => {}
...
fn reauthenticate_and_prepare_early(
    ...
    let retained = store
        .authenticate_bootstrap_origin_v12(&material.initial)
        .map_err(|_| ProductionContractsSessionBootstrapErrorV1::StoreRefused)?;
    if retained.session_id() != material.initial.session_id()
        || store
...
mod tests {
    ...
    #[test]
    fn v12_restart_authenticates_origin_after_durable_head_advanced(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let temporary = tempfile::tempdir()?;
        std::fs::set_permissions(temporary.path(), std::fs::Permissions::from_mode(0o700))?;
        let expected = expected_identity();
        ...
        let material = prepared_leg(chain, 0x53)?;
        let policy = production_policy(0x43)?;
        let creation = [0x73; 32];
        ...
        converge_leg_prefix(&store, &material)?;
        drop(reauthenticate_and_prepare_early(&store, chain, &material)?);
        let mut projection = material.initial.chain();
        projection.tip_height = 1;
        projection.tip_id = [0x75; 32];
        let advanced =
            store.update_chain_projection(material.initial.session_id(), 0, projection)?;
        let exact_advanced = advanced.as_bytes().to_vec();
        drop(store);
        let store = ContractsSessionStoreV1::prepare_open_resumed_production(
            state_capability(temporary.path())?,
            "advanced-contracts",
            policy,
            creation,
        ...
        .finish()?;
        // Reopen keeps the accepted later head and authenticates revision zero;
        // it must neither reject legitimate progress nor rewrite the origin.
        converge_leg_prefix(&store, &material)?;
        drop(reauthenticate_and_prepare_early(&store, chain, &material)?);
        assert_eq!(
            store
        ...
        let mut other_terms = material.initial.terms_hash();
        other_terms[0] ^= 1;
        let substituted = SessionRecordV1::new(
            SessionRecordFieldsV1 {
                session_id: material.initial.session_id(),
                revision: 0,
                phase: SessionPhaseV1::Created,
                terms_hash: other_terms,
                transcript_hash: material.initial.transcript_hash(),
                irreversible: material.initial.irreversible(),
                chain: material.initial.chain(),
            },
            material.initial.encrypted_payload(),
        )?;
        assert!(matches!(
            store.authenticate_bootstrap_origin_v12(&substituted),
...
/// Authenticate DOM genesis before Contracts Stores perform recovery audits.
/// Shares the existing Stage10 checks; persisted chain bytes do not mint trust.
pub(crate) fn authenticate_production_contracts_chain_v23(
    dom_chain_adapter: &DomHttpChainAdapterV1,
    authenticated_bootstrap: &AuthenticatedContractsBootstrapV1,
) -> Result<TrustedChainIdV1, ProductionContractsSessionBootstrapErrorV1> {
    let expected_identity = dom_chain_adapter.expected_identity().clone();
