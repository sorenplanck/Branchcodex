...
#[path = "production_node_universal.rs"]
mod universal;
#[cfg(feature = "production")]
pub(crate) use universal::{
    read_production_run_secrets_v11_from_stdin, ProductionRunSecretsV11, ProductionSecretPartsV4,
};
pub use universal::{
    read_production_secrets_v4_from_stdin, ProductionLegCredentialsV4, ProductionSecretsV4,
    ProductionSecretsV4Error,
};
...
pub struct ProductionSecretsV1 {
    ...
    xmr_graph_vault_key_v23: crate::production_dom_vaults_v12::ProductionXmrGraphVaultKeyV23,
...
impl ProductionSecretsV1 {
    ...
    pub(crate) fn into_parts(self) -> ProductionSecretPartsV1 {
        ProductionSecretPartsV1 {
            ...
            xmr_graph_vault_key_v23: self.xmr_graph_vault_key_v23,
...
mod tests {
    ...
    #[cfg(feature = "production")]
    #[test]
    fn v11_runtime_reader_preserves_exact_v3_and_never_falls_back_between_headers() {
        let bytes = canonical_secret_stream_v3(&[b'1'], &[b'2']);
        assert!(matches!(
            ProductionRunSecretsV11::read(bytes.as_slice()),
            Ok(ProductionRunSecretsV11::LegacyV3(_))
        ));
        let text = String::from_utf8(bytes).unwrap();
        for malformed in [
            text.replace("DOM-INTEROPD-SECRETS-V3", "DOM-INTEROPD-SECRETS-V4"),
            text.replace("DOM-INTEROPD-SECRETS-V3", "DOM-INTEROPD-SECRETS-V2"),
            text + "\n",
        ] {
            assert!(ProductionRunSecretsV11::read(malformed.as_bytes()).is_err());
        }
        assert!(
            ProductionRunSecretsV11::read(canonical_secret_stream_v2_with(b'f').as_slice())
                .is_err()
        );
    }
