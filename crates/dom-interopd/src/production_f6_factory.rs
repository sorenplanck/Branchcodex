...
use claim_enrollment_v23::ClaimPlanProfileV23;
...
pub(crate) struct AuthenticatedProductionF6AuthorityBundleV7 {
    ...
    claim_profile: ClaimPlanProfileV23,
}
...
impl AuthenticatedProductionF6AuthorityBundleV7 {
    ...
    /// Maximum local age of a live XMR ownership/quorum proof. The value is
    /// authenticated inside the same threshold-signed F6 bundle.
    pub(crate) const fn inventory_proof_max_age_seconds(&self) -> u64 {
    ...
    pub(crate) fn decode_and_authenticate(
        ...
        if enrollment
            ...
            .any(|leg| {
                authenticated
                    .monero_session(*leg)
                    .and_then(|session| session.native_enrollment_v23())
                    .is_none()
            })
        {
            return Err(invalid_binding_diag_v25(line!()));
        ...
        let claim_profile = if enrollment {
            ...
            ClaimPlanProfileV23::Bound {
                ...
                downstream: downstream_source_scope,
            }
        };
...
pub(crate) enum ProductionF6CounterpartyTermsOwnerV7 {
    ...
    Solana(crate::production_f6::extended_terms::ProductionSolanaF6TermsOwnerV7),
    Monero(crate::production_f6::extended_terms::ProductionXmrF6TermsOwnerV7),
}

impl ProductionF6CounterpartyTermsOwnerV7 {
    /// One factory for all positions/families; extra Bitcoin payout authority
    /// is a contradiction, never an ignored credential.
    pub(crate) fn from_authenticated(
        inputs: &AuthenticatedProductionInputsV1,
        leg: route_executor::LegIdV1,
        payout: Option<AuthenticatedBitcoinPayoutFaceV1>,
    ) -> Result<Self, ProductionF6ActivationRefusalV2> {
        let present = [
        ...
        if let Some(session) = inputs.evm_session(leg) {
            ...
            Ok(Self::Solana(
                crate::production_f6::extended_terms::ProductionSolanaF6TermsOwnerV7::from_session(
                    session,
                ),
            ))
        } else if let Some(session) = inputs.monero_session(leg) {
            Ok(Self::Monero(
...
pub(crate) struct ProductionF6PairFactoryRequestV7 {
    ...
    /// Required live-custody/quorum proof for the native XMR enrollment profile.
    pub native_xmr_inventory:
        Option<crate::production_xmr_inventory_v23::ProductionXmrInventoryVerifiedV23>,
    pub native_xmr_inventory_required: bool,
...
impl AuthenticatedProductionF6FinalClaimPlanV7 {
    ...
    > {
        self.profile.into_bound()
    }

    pub(crate) fn requires_native_templates(&self) -> bool {
        matches!(self.profile, ClaimPlanProfileV23::Enrollment(_))
    }
...
pub(crate) struct ProductionF6PairAuthoritiesFactoryV7 {
    ...
    native_xmr_inventory:
        Option<crate::production_xmr_inventory_v23::ProductionXmrInventoryVerifiedV23>,
    native_xmr_inventory_required: bool,
...
impl ProductionF6PairAuthoritiesFactoryV7 {
    pub(crate) fn new(
        ...
        if bundle.network_id != route.network_id
            ...
            || native_xmr_inventory_required != native_xmr_inventory.is_some()
            || (native_xmr_inventory_required
                && !matches!(&bundle.claim_profile, ClaimPlanProfileV23::Enrollment(_)))
        ...
        let final_claim_plan = AuthenticatedProductionF6FinalClaimPlanV7 {
            profile: bundle.claim_profile.clone(),
            route_id: bundle.route_id,
            composition_digest: bundle.composition_digest,
            route_scope_digest: bundle.route_scope_digest,
        };
        Ok(Self {
            bundle,
            route,
    ...
    fn bind_pair_inner(
        ...
        let build_policy = if matches!(
            ...
            PreF6TimePolicyV2::from_registry
        };
        let upstream_policy = build_policy(
            upstream_scope,
            &self.route.registry,
...
fn counterparty_face(
    ...
    match owner {
        ...
        ProductionF6CounterpartyTermsOwnerV7::Solana(owner) => {
            owner.into_face(binding, settlement, composition)
        }
        ProductionF6CounterpartyTermsOwnerV7::Monero(owner) => {
            owner.into_face(binding, settlement, composition)
        }
    }
    .map_err(|_| invalid_binding_diag_v25(line!()))
...
fn verify_bundle_signatures(
    ...
    let digest = digest_parts(&[domain, signed_prefix])?;
