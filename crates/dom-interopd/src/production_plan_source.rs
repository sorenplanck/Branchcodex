...
use adapter_dom_real::{RealDomClaimConsumerV1, RealDomError, RealDomRpcRuntimeV1};
...
enum VaultRecoveryAuthorizationV1<'authority> {
    #[expect(
        dead_code,
        reason = "Pinned-txid constructor retained; runtime binds the lock before a claim txid exists"
    )]
    CanonicalOnly,
...
///
/// It parks the existing runtime until the route and the native Store both
/// authorize public extraction. No post-funding verifier is required during
/// startup, and no runtime accessor is exposed to the composition root.
...
impl ProductionDomPublicSecretConsumerAuthorityV1 {
    /// Minted only by DOM child composition from its same shared runtime.
    /// Verification material is read later from the sole Contracts opening.
...
impl ProductionDomPublicSecretSourceV1 {
    ...
    /// clone of its already-open Store. The runtime came from the same DOM
    /// child and remains parked in the shared slot until installation.
...
impl ProductionChainPublicSecretSourceV1 for ProductionInstalledDomPublicSecretSourceV1 {
    ...
    fn reextract_for_chain(
        ...
        let actuator = DomContractsActuatorV1::bind(self.store.as_ref(), self.binding)
            .map_err(map_dom_secret_source_error)?;
        // Select by retained gate, not by absence of a receiver-only record.
        // A sender re-extracts only from its native admission plus a fresh
        // canonical chain proof. It never fabricates receiver observation.
        if self
            .store
            .retained_f7_funding_gate_v19(self.trusted_chain_id, self.binding.session_id())
            .map_err(map_f7_secret_store_error_v15)?
            .is_some()
        {
            let facts = self
                .store
            ...
            if facts.receiver_id() != self.binding.participant().participant_id() {
                let admitted = self
                    .store
                    ...
                    .map_err(map_f7_secret_store_error_v15)?;
                if admitted.tx_hash() != self.expected_claim_transaction_id {
                    return Err(AuthorityRefusalV1::Inconsistent);
                }
                return self
                    .runtime
                    .consume_admitted_f7_claim_v21(&facts, &admitted)
                    .map_err(map_f7_secret_observation_error_v15);
            }
            let observed = self
                .store
                .resume_f7_claim_observation_v15(self.trusted_chain_id, self.binding.session_id())
                .map_err(map_f7_secret_store_error_v15)?
                .ok_or(AuthorityRefusalV1::Unavailable)?;
            if observed.session_id() != self.binding.session_id()
                ...
                || observed.observation_digest() == ZERO_DIGEST
            {
                return Err(AuthorityRefusalV1::Inconsistent);
            }
            return self
                .runtime
                .consume_observed_f7_claim_v15(&facts, &observed)
                .map_err(map_f7_secret_observation_error_v15);
        }
        match actuator
        ...
        let authorization = actuator
            .resume_consumed_final_claim_authority_v2(&self.trusted_chain_id)
            .map_err(map_dom_secret_source_error)?;
        let verifier = actuator
            .build_retained_claim_verifier_v2(&self.trusted_chain_id, &authorization)
            .map_err(map_dom_secret_source_error)?;
        let consumer = RealDomClaimConsumerV1::new(Arc::clone(&self.runtime), Arc::new(verifier));
        actuator
            .extract_observed_claim_secret_v2(&consumer, &observed)
            .map_err(map_dom_secret_source_error)
    }
}
...
fn map_f7_secret_store_error_v15(
    ...
    match error {
        ...
        _ => AuthorityRefusalV1::Inconsistent,
    }
}

fn map_f7_secret_observation_error_v15(error: RealDomError) -> AuthorityRefusalV1 {
    match error {
...
pub(crate) struct ProductionEvmPublicSecretSourceV1<R: JsonRpc> {
    ...
    composition_digest: Digest32,
    chain_id: Digest32,
    expected_claim_transaction_id: Option<Digest32>,
    lock_id: Digest32,
    adapter: std::rc::Rc<EvmAdapter<R>>,
}
...
impl<R: JsonRpc> ProductionEvmPublicSecretSourceV1<R> {
    /// Bind before a claim txid exists. The exact lock is authenticated by the
    /// retained refund adapter; the route's public exposure supplies the txid
    /// only after durable exposure authorization. Every extraction rereads and
    /// verifies the finalized event for that lock, including after restart.
    pub(crate) fn from_retained_adapter_v4(
        route_id: RouteIdV1,
        composition_digest: Digest32,
        adapter: std::rc::Rc<EvmAdapter<R>>,
        ...
        Ok(Self {
            ...
            chain_id,
            lock_id,
            expected_claim_transaction_id: None,
            adapter,
        })
    }
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            composition_digest,
            chain_id,
            expected_claim_transaction_id: Some(expected_claim_transaction_id),
            lock_id,
...
impl<R: JsonRpc> ProductionChainPublicSecretSourceV1 for ProductionEvmPublicSecretSourceV1<R> {
    ...
    fn reextract_for_chain(
        ...
        if request.route_id() != self.route_id
            ...
            || self
                .expected_claim_transaction_id
                .is_some_and(|txid| exposure.transaction_id != txid)
            || exposure.evidence_digest == ZERO_DIGEST
            || exposure.observed_at_unix_ms == 0
        {
            return Err(AuthorityRefusalV1::Inconsistent);
        ...
        let evidence = self
            ...
            .map_err(map_evm_secret_source_error)?;
        if evidence.tx_hash != exposure.transaction_id
...
#[cfg(test)]
#[path = "production_evm_recovery_tests.rs"]
mod evm_recovery_tests_v4;
...
#[cfg(test)]
mod f7_receiver_v15_tests {
    use super::*;
    #[test]
    fn v15_public_extraction_distinguishes_absent_from_substituted_evidence() {
        assert!(matches!(
            map_f7_secret_observation_error_v15(RealDomError::EvidenceNotFound),
            AuthorityRefusalV1::Unavailable
        ));
        assert!(matches!(
            map_f7_secret_observation_error_v15(RealDomError::InsufficientConfirmations),
            AuthorityRefusalV1::Unavailable
        ));
        assert!(matches!(
            map_f7_secret_observation_error_v15(RealDomError::InvalidEvidence),
            AuthorityRefusalV1::Inconsistent
        ));
        assert!(matches!(
            map_f7_secret_store_error_v15(dom_scriptless_store::SessionStoreError::Quarantined),
            AuthorityRefusalV1::Inconsistent
        ));
    }
}
