...
use adapter_btc_live::{
    ArmedBitcoinFundingV1, BitcoinCoreEvidenceCollectorV1, BitcoinCoreNetworkV1,
    BitcoinCoreRpcClientV1, BitcoinExternalFundingCustodyV1, BitcoinPrebroadcastStoreV1,
    BitcoinRefundDelayV1, FreshBitcoinClaimExtractionAuthorityV1,
    FreshBitcoinPreparedClaimPublicV1, LiveBitcoinError, ReopenedBitcoinFundingV1,
...
use btc_actuator::{
    ...
    BitcoinActuationScopeV1, BitcoinActuatorErrorV1, BitcoinAdaptorSecretV1,
    BitcoinClaimExtractionContextV1, BitcoinClaimSessionV1, BitcoinDurableOperationViewV1,
    BitcoinFeeBumpPolicyV1, BitcoinFundingCustodyViewV1, BitcoinOperationBindingViewV1,
    BitcoinOperationKindV1, BitcoinOperationLocatorV1, BitcoinOperationStageV1,
    BitcoinOperationViewV1, BitcoinPortCallJournalStatusV1, BitcoinPortCallKeyV1,
    BitcoinPortCallKindV1, BitcoinPortCallOutcomeV1, BitcoinPreSignatureV1,
...
use crate::production_bitcoin_claim_driver::{
    BitcoinPostAnchorBoundRequestV11, BitcoinPostAnchorCallV11, BitcoinPostAnchorErrorV8,
    BitcoinPostAnchorExternalResourcesV11, BitcoinPostAnchorResourcesV8,
    ProductionBitcoinPostAnchorDriverV8,
};
...
use crate::production_contracts::{
    ProductionBitcoinPrefundingReadinessV11, ProductionContractsConsumedPostAnchorV2,
};
...
pub(crate) struct ProductionBitcoinClaimMaterializationAuthorityV1 {
    ...
    participant_extraction: Option<BitcoinClaimExtractionContextV1>,
}
...
pub(crate) struct ProductionBitcoinPublicExtractionHandoffV1 {
    ...
    extraction: ProductionBitcoinPublicExtractionV3,
}

enum ProductionBitcoinPublicExtractionV3 {
    Fresh(FreshBitcoinClaimExtractionAuthorityV1),
    Participant {
        expected_txid: Digest32,
...
impl ProductionBitcoinPublicExtractionHandoffV1 {
    pub(crate) const fn leg(&self) -> SettlementLegV1 {
        self.leg
    }
    pub(crate) const fn settlement_id(&self) -> Digest32 {
        self.settlement_id
    }
    ...
    pub(crate) const fn expected_txid(&self) -> Digest32 {
        match &self.extraction {
            ProductionBitcoinPublicExtractionV3::Fresh(authority) => authority.expected_txid(),
            ProductionBitcoinPublicExtractionV3::Participant { expected_txid, .. } => {
                *expected_txid
            }
        }
    }
    ...
    pub(crate) fn extract_confirmed(
        &mut self,
    ) -> Result<counterparty_api::RevealedSecretBytes, ChildAuthorityRefusalV1> {
        match &mut self.extraction {
            ProductionBitcoinPublicExtractionV3::Fresh(authority) => {
                ...
                secret.move_into(&mut scalar);
                Ok(counterparty_api::RevealedSecretBytes::new(*scalar))
            }
            ProductionBitcoinPublicExtractionV3::Participant {
                expected_txid,
                context,
            } => {
                let evidence = BitcoinCoreEvidenceCollectorV1::new(&self.rpc)
                    .collect_confirmed(*expected_txid, self.minimum_confirmations)
                    .map_err(map_live_bitcoin_error)?;
                btc_actuator::extract_revealed_secret_from_confirmed_claim(
                    context,
...
impl ProductionBitcoinClaimMaterializationAuthorityV1 {
    pub(crate) fn bind_prepared_v11(
        binding: &ProductionBitcoinClaimBindingV11,
        leg: LegIdV1,
        session: BitcoinClaimSessionV1,
        ...
        Ok(Self {
            ...
            route_scope_digest: binding.route_scope_digest,
            composition_digest: binding.composition_digest,
            role_plan_digest: binding.role_plan_digest,
            source_scope_digest: binding.source_scope_digest,
            fresh_funding_owner_digest: None,
            fresh_public: None,
            participant_extraction: None,
        })
    }

    /// Reauthenticates the full session before any nonce is exposed. This is
    /// an identity check only; it grants neither funding nor M.8 authority.
    pub(crate) fn authenticate_session_v8(
        ...
        if role_plan.route_id() != inputs.admission().route_id()
            ...
            || session.actuation_terms_digest_v11()
                != inputs.admission().frozen_bindings().terms_digest
        ...
        session.session_digest().map_err(map_actuator_error)?;
        Ok(entry.secret_source_scope_digest())
    }
    ...
    pub(crate) fn bind(
        ...
        let source_scope_digest = Self::authenticate_session_v8(
            inputs,
            role_plan,
            upstream_scope,
            downstream_scope,
        ...
        if session.session_digest().map_err(map_actuator_error)? != pre_signature.session_digest() {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        let composition = inputs.composition();
        Ok(Self {
    ...
    pub(crate) fn bind_recovered_fresh_v1(
        ...
        if role_plan.route_id() != inputs.admission().route_id()
            ...
            || funding.leg != leg
            || funding.terms_digest != session.actuation_terms_digest_v11()
    ...
    fn authenticates_fresh_funding_owner(
        ...
        let (Some(expected), Some(public)) =
            ...
            return match &self.state {
                ProductionBitcoinClaimMaterializationStateV1::ActuatorReady(_) => {
                    self.authenticate_participant_funding(funding)
                }
    ...
    fn authenticate_participant_funding(
        &self,
        funding: &ProductionBitcoinFundingAuthorityV1,
    ) -> Result<(), ChildAuthorityRefusalV1> {
        Self::authenticate_funding_session_v9(&self.session, funding)
    }

    /// Checks the already authenticated physical funding owner before M.8
    /// exposes a nonce. The completed claim repeats this check on installation.
    pub(crate) fn authenticate_funding_session_v9(
        session: &BitcoinClaimSessionV1,
        funding: &ProductionBitcoinFundingAuthorityV1,
    ) -> Result<(), ChildAuthorityRefusalV1> {
        let summary = funding.armed.funding_summary();
        let network_matches = matches!(
            (session.network, funding.custody.network()),
        ...
        let delay_matches = matches!(
            (session.refund_delay, summary.refund_delay()),
            (BitcoinCsvDelayV1::Blocks(a), BitcoinRefundDelayV1::Blocks(b)) if a == b
        ) || matches!(
            (session.refund_delay, summary.refund_delay()),
            (BitcoinCsvDelayV1::Time512s(a), BitcoinRefundDelayV1::Time512Seconds(b)) if a == b
        );
        // The signing session already rebuilds the Taproot contract from its
        // roster/refund key/CSV. Comparing the script ties all of them to the
        // authenticated funding owner; a common route id alone is insufficient.
        if !network_matches
            || !delay_matches
            || session.route_id != funding.route_id
            || session.actuation_terms_digest_v11() != funding.terms_digest
            || session.terms_digest != funding.settlement_terms_digest
            || session.settlement_id != funding.settlement_id
            || session.registry_digest != funding.deployment.registry_digest()
            || session.profile_digest != funding.deployment.profile_digest()
            || session.funding_txid != funding.custody.funding_txid()
            || session.funding_vout != funding.custody.contract_vout()
            || session.funding_amount_sat != funding.custody.contract_amount_sat()
            || session.contract_script_pubkey != funding.armed.contract_script_pubkey()
            || session.deployment_digest
                != resolved_bitcoin_deployment_digest_v1(&funding.deployment)
                    .map_err(map_actuator_error)?
        {
    ...
    fn ensure_exact_claim(
        ...
        if request.action != SettlementActionV1::Claim
            ...
            || request.terms_digest != self.session.actuation_terms_digest_v11()
        ...
        let finalized = match state {
            ProductionBitcoinClaimMaterializationStateV1::ActuatorReady(pre_signature) => {
                let (exact, extraction) = pre_signature
                    .finalize_claim_with_extraction(secret)
                    .map_err(map_actuator_error)?;
                self.participant_extraction = Some(extraction);
                ProductionBitcoinClaimMaterializationStateV1::Finalized {
                    exact,
    ...
    fn take_fresh_extraction(
        &mut self,
        expected_txid: Digest32,
    ) -> Result<ProductionBitcoinPublicExtractionV3, ChildAuthorityRefusalV1> {
        if let ProductionBitcoinClaimMaterializationStateV1::Finalized {
            exact,
            durably_retained: true,
            ..
        } = &self.state
        {
            if self.participant_extraction.is_some() {
                let context = take_validated_handoff_authority(
                    &mut self.participant_extraction,
                    exact.txid() == expected_txid,
                )?;
                return Ok(ProductionBitcoinPublicExtractionV3::Participant {
                    expected_txid,
                    context,
                });
            }
        ...
        match &mut self.state {
            ProductionBitcoinClaimMaterializationStateV1::Finalized {
                ...
                take_validated_handoff_authority(fresh_extraction, authenticated)
                    .map(ProductionBitcoinPublicExtractionV3::Fresh)
            }
            ProductionBitcoinClaimMaterializationStateV1::RecoveryExtractionOnly {
...
/// Native receipt and bilateral recovery readiness frozen before funding.
/// The action effect is bound later by the real materializer, before any DOM
/// first exposure. No placeholder effect or nonce is allocated at bootstrap.
pub(crate) struct ProductionBitcoinPreparedClaimV11 {
    receipt: adapter_btc_live::BitcoinFreshRouteReceiptV1,
    readiness: ProductionBitcoinPrefundingReadinessV11,
    route_scope_digest: Digest32,
    composition_digest: Digest32,
    role_plan_digest: Digest32,
    source_scope_digest: Digest32,
    round: Option<ProductionBitcoinClaimBindingV11>,
    signing_scope: Option<BitcoinActuationScopeV1>,
    driver: ProductionBitcoinPostAnchorDriverV8,
}

/// Closed public session context derived only by the authenticated child's
/// actual action request. It cannot import a nonce, key or pre-signature.
pub(crate) struct ProductionBitcoinClaimBindingV11 {
    session: BitcoinClaimSessionV1,
    leg: LegIdV1,
    route_scope_digest: Digest32,
    composition_digest: Digest32,
...
impl ProductionBitcoinClaimBindingV11 {
    pub(crate) const fn session(&self) -> &BitcoinClaimSessionV1 {
        &self.session
    }
    ...
    pub(crate) fn require_role(
        ...
        if role.route_id() != self.session.route_id
            || role.session_id().0 != self.session.session_id
            || role.settlement_id().0 != self.session.settlement_id
            || role
                .terms_hash()
                .map_err(|_| ChildAuthorityRefusalV1::Conflict)?
                != self.session.terms_digest
            || role.composed_role_plan_digest() != self.role_plan_digest
            || role.composition_binding_digest() != self.composition_digest
            || role.secret_source_scope_digest() != self.source_scope_digest
            || role.adaptor_point_sec1() != self.session.adaptor_point
            || !matches!(
                (self.leg, role.route_leg()),
                (LegIdV1::Upstream, ComposedSettlementLegV1::Upstream)
                    | (LegIdV1::Downstream, ComposedSettlementLegV1::Downstream)
            )
        {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
...
impl ProductionBitcoinPreparedClaimV11 {
    pub(crate) fn authenticate_receipt_v11(
        inputs: &AuthenticatedProductionInputsV1,
        role_plan: &ComposedFinalClaimRolePlanV1,
        upstream_scope: &FinalClaimSecretSourceScopeV1,
        downstream_scope: &FinalClaimSecretSourceScopeV1,
        ...
        let scope = ProductionBitcoinMaterializationScopeV1::authenticate(
            inputs,
            role_plan,
            upstream_scope,
            downstream_scope,
        ...
        let role = readiness.role();
        let settlement = match leg {
            LegIdV1::Upstream => inputs.composition().upstream(),
            LegIdV1::Downstream => inputs.composition().downstream(),
        };
        let participant = inputs
            .bitcoin_session(leg)
            .ok_or(ChildAuthorityRefusalV1::Conflict)?;
        if role.route_id() != scope.route_id
            || role.session_id().0 != settlement.session_id.0
            || role.settlement_id().0 != scope.settlement_id
            || role.adaptor_point_sec1() != inputs.composition().adaptor_point_sec1()
            || role.composed_role_plan_digest() != scope.role_plan_digest
            || role.composition_binding_digest() != scope.composition_digest
            || role.secret_source_scope_digest() != scope.source_scope_digest
            || funding.leg != leg
            || funding.settlement_id != scope.settlement_id
            || receipt.funding_txid() != funding.custody.funding_txid()
            || receipt.contract_vout() != funding.custody.contract_vout()
            || receipt.contract_amount_sat() != funding.custody.contract_amount_sat()
            || receipt.contract_script_pubkey() != funding.armed.contract_script_pubkey()
            || receipt.claim_roster() != *participant.roster()
            || receipt.route_binding() != funding.custody.route_binding()
            || !matches!(
                (scope.leg, role.route_leg()),
                (SettlementLegV1::Upstream, ComposedSettlementLegV1::Upstream)
                    | (
        ...
        readiness.revalidate().map_err(map_readiness_error_v11)?;
        Ok(Self {
            receipt,
            readiness,
            ...
            round: None,
            signing_scope: None,
            driver: ProductionBitcoinPostAnchorDriverV8::default(),
        })
    }

    fn authenticates_scope(&self, scope: &ProductionBitcoinMaterializationScopeV1) -> bool {
        scope.route_id == self.readiness.role().route_id()
            && scope.settlement_id == self.readiness.role().settlement_id().0
            && scope.route_scope_digest == self.route_scope_digest
            && scope.composition_digest == self.composition_digest
            && scope.role_plan_digest == self.role_plan_digest
            && scope.source_scope_digest == self.source_scope_digest
            && matches!(
                (scope.leg, self.readiness.role().route_leg()),
                (SettlementLegV1::Upstream, ComposedSettlementLegV1::Upstream)
                    | (
                        SettlementLegV1::Downstream,
                        ComposedSettlementLegV1::Downstream
    ...
    fn session_for_request(
        ...
        request: &ProductionChildMaterializationRequestV1,
        funding: &ProductionBitcoinFundingAuthorityV1,
    ) -> Result<BitcoinClaimSessionV1, ChildAuthorityRefusalV1> {
        let receipt = &self.receipt;
        let network = match funding.custody.network() {
            BitcoinCoreNetworkV1::Regtest => BitcoinNetworkV1::Regtest,
            BitcoinCoreNetworkV1::PublicSignet => BitcoinNetworkV1::PublicSignet,
            BitcoinCoreNetworkV1::CustomSignet => BitcoinNetworkV1::CustomSignet,
        };
        let refund_delay = match funding.armed.funding_summary().refund_delay() {
            BitcoinRefundDelayV1::Blocks(n) => BitcoinCsvDelayV1::Blocks(n),
            BitcoinRefundDelayV1::Time512Seconds(n) => BitcoinCsvDelayV1::Time512s(n),
        };
        let session = BitcoinClaimSessionV1 {
            ...
            session_id: self.readiness.role().session_id().0,
            terms_digest: funding.settlement_terms_digest,
            route_terms_digest_v11: Some(funding.terms_digest),
            registry_digest: funding.deployment.registry_digest(),
            profile_digest: funding.deployment.profile_digest(),
            deployment_digest: resolved_bitcoin_deployment_digest_v1(&funding.deployment)
                .map_err(map_actuator_error)?,
            network,
            ...
            contract_script_pubkey: receipt.contract_script_pubkey().to_vec(),
            ...
            adaptor_point: self.readiness.role().adaptor_point_sec1(),
            attempt: 0,
        };
        session.session_digest().map_err(map_actuator_error)?;
...
fn map_readiness_error_v11(
    error: dom_scriptless_store::SessionStoreError,
) -> ChildAuthorityRefusalV1 {
    match error {
        dom_scriptless_store::SessionStoreError::SessionNotFound
        | dom_scriptless_store::SessionStoreError::FundingAuthorityUnavailable => {
            ChildAuthorityRefusalV1::Refused
        }
        _ => ChildAuthorityRefusalV1::Conflict,
    }
}
...
impl ProductionBitcoinFundingAuthorityV1 {
    pub(crate) const fn settlement_id_v7(&self) -> Digest32 {
        self.settlement_id
    }

    pub(crate) fn post_anchor_network_v11(&self) -> (u32, [u8; 32]) {
        (
            self.deployment.profile().finality.min_confirmations,
    ...
    pub(crate) fn new(
        ...
        let settlement_terms_digest = settlement
            .terms_hash()
            .map_err(|_| ChildAuthorityRefusalV1::Conflict)?;
...
pub(crate) struct ProductionBitcoinChildPortV1<R, C> {
    ...
    prepared_claim: Option<ProductionBitcoinPreparedClaimV11>,
}
    ...
    C: ProductionBitcoinChildClockV1,
{
    ...
    /// Renew only this child's live native fencing generation. Historical
    /// operation scopes retain their own expiry and are never rewritten.
    pub(crate) fn with_lease_renewal_v12(
        mut self,
        duration_ms: Option<u64>,
    ) -> Result<Self, ChildAuthorityRefusalV1> {
        if duration_ms.is_some_and(|duration| duration == 0 || duration > 3_600_000) {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        self.lease_renewal_ms_v12 = duration_ms;
    ...
    /// Builds the live funding/refund child from bilateral native readiness.
    /// Claim is still unavailable; the same child installs it only after its
    /// retained actuator completes the post-anchor M.8 exchange.
    pub(crate) fn new_prefunding_v11(
        actuator: DurableBitcoinActuatorV1,
        rpc: R,
        lease: BitcoinStorageLeaseStatusV1,
        clock: C,
        ...
        if !prepared.authenticates_scope(&scope) || route_leg(scope.leg) != funding.leg {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        prepared
            .readiness
            .revalidate()
            .map_err(map_readiness_error_v11)?;
        let mut port = Self::new(actuator, rpc, lease, clock, funding)?;
        port.materialization_scope = Some(scope);
        port.prepared_claim = Some(prepared);
        Ok(port)
    }

    fn install_post_anchor_claim_v11(
        &mut self,
        claim: ProductionBitcoinClaimMaterializationAuthorityV1,
    ) -> Result<(), ChildAuthorityRefusalV1> {
        let prepared = self
        ...
        if self.claim.is_some()
            || Some(&claim.session) != prepared.round.as_ref().map(|round| &round.session)
            || claim.route_scope_digest != prepared.route_scope_digest
            || claim.composition_digest != prepared.composition_digest
            || claim.role_plan_digest != prepared.role_plan_digest
            || claim.source_scope_digest != prepared.source_scope_digest
        {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        claim.authenticates_fresh_funding_owner(&self.funding)?;
    ...
    pub(crate) fn take_fresh_public_extraction_handoff(
        ...
        if expected.settlement_id != self.funding.settlement_id
            || route_leg(expected.leg) != self.funding.leg
            || claim.session.route_id != self.funding.route_id
    ...
    pub(crate) fn restore_fresh_public_extraction_handoff(
        ...
        let chain_id = self.funding.deployment.profile().chain_id.0;
        if handoff.settlement_id != self.funding.settlement_id
            || route_leg(handoff.leg) != self.funding.leg
            || self.returned_extraction_handoff.is_some()
    ...
    C: ProductionBitcoinChildClockV1,
{
    ...
    fn settlement_id(&self) -> Option<Digest32> {
        Some(self.funding.settlement_id)
    }
    ...
    fn renew_actuator_lease_v12(&mut self) -> Result<(), ChildAuthorityRefusalV1> {
        ...
        if remaining <= duration / 2 {
            self.lease = renew_bitcoin_lease_v12(&mut self.actuator, self.lease, now, duration)?;
        }
        Ok(())
    }
    ...
    fn prepare_bitcoin_claim_v11(
        ...
        let scope = self
            ...
            .as_ref()
            .ok_or(ChildAuthorityRefusalV1::Refused)?;
        if !scope.validates(&request)
            || request.action != SettlementActionV1::Claim
            || !matches!(
                request.exposure,
            ...
            || request.registry_digest != self.funding.deployment.registry_digest()
            || request.profile_digest != self.funding.deployment.profile_digest()
            || request.deployment_digest
                != resolved_bitcoin_deployment_digest_v1(&self.funding.deployment)
                    .map_err(map_actuator_error)?
        ...
        let Some(prepared) = self.prepared_claim.as_mut() else {
            return if self.claim.is_some() {
                Ok(())
            } else {
                Err(ChildAuthorityRefusalV1::Refused)
            };
        };
        let session = prepared.session_for_request(&request, &self.funding)?;
        if let Some(previous) = prepared.round.as_ref() {
            if previous
                .session
                .session_digest()
                .map_err(map_actuator_error)?
        ...
        // Storage renewal must not silently extend a previously issued claim
        // scope. Its expiry participates in the exact durable custody digest.
        let signing_until = prepared
            .signing_scope
            .as_ref()
            .map(BitcoinActuationScopeV1::valid_until_ms)
            .unwrap_or(self.lease.expires_at_ms());
        if now >= signing_until {
            return Err(ChildAuthorityRefusalV1::Unavailable);
        }
        let signing_scope =
            BitcoinActuationScopeV1::authorize(BitcoinActuationScopeAuthorizationV1 {
                deployment: &self.funding.deployment,
                route_id: session.route_id,
                effect_id: session.effect_id,
                fence_epoch: session.fence_epoch,
                leg: match self.funding.leg {
                ...
                expected_txid: session
                    .expected_claim_txid_v11()
                    .map_err(map_actuator_error)?,
                intent_digest: session.session_digest().map_err(map_actuator_error)?,
                contract_outpoint: Some(btc_actuator::BitcoinOutpointV1 {
                    txid: session.funding_txid,
                    vout: session.funding_vout,
                }),
                contract_amount_sat: session.funding_amount_sat,
                refund_record_digest: None,
                fee_policy: BitcoinFeeBumpPolicyV1 {
                    initial_fee_sat: session.fee_sat,
                    maximum_fee_sat: session.fee_sat,
                    maximum_fee_rate_sat_vbyte: self
                        .funding
                        .deployment
                        .deployment()
                ...
                valid_until_ms: signing_until,
            })
            .map_err(map_actuator_error)?;
        if prepared
            .signing_scope
            .as_ref()
            .is_some_and(|previous| previous.scope_digest() != signing_scope.scope_digest())
        {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        if let Some(claim) = self.claim.as_mut() {
            if claim.session.session_digest().map_err(map_actuator_error)?
                != session.session_digest().map_err(map_actuator_error)?
            {
                return Err(ChildAuthorityRefusalV1::Conflict);
            }
            claim.session.fence_epoch = session.fence_epoch;
        }
        prepared.round = Some(ProductionBitcoinClaimBindingV11 {
            session,
            leg: self.funding.leg,
            route_scope_digest: prepared.route_scope_digest,
            composition_digest: prepared.composition_digest,
            role_plan_digest: prepared.role_plan_digest,
            source_scope_digest: prepared.source_scope_digest,
        });
        prepared.signing_scope = Some(signing_scope);
        if self.claim.is_some() {
            Ok(())
        } else {
    ...
    fn drive_bitcoin_post_anchor_v11(
        ...
        if call.route_id != self.funding.route_id
            || call.settlement_id != self.funding.settlement_id
            || call.composition_digest != prepared.composition_digest
            || resources.participant.leg() != self.funding.leg
        {
            return Err(BitcoinPostAnchorErrorV8::Scope);
        }
        let binding = prepared
            .round
            .as_ref()
            .ok_or(BitcoinPostAnchorErrorV8::AwaitingClaimMaterialization)?;
        let scope = prepared
            .signing_scope
            .as_ref()
            .ok_or(BitcoinPostAnchorErrorV8::AwaitingClaimMaterialization)?;
        ...
        let (dom_funding_txid, dom_claim_round_transcript) = prepared
            .readiness
            .post_anchor_request_facts_v11()
            .map_err(|_| BitcoinPostAnchorErrorV8::Scope)?;
        let completed = prepared.driver.drive_bound_v11(
            BitcoinPostAnchorBoundRequestV11 {
                binding,
                role_binding: prepared.readiness.role(),
                ready: prepared.readiness.ready(),
                policy: prepared.readiness.policy(),
                scope,
                funding: &self.funding,
                dom_funding_txid,
                dom_claim_round_transcript,
            },
            BitcoinPostAnchorResourcesV8 {
                relay: resources.relay,
                scanner: resources.scanner,
                participant: resources.participant,
                actuator: &mut self.actuator,
                bitcoin: self.funding.rpc.as_ref(),
                transport: resources.transport,
            },
        )?;
        self.install_post_anchor_claim_v11(completed.claim)
            .map_err(|_| BitcoinPostAnchorErrorV8::Scope)?;
        Ok(Some(completed.contracts))
    }
    ...
    fn materialize(
        ...
        self.renew_actuator_lease_v12()?;
        if request.action == SettlementActionV1::Claim {
            self.prepare_bitcoin_claim_v11(request)?;
        }
    ...
    fn externalize(
        ...
        let outcome = match validated.expected.action {
            SettlementActionV1::Funding => {
                ...
                if let Some(prepared) = self.prepared_claim.as_ref() {
                    // A stored plan is not a durable permission to ignore a
                    // subsequently aborted or contradicted DOM recovery path.
                    prepared
                        .readiness
                        .revalidate()
                        .map_err(map_readiness_error_v11)?;
...
fn handoff_authenticates_expected(
    ...
    handoff.leg == expected.leg
        && handoff.settlement_id == expected.settlement_id
...
fn renew_bitcoin_lease_v12(
    actuator: &mut DurableBitcoinActuatorV1,
    lease: BitcoinStorageLeaseStatusV1,
    now: u64,
    duration: u64,
) -> Result<BitcoinStorageLeaseStatusV1, ChildAuthorityRefusalV1> {
    ...
    actuator
        .renew_lease(lease.fence_epoch(), now, duration)
        .map_err(map_actuator_error)
}
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn v12_bitcoin_renewal_keeps_fence_and_refuses_expired_or_stale_epoch() {
        use std::os::unix::fs::PermissionsExt;
        let root = tempfile::tempdir().unwrap();
        std::fs::set_permissions(root.path(), std::fs::Permissions::from_mode(0o700)).unwrap();
        let mut actuator =
            DurableBitcoinActuatorV1::create(&root.path().join("lease.sqlite"), [82; 32]).unwrap();
        let first = actuator.acquire_lease(1_000, 1_000).unwrap();
        let renewed = renew_bitcoin_lease_v12(&mut actuator, first, 1_600, 1_000).unwrap();
        assert_eq!(renewed.fence_epoch(), first.fence_epoch());
        assert_eq!(renewed.expires_at_ms(), 2_600);
        assert!(renew_bitcoin_lease_v12(&mut actuator, renewed, 2_600, 1_000).is_err());
        let takeover = actuator.acquire_lease(2_601, 1_000).unwrap();
        assert_eq!(takeover.fence_epoch(), renewed.fence_epoch() + 1);
        assert!(renew_bitcoin_lease_v12(&mut actuator, renewed, 2_602, 1_000).is_err());
        assert!(renew_bitcoin_lease_v12(&mut actuator, takeover, 2_700, 0).is_err());
        assert!(renew_bitcoin_lease_v12(&mut actuator, takeover, 2_700, u64::MAX).is_err());
        let still_current = renew_bitcoin_lease_v12(&mut actuator, takeover, 2_800, 1_000).unwrap();
        assert_eq!(still_current.fence_epoch(), takeover.fence_epoch());
    }
    ...
    assert_not_impl_any!(ProductionBitcoinPreparedClaimV11: Clone, Copy, core::fmt::Debug);
    assert_not_impl_any!(ProductionBitcoinPrefundingReadinessV11: Clone, Copy, core::fmt::Debug);
