...
#[path = "production_xmr_downstream_claim_observation_v23.rs"]
mod downstream_claim_observation_v23;
#[path = "production_xmr_graph_commit_runtime_v23.rs"]
mod graph_commit_runtime_v23;
#[path = "production_relay_xmr_signing_v23.rs"]
mod xmr_recovery_signing_v23;
#[path = "production_xmr_refund_template_binding_v23.rs"]
mod xmr_refund_template_binding_v23;
pub(crate) use xmr_recovery_signing_v23::ProductionXmrRecoverySigningOwnerV23;
#[path = "production_xmr_auxiliary_relay_v23.rs"]
mod xmr_auxiliary_relay_v23;
pub(crate) use xmr_auxiliary_relay_v23::XmrAuxiliaryRelayOpenModeV23;
#[path = "production_bootstrap_runtime_v16.rs"]
...
#[path = "production_contracts_graph_pin_v23.rs"]
mod graph_pin_v23;
...
#[path = "production_dom_claim_runtime_v12.rs"]
mod dom_claim_runtime_v12;
pub(crate) use dom_claim_runtime_v12::{
    ProductionDomClaimAnchorsV12, ProductionDomClaimCompletionV12,
...
pub(crate) use f7_readiness_v19::ProductionF7ReadinessErrorV19;

#[path = "production_f7_runtime_v12.rs"]
mod f7_runtime_v12;
...
mod post_m8_claim_v22;
pub(crate) use post_m8_claim_v22::ProductionPostM8ErrorV22;

#[path = "production_claim_owner_v21.rs"]
mod claim_owner_v21;
#[path = "production_dom_xmr_refund_readiness_v23.rs"]
mod dom_xmr_refund_readiness_v23;

#[path = "production_final_claim_v14.rs"]
mod final_claim_v14;
pub(crate) use final_claim_v14::{
    ProductionF7ClaimAdaptationV14, ProductionF7FinalClaimErrorV14,
    ProductionF7FinalClaimRequestV14, ProductionF7FinalClaimStepV14,
};
...
use adapter_dom_real::{RealDomClaimConsumerV1, RealDomError};
use dom_actuator::{
...
use dom_scriptless_store::{
    ...
    PreparedPostAnchorClaimPreSignatureTransportAuthorityV2, PreparedXmrRemoteSweepImportV23,
    SessionStoreError,
};
...
impl<F: F6TransportPortV1> ProductionContractsV1<F> {
    /// Reopen a real V22 graph under this leg's existing Contracts opening.
    /// Missing custody or a legacy unconditional graph cannot create a child.
    pub(crate) fn reopen_xmr_recovery_v22(
        ...
        client: crate::production_child_dom::ProductionDomXmrRecoveryClientV12,
    ) -> Result<
    ...
    > {
        ...
        let scope = self
            ...
            .map_err(|_| ChildAuthorityRefusalV1::Conflict)?;
        let custody = Rc::new(
            dom_scriptless_store::XmrRecoveryCustodyV11::open_existing(
                parent, directory, scope, key,
            )
            .map_err(|_| ChildAuthorityRefusalV1::Conflict)?,
        );
        custody
...
/// Retained same-Store proof of bilateral readiness for a Bitcoin funding
/// child. It is deliberately distinct from the later M.8 claim authority.
/// Holding the wrapper is insufficient: dispatch rechecks the signed native
/// ancestry through its retained physical Store before exposing funding.
pub(crate) struct ProductionBitcoinPrefundingReadinessV11 {
    store: Rc<ContractsSessionStoreV1>,
    chain: TrustedChainIdV1,
    role: dom_final_claim_binding::FinalClaimRoleBindingV1,
    ready: dom_final_claim_binding::OperationalM8ReadyBindingV2,
    policy: adapter_btc::timelock::M8TimingPolicyV1,
}

impl ProductionBitcoinPrefundingReadinessV11 {
    pub(crate) fn revalidate(&self) -> Result<(), SessionStoreError> {
        self.store.require_operational_m8_bilateral_readiness_v11(
            &self.chain,
            &self.role,
            &self.ready,
        )
    ...
    pub(crate) const fn role(&self) -> &dom_final_claim_binding::FinalClaimRoleBindingV1 {
        &self.role
    }

    pub(crate) const fn ready(&self) -> &dom_final_claim_binding::OperationalM8ReadyBindingV2 {
        &self.ready
    }

    pub(crate) const fn policy(&self) -> &adapter_btc::timelock::M8TimingPolicyV1 {
        &self.policy
    }

    pub(crate) fn post_anchor_request_facts_v11(
        &self,
    ) -> Result<([u8; 32], [u8; 32]), SessionStoreError> {
        self.store
            .bitcoin_post_anchor_request_facts_v11(&self.chain, self.role.session_id().0)
    }
}
...
impl ProductionContractsConsumedPostAnchorV2 {
    /// Rechecks the same live owner against fresh real-chain F7 evidence.
    pub(crate) fn refresh_with_f7_v8(
        &self,
        verified: VerifiedF7AnchorAuthorizationV2,
    ) -> Result<(), ProductionContractsPostAnchorErrorV2> {
        self.store
            .revalidate_consumed_with_f7_v8(&self.authorization, verified)
            .map_err(|_| ProductionContractsPostAnchorErrorV2::StoreRefused)
    }
...
pub(crate) struct ProductionDomRefundStoreFaceV1 {
    ...
    native_xmr_refund_v23: Option<Rc<RefCell<claim_owner_v21::ProductionClaimOwnerV21>>>,
}
...
impl ProductionDomRefundStoreFaceV1 {
    ...
    /// Read-only extraction authority retaining this exact Store opening.
    /// This does not issue another refund broadcast or signing authority.
    pub(crate) fn refund_reveal_source_v10(
        &self,
        scanner: crate::production_child_dom::ProductionDomRefundScannerV10,
        pre: dom_scriptless_crypto::VerifiedRefundPreSignatureV1,
        kernel_index: u32,
        minimum_confirmations: u32,
        max_reorg_depth: u32,
    ) -> Result<ProductionDomRefundRevealSourceV10, settlement_coordinator::ChildAuthorityRefusalV1>
    {
        use settlement_coordinator::ChildAuthorityRefusalV1 as Refusal;
        // Bind the read-only source before funding. Its first observation
        // authenticates the committed funding/refund facts, so preparing the
        // source need not wait for funding or the later claim-signing round.
        if pre.chain_id() != self.trusted_chain_id.as_bytes()
            || minimum_confirmations == 0
            || pre.session_id() != &self.binding.session_id()
            || max_reorg_depth < minimum_confirmations
        {
            return Err(Refusal::Conflict);
        }
        Ok(ProductionDomRefundRevealSourceV10 {
...
/// Scope-bound DOM refund observer. No scalar or caller-shaped success flag
/// can replace canonical DOM evidence and the retained Contracts artifact.
pub(crate) struct ProductionDomRefundRevealSourceV10 {
    store: Rc<ContractsSessionStoreV1>,
    session_id: [u8; 32],
    trusted_chain_id: TrustedChainIdV1,
    scanner: crate::production_child_dom::ProductionDomRefundScannerV10,
...
impl ProductionDomRefundRevealSourceV10 {
    pub(crate) fn require_binding(
        ...
        minimum_confirmations: u32,
        max_reorg_depth: u32,
    ) -> Result<(), settlement_coordinator::ChildAuthorityRefusalV1> {
        if self.session_id != session
            || self.pre.chain_id() != &chain
            || self.pre.template_hash() != &template
            || self.pre.refund_adaptor_point() != point
    ...
    pub(crate) fn observe(
        ...
        adapter_dom_real::VerifiedDomRefundSecretV10,
        settlement_coordinator::ChildAuthorityRefusalV1,
    > {
        use settlement_coordinator::ChildAuthorityRefusalV1 as Refusal;
        let facts = self
            .store
            .real_dom_refund_facts_v11(self.trusted_chain_id, self.session_id)
            .map_err(|error| match error {
                SessionStoreError::SessionNotFound => Refusal::Unavailable,
                _ => Refusal::Conflict,
            })?;
        let evidence = kaystra_core::state::EvidenceRefV1 {
            chain_id: kaystra_core::types::ChainId(*facts.chain_id()),
            tx_id: *facts.refund_tx_hash(),
            event_index: 0,
            block_height: 0,
            block_anchor: [0; 32],
        };
        self.scanner
            .extract(
                &self.store,
                adapter_dom_real::DomRefundExtractionRequestV10 {
                    trusted_chain_id: self.trusted_chain_id,
                    session_id: self.session_id,
                    evidence: &evidence,
                    pre_signature: &self.pre,
                    expected_refund_point: self.pre.refund_adaptor_point(),
            ...
            .map_err(|error| match error {
                // An honest absent/immature transaction can be observed later.
                adapter_dom_real::RealDomError::EvidenceNotFound
                | adapter_dom_real::RealDomError::InsufficientConfirmations
...
pub(crate) struct ProductionDomChildStoreAuthorityV1 {
    ...
    claim_owner_v21: Rc<RefCell<claim_owner_v21::ProductionClaimOwnerV21>>,
}
...
impl ProductionDomChildStoreAuthorityV1 {
    ...
    pub(crate) fn stage_f7_claim_admission_v21(
        &mut self,
        chain: &TrustedChainIdV1,
        admitted: dom_actuator::DomF7FinalClaimAdmissionV14,
    ) -> Result<(), ProductionContractsOutboundErrorV1> {
        self.final_claim_transport
            .stage_f7(chain, admitted.into_transport_authority())
    }
    ...
    F: F6TransportPortV1,
{
    fn stage_f7(
        ...
        if admitted.session_id() != self.session_id {
            return Err(SessionStoreError::InvalidTransition.into());
        }
        let retained = self
            .store
            .resume_f7_final_claim_admission_v14(*chain, self.session_id)?;
        if retained.tx_hash() != admitted.tx_hash()
            || retained.exposure_digest() != admitted.exposure_digest()
            || retained.admission_digest() != admitted.admission_digest()
        {
            return Err(SessionStoreError::InvalidTransition.into());
        }
        ...
        {
            Progress::TransportReconciled => Ok(()),
            Progress::TransportCommitted => {
                let OutboundDsc1RecoveryV1::Committed(outbound) = pending else {
                    return Err(SessionStoreError::InvalidTransition.into());
                };
                self.store
                    .revalidate_committed_f7_final_claim_transport_v14(*chain, &outbound)?;
                self.relay
                    .try_borrow_mut()
            ...
            Progress::NeedsAdaptation | Progress::Exposed => {
                Err(SessionStoreError::InvalidTransition.into())
            }
        }
    }
    ...
    F: F6TransportPortV1,
{
    ...
    xmr_remote_transport_authority_issued: Cell<bool>,
}
    ...
    F: F6TransportPortV1,
{
    ...
    /// Retains one consumed post-anchor owner through initial publication,
    /// reconnects and process recovery. A new F7 result is required to enter.
    pub(crate) fn consume_or_resume_post_anchor_v8(
        &self,
        verified: VerifiedF7AnchorAuthorizationV2,
    ) -> Result<ProductionContractsConsumedPostAnchorV2, ProductionContractsPostAnchorErrorV2> {
        if verified.session_id() != &self.session_id || verified.route_id() != &self.route_id {
            return Err(ProductionContractsPostAnchorErrorV2::StoreRefused);
        }
        let authorization = self
            .store
            .consume_or_resume_post_anchor_with_f7_v8(verified)
            .map_err(|_| ProductionContractsPostAnchorErrorV2::StoreRefused)?;
        Ok(ProductionContractsConsumedPostAnchorV2 {
            store: Rc::clone(&self.store),
            authorization,
    ...
    /// Retains the exact prepared/refund-armed Contracts owner for a Bitcoin
    /// child that will run M.8 only after both funding anchors are verified.
    /// No claim nonce, signing token or broadcast authority is issued here.
    pub(crate) fn bitcoin_prefunding_readiness_v11(
        &self,
        binding: DomSessionBindingV1,
        role: &dom_final_claim_binding::FinalClaimRoleBindingV1,
        ready: &dom_final_claim_binding::OperationalM8ReadyBindingV2,
    ) -> DomActuatorResult<ProductionBitcoinPrefundingReadinessV11> {
        self.validate_dom_binding(binding)?;
        if role.route_id() != self.route_id
            || role.session_id().0 != self.session_id
            || role.dom_chain_id().0 != binding.chain_id()
            || role
                .terms_hash()
                .map_err(|_| DomActuatorError::InvalidBinding)?
                != binding.terms_digest()
            || ready.session_id().0 != self.session_id
        {
        ...
        let (_, _, policy) = self
            .store
            .resume_operational_m8_bilateral_bindings_v11(&chain, self.session_id)
            .map_err(|_| DomActuatorError::InvalidStage)?;
        let authority = ProductionBitcoinPrefundingReadinessV11 {
            store: Rc::clone(&self.store),
            chain,
            role: role.clone(),
            ready: ready.clone(),
    ...
    pub(crate) fn bitcoin_retained_prefunding_readiness_v11(
        &self,
        binding: DomSessionBindingV1,
    ) -> DomActuatorResult<ProductionBitcoinPrefundingReadinessV11> {
        self.validate_dom_binding(binding)?;
        ...
        let (role, ready, _) = self
            .store
            .resume_operational_m8_bilateral_bindings_v11(&chain, self.session_id)
            .map_err(|_| DomActuatorError::InvalidStage)?;
        self.bitcoin_prefunding_readiness_v11(binding, &role, &ready)
    }

    /// Retain the same physical Contracts owner for a selected XMR recovery
    /// driver. The driver reauthorizes against signed readiness and exact DOM
    /// funding on every tick; it never exposes the underlying Store or keys.
    pub(crate) fn xmr_recovery_driver_v12(
        &self,
        binding: DomSessionBindingV1,
        gate: Rc<dom_scriptless_store::PreparedOperationalXmrFundingGateV12>,
        custody: Rc<dom_scriptless_store::XmrRecoveryCustodyV11>,
        client: crate::production_child_dom::ProductionDomXmrRecoveryClientV12,
    ) -> DomActuatorResult<crate::production_xmr_recovery_driver_v12::ProductionXmrRecoveryDriverV12>
    {
        self.validate_dom_binding(binding)?;
        if custody.scope().binding.session_id != self.session_id
            || custody.scope().binding.chain_id != binding.chain_id()
            || custody.scope().binding.terms_hash != binding.terms_digest()
        {
            return Err(DomActuatorError::InvalidBinding);
        }
        crate::production_xmr_recovery_driver_v12::ProductionXmrRecoveryDriverV12::new(
    ...
    /// Issues the sole XMR 0x19/0x1a transport face from this exact Store/Relay
    /// opening. The Store chooses and authenticates requester/signer identities;
    /// this face cannot sign a Monero transaction or import an EVM response.
        ...
        F: 'static,
    {
        ...
        Ok(Box::new(ProductionXmrRemoteContractsAuthorityV23 {
            session_id: self.session_id,
            route_id: self.route_id,
            local_participant: self.local_participant,
            remote_participant: self.remote_participant,
    ...
    /// Issues the mutually exclusive signer-side face for a local XMR claim
    /// receiver. Requester and responder cannot both be issued from one leg
    /// opening, preventing one process from acquiring both role surfaces.
    ...
    pub(crate) fn refresh_reissued_contracts_ingress_v16(
        &mut self,
        authority: PreparedContractsIngressV1,
    ) -> Result<(), ContractsRelayIngressErrorV1> {
        self.relay
            .try_borrow_mut()
            .map_err(|_| ContractsRelayIngressErrorV1::OwnerBusy)?
            .refresh_reissued_contracts_ingress_v16(authority)
    }
...
mod tests {
    ...
    #[test]
    fn v19_missing_f7_gate_is_not_a_missing_session() -> TestResult {
        let fixture = TestFixture::new(IdentityPlacement::Local, false)?;
        let before = fixture.store.load_session(SESSION)?;
        assert!(fixture
            .store
            .retained_f7_funding_gate_v19(fixture.chain, SESSION)?
            .is_none());
        assert!(matches!(
            fixture
                .store
                .retained_f7_funding_gate_v19(fixture.chain, OTHER_SESSION),
            Err(SessionStoreError::SessionNotFound)
        ));
        assert_eq!(
            fixture.store.load_session(SESSION)?.as_bytes(),
            before.as_bytes()
        );
        Ok(())
    }
    ...
    #[test]
    fn v19_readiness_scheduler_does_not_steal_an_unrelated_pending_request() -> TestResult {
        let fixture = TestFixture::new(IdentityPlacement::Local, false)?;
        fixture
            .store
            .bind_local_transport_signer(SESSION, fixture.local_key_reference)?;
        let request = prepare_abort_request(&fixture.store, fixture.chain, SESSION, [0x81; 32])?;
        ...
        let before = owner.store.load_session(SESSION)?;
        assert_eq!(
        ...
        let OutboundDsc1RecoveryV1::SigningRequest(retained) =
            ...
            panic!("the existing request must remain pending");
        };
        assert_eq!(retained.request_id(), &request_id);
        assert_eq!(
            owner.store.load_session(SESSION)?.as_bytes(),
            before.as_bytes()
        );
        Ok(())
    }
