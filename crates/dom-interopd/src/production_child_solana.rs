    ...
    pub(crate) fn authenticate(
        ...
        Ok(Self {
            ...
            setup_binding_hash: inputs
                ...
                .setup()
                .binding_hash(),
            deployment_digest: resolved_solana_deployment_digest_v1(
                inputs
                    .solana_session(leg)
                    .ok_or(ChildAuthorityRefusalV1::Conflict)?
                    .deployment(),
...
pub(crate) struct ProductionSolanaChildPortV1<R, C> {
    ...
    token_accounts_v7: Option<crate::production_solana_signer::ProductionSolanaTokenAccountsV7>,
...
impl<R: SolanaRpc, C: ProductionSolanaChildClockV1> ProductionSolanaChildPortV1<R, C> {
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            token_accounts_v7: None,
            lease_owner_v11: None,
        })
    }

    /// Install the finite-lease issuer that retains this physical store's
    /// process lock. Legacy/test constructors keep their explicit fixed lease.
    pub(crate) fn with_lease_owner_v11(
        ...
        if self.lease_owner_v11.is_some()
            || self.funder_lease.fencing_epoch() != self.beneficiary_lease.fencing_epoch()
        {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        owner.require_scope(
            crate::production_config::ProductionChainFamilyV11::Sol,
            self.setup.binding_hash(),
            self.deployment.deployment().genesis_hash,
            self.funder_lease.fencing_epoch(),
        )?;
        self.lease_owner_v11 = Some(owner);
        Ok(self)
    }

    fn refresh_leases_v11(&mut self, now: u64) -> Result<(), ChildAuthorityRefusalV1> {
        if let Some(owner) = &self.lease_owner_v11 {
            let funder = owner.solana_lease(self.setup.funder(), now)?;
            let beneficiary = owner.solana_lease(self.setup.recipient(), now)?;
            self.funder_lease = funder;
            self.beneficiary_lease = beneficiary;
        }
        Ok(())
    }
    ...
    pub(crate) fn new_materializing(
        ...
        if scope.settlement_id != setup.settlement_id()
            ...
            || scope.deployment_digest != resolved_solana_deployment_digest_v1(&deployment)?
    ...
    fn instructions_for_action(
        ...
        crate::production_solana_signer::instructions_v7(
            &self.setup,
            self.token_accounts_v7,
            action,
            claim_secret_be,
        )
    }

    /// Bind all SPL account choices before any new message is signed.
    pub(crate) fn with_token_accounts_v7(
        mut self,
        accounts: Option<crate::production_solana_signer::ProductionSolanaTokenAccountsV7>,
    ) -> Result<Self, ChildAuthorityRefusalV1> {
        crate::production_solana_signer::validate_token_accounts_v7(&self.setup, accounts)?;
        self.token_accounts_v7 = accounts;
        Ok(self)
    }
    ...
    fn materialize_solana_child(
        ...
        let result = self.materialize_with_secret(action, locator, claim_secret_arg, authority);
        claim_secret.zeroize();
        result
    }
    ...
    for ProductionSolanaChildPortV1<R, C>
{
    ...
    > {
        let view = match self
            .actuator
            .view(self.locator(SettlementActionV1::Funding))
        {
            Ok(view) => view,
            Err(SolanaActuatorErrorV1::NotFound) => return Ok(None),
            Err(error) => return Err(map_actuator_error(error)),
        };
        if view.secret_exposed {
            return Err(ChildAuthorityRefusalV1::Conflict);
        }
        self.validate_retained_transaction(SettlementActionV1::Funding, &view)?;
        Ok(Some(
            crate::production_child_router::ProductionRetainedFundingIdV20::Solana(view.signature),
        ))
    }
