...
const DOM_XMR_NATIVE_FACE_DOMAIN_V23: &[u8] = b"DOM-INTEROP/REFUND-ARMING/DOM-XMR-NATIVE/V23\0";
#[path = "production_dom_xmr_refund_face_v23.rs"]
mod dom_xmr_refund_face_v23;
...
impl ProductionEvmRefundFaceV1 {
    /// Share the already-open finalized adapter with public-secret recovery.
    /// No RPC credentials or additional signing authority are opened here.
    pub(crate) fn public_secret_source_v4(
        &self,
        inputs: &crate::production_inputs::AuthenticatedProductionInputsV1,
        leg: LegIdV1,
    ) -> Result<
        crate::production_plan_source::ProductionEvmPublicSecretSourceV1<HttpJsonRpc>,
        crate::supervisor::AuthorityRefusalV1,
    > {
        ...
        let config = self.deployment.adapter_config();
        if expected.registry_digest() != self.deployment.registry_digest()
            || expected.profile_digest() != self.deployment.profile_digest()
            || expected.adapter_config() != config
            || self.adapter.config() != &config
            || config.session_id != terms.session_id.0
            || config.dom_chain_id != terms.dom_leg.chain_id.0
        ...
        let lock_terms = LockTerms {
            ...
            adaptor_address: adaptor_address(&terms.adaptor_point_sec1)
                .map_err(|_| AuthorityRefusalV1::Inconsistent)?,
            deadline,
        };
        ...
        crate::production_plan_source::ProductionEvmPublicSecretSourceV1::from_retained_adapter_v4(
...
impl ProductionRefundFaceVerifierV1 for BoundDomRefundFaceV1 {
    ...
    fn verify(&self) -> Result<FaceEvidenceV1, AuthorityRefusalV1> {
        self.inner
            ...
            .map_err(|_| AuthorityRefusalV1::Inconsistent)?;
        let binding = self.inner.authority.binding();
        if self.inner.authority.native_xmr_refund_selected_v23() {
            return self.verify_native_xmr_refund_v23();
        }
        let artifact = self
            .inner
            .authority
...
fn bind_dom_face(
    ...
    let static_digest = digest_parts(
        if face.authority.native_xmr_refund_selected_v23() {
...
fn decode_receipt_full(bytes: &[u8]) -> Result<DecodedReceiptV1, AuthorityRefusalV1> {
    ...
    if cursor != bytes.len()
        ...
        || !matches!(upstream_counterparty.kind, 2..=5)
        || !matches!(downstream_counterparty.kind, 2..=5)
