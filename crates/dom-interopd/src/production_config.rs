...
#[path = "production_config_universal.rs"]
mod universal;
pub use universal::{
    load_production_bootstrap_v11, ProductionChainFamilyV11, ProductionUniversalBootstrapFieldsV11,
    ProductionUniversalLegV11, PRODUCTION_CREATE_CONFIG_FILE_V11,
    PRODUCTION_REOPEN_CONFIG_FILE_V11,
};
#[cfg(feature = "production")]
pub(crate) use universal::{
    load_production_create_or_resume_bootstrap_v11, provisioning_binding_for_v11_bootstrap,
    universal_actuator_stage_v11,
};
...
impl ProductionBootstrapFamilyV1 {
    ...
    const fn path_role_count(self) -> usize {
        match self {
            ...
            Self::V11 => 0, // V11 is an exact envelope, decoded independently.
        }
    }
...
enum ProductionFamilyExtrasV1 {
    /// Common DOM/Relay graph plus two independently selected external owners.
    V11(Box<universal::ProductionUniversalBootstrapV11>),
...
impl ProductionBootstrapConfigV1 {
    ...
    pub fn contracts_transport_identity_store(&self) -> Option<&Path> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.contracts_transport_identity_store(),
    ...
    pub fn contracts_budget_policy(&self) -> Option<&Path> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.contracts_budget_policy(),
    ...
    pub const fn f6_paths_v4(&self) -> Option<&ProductionF6PathReferencesV4> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.f6_paths_v4(),
    ...
    pub fn contracts_bootstrap(&self) -> Option<&Path> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.contracts_bootstrap(),
    ...
    pub const fn contracts_bootstrap_pins_v5(&self) -> Option<ProductionContractsBootstrapPinsV5> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.contracts_bootstrap_pins_v5(),
            ProductionFamilyExtrasV1::V5 {
                contracts_bootstrap_pins,
                ..
            }
    ...
    pub const fn relay_authority_pins_v6(&self) -> Option<ProductionRelayAuthorityPinsV6> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => v.common.relay_authority_pins_v6(),
            ProductionFamilyExtrasV1::V6 {
                relay_authority_pins,
                ..
            }
    ...
    pub fn bitcoin_prebroadcast_store_v7(&self) -> Option<&Path> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(_) => None,
            ProductionFamilyExtrasV1::V7 {
                bitcoin_prebroadcast_store,
                ..
            }
    ...
    pub const fn f6_paths_v8(&self) -> Option<&ProductionF6PathReferencesV8> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => Some(&v.f6_paths),
    ...
    pub const fn f6_authority_bundle_digest_v8(&self) -> Option<[u8; 32]> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => Some(v.fields.f6_authority_bundle_digest),
            ProductionFamilyExtrasV1::V8 {
                f6_authority_bundle_digest,
                ..
            }
    ...
    pub const fn refund_arming_authority_epoch_v9(&self) -> Option<u64> {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(v) => Some(v.fields.refund_arming_authority_epoch),
            ProductionFamilyExtrasV1::V9 {
                refund_arming_authority_epoch,
                ..
            }
    ...
    const fn family(&self) -> ProductionBootstrapFamilyV1 {
        match &self.extras {
            ProductionFamilyExtrasV1::V11(_) => ProductionBootstrapFamilyV1::V11,
    ...
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, ProductionConfigErrorV1> {
        if let ProductionFamilyExtrasV1::V11(v) = &self.extras {
            return v.encode(self.mode);
        }
    ...
    fn canonical_body(&self) -> String {
        ...
        match self.family() {
            ProductionBootstrapFamilyV1::V11 => {
                if let ProductionFamilyExtrasV1::V11(value) = &self.extras {
                    return value.canonical_body(self.mode);
                }
            }
        ...
        match &self.extras {
            ProductionFamilyExtrasV1::V11(value) => return value.canonical_body(self.mode),
            ProductionFamilyExtrasV1::None => {}
            ProductionFamilyExtrasV1::V2 { identity_store } => {
    ...
    fn equivalent_except_mode(&self, other: &Self) -> bool {
        self.pins == other.pins
            ...
            && match (&self.extras, &other.extras) {
                (ProductionFamilyExtrasV1::V11(a), ProductionFamilyExtrasV1::V11(b)) => {
                    a.fields == b.fields && a.common.equivalent_except_mode(&b.common)
                }
                (a, b) => a == b,
            }
    }
...
fn decode_config(
    ...
    if family == ProductionBootstrapFamilyV1::V11 {
        return universal::decode(bytes, expected_mode);
    }
...
fn resolve_and_validate_layout(
    ...
    for role in ProductionPathRoleV1::ALL {
        if config.universal_v11().is_some()
            && matches!(
                role,
    ...
    let solana_actuator_database = derive_optional_chain_actuator_database(
        ...
        SOLANA_ACTUATOR_DATABASE_FILE_V1,
        creating,
        f6_paths_v8.is_some() && config.universal_v11().is_none(),
    )?;
...
mod tests {
    ...
    ) -> ProductionUniversalBootstrapFieldsV11 {
        let leg = |family, id, suffix: &str| ProductionUniversalLegV11 {
            family,
            settlement_id: [id; 32],
            session_id: [id + 2; 32],
            chain_id: [id + 4; 32],
            actuator_store: format!("state/{suffix}-actuator.sqlite3"),
            authority_bundle: format!("inputs/{suffix}-authority.v11"),
            authority_bundle_digest: ProductionUniversalLegV11::bundle_digest(suffix.as_bytes())
                .unwrap(),
        };
        ProductionUniversalBootstrapFieldsV11 {
            f6_paths: standard_f6_paths_v8(),
    ...
    #[test]
    fn v11_all_sixteen_bootstraps_have_only_position_selected_external_resources() {
        use ProductionChainFamilyV11::*;
        for a in [Btc, Evm, Sol, Xmr] {
            for b in [Btc, Evm, Sol, Xmr] {
                let fields = universal_fields_v11(a, b);
                let config = ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(
                    golden_create_config_v6(),
                    fields.clone(),
                )
                .unwrap();
                assert!(config.bitcoin_prebroadcast_pins_v7().is_none());
                assert!(config.bitcoin_prebroadcast_store_v7().is_none());
                assert!(config.operational_policies_v10().is_none());
                let encoded = config.canonical_bytes().unwrap();
                let decoded = ProductionBootstrapConfigV1::decode_canonical_v11_for_mode(
                    &encoded,
                    ProductionBootstrapModeV1::Create,
                )
                .unwrap();
                assert!(decoded.universal_v11() == Some(&fields));
                assert_eq!(decoded.canonical_bytes().unwrap(), encoded);
                assert!(ProductionBootstrapConfigV1::decode_canonical_v10_for_mode(
                    &encoded,
                    ProductionBootstrapModeV1::Create
                ...
                .is_err());
                assert!(ProductionBootstrapConfigV1::decode_canonical_v11_for_mode(
                    &encoded,
                    ProductionBootstrapModeV1::ReopenExisting
                )
                .is_err());
                let mut reopen_common = golden_create_config_v6();
                reopen_common.mode = ProductionBootstrapModeV1::ReopenExisting;
                let reopen =
                    ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(reopen_common, fields)
                        .unwrap();
                assert!(config.equivalent_except_mode(&reopen));
            }
        }
    }

    #[test]
    fn v11_refuses_resource_aliases_and_legacy_bitcoin_authority_inheritance() {
        use ProductionChainFamilyV11::*;
        assert!(ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(
            golden_create_config_v7(),
            universal_fields_v11(Sol, Xmr)
        )
        .is_err());
        for mutation in 0..6 {
            let mut fields = universal_fields_v11(Sol, Xmr);
            match mutation {
                0 => fields.legs[1].actuator_store = fields.legs[0].actuator_store.clone(),
                1 => fields.legs[1].authority_bundle = fields.legs[0].authority_bundle.clone(),
                2 => fields.legs[0].actuator_store = "state/dom-actuator.sqlite3".into(),
                3 => fields.legs[1].session_id = fields.legs[0].session_id,
                4 => fields.legs[0].authority_bundle = "../escape".into(),
                _ => fields.remote_relay_database_ids[1] = fields.remote_relay_database_ids[0],
            }
            assert!(ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(
                golden_create_config_v6(),
                fields
    ...
    #[test]
    fn v11_envelope_rejects_corruption_trailing_bytes_and_unknown_json_fields() {
        let config = ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(
            golden_create_config_v6(),
            universal_fields_v11(ProductionChainFamilyV11::Sol, ProductionChainFamilyV11::Xmr),
        )
        .unwrap();
        let bytes = config.canonical_bytes().unwrap();
        for index in [0, 32, bytes.len() / 2, bytes.len() - 2] {
        ...
        .is_err());
        let mut json = serde_json::to_value(universal_fields_v11(
            ProductionChainFamilyV11::Sol,
            ProductionChainFamilyV11::Xmr,
        ))
        .unwrap();
        json["mandatory_bitcoin"] = serde_json::Value::Bool(true);
        assert!(serde_json::from_value::<ProductionUniversalBootstrapFieldsV11>(json).is_err());
    }

    #[test]
    fn v11_sol_xmr_layout_never_requires_or_creates_evm_or_bitcoin_state() {
        let fixture = Fixture::new();
        fixture.create_v5_inputs();
        write_owner_file(
            &fixture.root.join(F6_AUTHORITY_BUNDLE_PATH_V8),
        ...
        for (mode, name) in [
            ...
            let common = fixture.config_v6(mode);
            let config =
                ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(common, fields.clone())
                    .unwrap();
            write_owner_file(&fixture.root.join(name), &config.canonical_bytes().unwrap());
        }
        for (leg, bytes) in fields
            .legs
            .iter()
            .zip([b"upstream".as_slice(), b"downstream".as_slice()])
        {
            write_owner_file(&fixture.root.join(&leg.authority_bundle), bytes);
        }
        let created =
            load_production_bootstrap_v11(&fixture.root, ProductionBootstrapModeV1::Create)
                .unwrap();
        assert!(created.layout().bitcoin_prebroadcast_store_v7().is_none());
        assert!(created.layout().solana_actuator_database().is_none());
        assert!(created.layout().xmr_actuator_database().is_none());
        fixture.create_managed_state();
        for role in [
            ProductionPathRoleV1::EvmActuatorStore,
            ProductionPathRoleV1::BitcoinActuatorStore,
            ...
            std::fs::remove_file(fixture.root.join(standard_paths()[role.index()].as_str()))
                .unwrap();
        }
        fixture.create_retained_f6_state_v4_for_v8();
        fixture.create_f6_state_v8();
        ...
        for leg in &fields.legs {
            write_owner_file(&fixture.root.join(&leg.actuator_store), b"actuator");
        }
        load_production_bootstrap_v11(&fixture.root, ProductionBootstrapModeV1::ReopenExisting)
            .unwrap();
        for role in [
            ProductionPathRoleV1::EvmActuatorStore,
            ProductionPathRoleV1::BitcoinActuatorStore,
            ...
            assert!(!fixture
                .root
                .join(standard_paths()[role.index()].as_str())
                .exists());
        }
        std::fs::remove_file(fixture.root.join(&fields.legs[1].actuator_store)).unwrap();
        assert!(load_production_bootstrap_v11(
            &fixture.root,
            ProductionBootstrapModeV1::ReopenExisting
        )
        .is_err());
        assert!(!fixture.root.join(&fields.legs[1].actuator_store).exists());
    }

    #[cfg(feature = "production")]
    #[test]
    fn v11_position_stages_resume_exactly_and_refuse_unstarted_or_lost_stores() {
        let fixture = Fixture::new();
        fixture.create_v5_inputs();
        write_owner_file(
            &fixture.root.join(F6_AUTHORITY_BUNDLE_PATH_V8),
        ...
        let fields =
            universal_fields_v11(ProductionChainFamilyV11::Xmr, ProductionChainFamilyV11::Sol);
        for (mode, name) in [
            (
                ProductionBootstrapModeV1::Create,
                PRODUCTION_CREATE_CONFIG_FILE_V11,
            ...
            let config = ProductionBootstrapConfigV1::from_common_v6_and_legs_v11(
                fixture.config_v6(mode),
                fields.clone(),
            )
            .unwrap();
            write_owner_file(&fixture.root.join(name), &config.canonical_bytes().unwrap());
        }
        for (resources, bytes) in fields
            .legs
            .iter()
            .zip([b"upstream".as_slice(), b"downstream".as_slice()])
        {
            write_owner_file(&fixture.root.join(&resources.authority_bundle), bytes);
        }
        let loaded = load_production_create_or_resume_bootstrap_v11(&fixture.root).unwrap();
        let binding = provisioning_binding_for_v11_bootstrap(&loaded).unwrap();
        assert_eq!(
            loaded.config().remote_relay_database_ids(),
            Some(fields.remote_relay_database_ids)
        );
        let mut journal =
            DurableProductionProvisioningJournalV1::create(&fixture.root, binding).unwrap();
        for stage in [
            ...
            journal.begin(stage).unwrap();
            if stage == ProductionProvisioningStageV1::RouteSecretVault {
                create_owner_dir(&fixture.root.join(ROUTE_SECRET_VAULT_ROOT_NAME_V1));
            }
            for role in ProductionPathRoleV1::ALL {
                if provisioning_stage_for_role(role) == Some(stage) {
                    let path = loaded.layout().path(role);
                    match role.kind() {
                        ProductionPathKindV1::ManagedFile => {
                            write_owner_file(path, b"retained-stage")
                        }
                        ProductionPathKindV1::ManagedDirectory => create_owner_dir(path),
                        _ => panic!("managed stage fixture"),
                    }
                }
            }
            journal.complete(stage).unwrap();
        }
        drop(journal);
        let upstream = loaded
            .universal_actuator_path_v11(LegIdV1::Upstream)
            .unwrap();
        let downstream = loaded
            .universal_actuator_path_v11(LegIdV1::Downstream)
            .unwrap();
        // Correct bytes cannot excuse a store whose own journal slot never began.
        write_owner_file(&upstream, b"retained-xmr");
        assert!(load_production_create_or_resume_bootstrap_v11(&fixture.root).is_err());
        fs::remove_file(&upstream).unwrap();
        let mut journal =
            DurableProductionProvisioningJournalV1::open(&fixture.root, binding).unwrap();
        let upstream_stage = universal_actuator_stage_v11(LegIdV1::Upstream);
        assert_eq!(
            upstream_stage,
            ProductionProvisioningStageV1::EvmActuatorStore
        );
        journal.begin(upstream_stage).unwrap();
        drop(journal);
        load_production_create_or_resume_bootstrap_v11(&fixture.root).unwrap();
        write_owner_file(&upstream, b"retained-xmr");
        load_production_create_or_resume_bootstrap_v11(&fixture.root).unwrap();
        // A started upstream slot does not authorize a downstream SOL database.
        write_owner_file(&downstream, b"unexpected-sol");
        assert!(load_production_create_or_resume_bootstrap_v11(&fixture.root).is_err());
        fs::remove_file(&downstream).unwrap();
        let mut journal =
            DurableProductionProvisioningJournalV1::open(&fixture.root, binding).unwrap();
        journal.complete(upstream_stage).unwrap();
        drop(journal);
        fs::remove_file(&upstream).unwrap();
        assert!(load_production_create_or_resume_bootstrap_v11(&fixture.root).is_err());
        ...
        for role in [
            ...
            assert!(!loaded.layout().path(role).exists());
        }
    }

    #[cfg(feature = "production")]
    #[test]
    fn v11_journal_binding_refuses_rewritten_companions_and_family_transplants() {
        let fixture = Fixture::new();
        fixture.create_v5_inputs();
        write_owner_file(
            &fixture.root.join(F6_AUTHORITY_BUNDLE_PATH_V8),
        ...
        let install = |fields: &ProductionUniversalBootstrapFieldsV11| {
            for (mode, name) in [
                (
                    ProductionBootstrapModeV1::Create,
                    PRODUCTION_CREATE_CONFIG_FILE_V11,
                ...
                .unwrap();
                write_owner_file(&path, &config.canonical_bytes().unwrap());
            }
        };
        install(&fields);
        for (resources, bytes) in fields
        ...
        let loaded = load_production_create_or_resume_bootstrap_v11(&fixture.root).unwrap();
        let binding = provisioning_binding_for_v11_bootstrap(&loaded).unwrap();
        drop(DurableProductionProvisioningJournalV1::create(&fixture.root, binding).unwrap());
        fields.legs[0].family = ProductionChainFamilyV11::Sol;
        install(&fields);
        assert_eq!(
            provisioning_binding_for_v11_bootstrap(&loaded),
            Err(ProductionConfigErrorV1::IntegrityMismatch)
        );
        assert_eq!(
            load_production_create_or_resume_bootstrap_v11(&fixture.root).unwrap_err(),
            ProductionConfigErrorV1::ProvisioningJournalRefused
        );
    }
}
