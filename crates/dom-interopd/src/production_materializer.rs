...
/// Purpose-specific late M.8 pump over the very same temporarily-borrowed
/// router used by materialization and dispatch. No child or Store is reopened.
pub(crate) struct ProductionBitcoinPostAnchorPumpV11 {
    router: SharedProductionSettlementRouterV1,
}

/// Root heartbeat sharing exactly the router used for materialization and
/// dispatch. No Store, process lock or participant authority is reopened.
pub(crate) struct ProductionActuatorHeartbeatV12 {
    router: SharedProductionSettlementRouterV1,
}

/// Read-only identity handoff using the same physical child router. Restart
/// queries its durable native custody; no volatile cache or second DB owner.
pub(crate) struct ProductionFundingIdentityReaderV20 {
    router: SharedProductionSettlementRouterV1,
}
impl ProductionFundingIdentityReaderV20 {
    pub(crate) fn read(
        &mut self,
        ...
        Option<crate::production_child_router::ProductionRetainedFundingIdV20>,
        ChildAuthorityRefusalV1,
    > {
        self.router.with_router(|router| {
            router.retained_funding_id_v20(face, leg, route_id, settlement_id)
        })
    }
}
...
impl ProductionBitcoinPostAnchorPumpV11 {
    pub(crate) fn drive(
        &mut self,
        leg: SettlementLegV1,
        call: crate::production_bitcoin_claim_driver::BitcoinPostAnchorCallV11,
        resources: crate::production_bitcoin_claim_driver::BitcoinPostAnchorExternalResourcesV11<
            '_,
            '_,
        >,
    ) -> Result<
        Option<crate::production_contracts::ProductionContractsConsumedPostAnchorV2>,
        crate::production_bitcoin_claim_driver::BitcoinPostAnchorErrorV8,
    > {
        self.router
            .with_router(|router| Ok(router.drive_bitcoin_post_anchor_v11(leg, call, resources)))
            .map_err(|_| crate::production_bitcoin_claim_driver::BitcoinPostAnchorErrorV8::Scope)?
    }
}
...
impl ProductionSettlementMaterializationOwnerV1 {
    ...
    pub(crate) fn funding_identity_reader_v20(&self) -> ProductionFundingIdentityReaderV20 {
        ProductionFundingIdentityReaderV20 {
            router: SharedProductionSettlementRouterV1 {
                slot: Rc::clone(&self.runtime.router.slot),
            },
        }
    }

    pub(crate) fn bitcoin_post_anchor_pump_v11(&self) -> ProductionBitcoinPostAnchorPumpV11 {
        ProductionBitcoinPostAnchorPumpV11 {
            router: SharedProductionSettlementRouterV1 {
                slot: Rc::clone(&self.runtime.router.slot),
            },
        }
    ...
    pub(crate) fn actuator_heartbeat_v12(&self) -> ProductionActuatorHeartbeatV12 {
        ProductionActuatorHeartbeatV12 {
            router: SharedProductionSettlementRouterV1 {
                slot: Rc::clone(&self.runtime.router.slot),
            },
        }
...
impl ProductionSettlementDraftMaterializerV1 for ProductionSettlementDraftMaterializerV2 {
    ...
    fn materialize_with_verified_public_secret(
        ...
        let mut counterparty = self.child_request(
            request,
            leg,
            semantic,
            effect,
        ...
        let mut dom = self.child_request(
            request,
            leg,
            semantic,
            effect,
    ...
    fn materialize_deferred_with_verified_public_secret(
        ...
        let request = ProductionChildMaterializationRequestV1 {
            ...
            public_secret_evidence_digest: capability.exposure().evidence_digest,
...
impl ProductionSettlementDraftMaterializerV2 {
    ...
    fn child_request(
        ...
        ProductionChildMaterializationRequestV1 {
            ...
            public_secret_evidence_digest: ZERO_DIGEST,
            exposure,
        }
    }
    ...
    fn materialize_first_exposure(
        ...
        let first = self
            .router
            .with_router(|router| {
                if leg.counterparty_face == SettlementFaceV1::Bitcoin {
                    // A real request fixes the future Bitcoin effect before
                    // the first DOM secret may leave its custody boundary.
                    router.prepare_bitcoin_claim_v11(authority_request.counterparty)?;
                }
...
/// DOM chain (downstream `LocalOrigin`, or upstream
/// `VerifiedDownstreamDomClaimV23` after the same route’s verified reveal).
/// EVM, Bitcoin and Solana counterparty claims all
...
mod tests {
    ...
    fn first_exposure_authority_rejects_each_common_scope_transplant() {
        ...
        // The DOM first-exposure condition is independent of which external
        // child is selected. Actual XMR admission still requires its recovery
        // authority; this scope checker cannot grant it.
        for face in [
            SettlementFaceV1::Evm,
            SettlementFaceV1::Bitcoin,
            SettlementFaceV1::Solana,
            ...
            selected.counterparty_face = face;
            assert_eq!(validate_first_exposure_scope(expected, &selected), Ok(()));
            selected.first_face = face;
            assert_eq!(
                validate_first_exposure_scope(expected, &selected),
                Err(ChildAuthorityRefusalV1::Conflict)
            );
        }
        let mut invalid = baseline;
        invalid.counterparty_face = SettlementFaceV1::Dom;
        assert_eq!(
            validate_first_exposure_scope(expected, &invalid),
            Err(ChildAuthorityRefusalV1::Conflict)
        );
