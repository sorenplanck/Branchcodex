...
impl ProductionBitcoinPrebroadcastChildHandoffV7 {
    /// Freezes the receipt's actual economic claim before the runtime has an
    /// action effect. The same owner's native bilateral readiness is required;
    /// effect binding and M.8 remain later materializer operations.
    pub(crate) fn prepare_claim_v11(
        &self,
        ...
        readiness: crate::production_contracts::ProductionBitcoinPrefundingReadinessV11,
    ) -> Result<
        crate::production_child_btc::ProductionBitcoinPreparedClaimV11,
        ProductionBitcoinPrebroadcastErrorV7,
    > {
        if self.recovered_claim.is_some() {
            return Err(ProductionBitcoinPrebroadcastErrorV7::ClaimRecoveryNotComposable);
        }
        crate::production_child_btc::ProductionBitcoinPreparedClaimV11::authenticate_receipt_v11(
            inputs,
            role_plan,
            upstream_scope,
            downstream_scope,
...
impl ProductionBitcoinPrebroadcastOwnerV7 {
    ...
    pub(crate) fn open_existing(
        ...
        Self::open_authenticated_leg(inputs, path, pins, rpc)
    }

    /// Opens one selected V11 Bitcoin position from its authenticated family
    /// bundle. The other leg may be Bitcoin as well and has its own owner,
    /// route binding, receipt and participant session.
    pub(crate) fn open_existing_for_leg_v11(
        bootstrap: &ValidatedProductionBootstrapV1,
        inputs: &AuthenticatedProductionInputsV1,
        leg: LegIdV1,
        path: &std::path::Path,
        ...
        let selected = &fields.legs[index];
        let relative = path
            .strip_prefix(bootstrap.layout().state_dir())
            .map_err(|_| ProductionBitcoinPrebroadcastErrorV7::InvalidConfiguration)?;
        if pins.leg != leg
            || selected.family != crate::production_config::ProductionChainFamilyV11::Btc
            || selected.settlement_id != pins.settlement_id
            || selected.session_id != pins.session_id
        ...
        validate_authenticated_leg_scope_v11(inputs, bootstrap.config().pins(), pins)?;
        Self::open_authenticated_leg(inputs, path, pins, rpc)
    }

    fn open_authenticated_leg(
        inputs: &AuthenticatedProductionInputsV1,
        path: &std::path::Path,
        pins: ProductionBitcoinPrebroadcastPinsV7,
        rpc: Rc<BitcoinCoreRpcClientV1>,
...
fn validate_authenticated_scope(
    ...
    validate_authenticated_leg_scope_v11(inputs, route_pins, pins)
}

fn validate_authenticated_leg_scope_v11(
    inputs: &AuthenticatedProductionInputsV1,
    route_pins: ProductionRoutePinsV1,
    pins: ProductionBitcoinPrebroadcastPinsV7,
) -> Result<(), ProductionBitcoinPrebroadcastErrorV7> {
