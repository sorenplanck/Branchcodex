    ...
    Y: RouteSecretRetirementAuthority,
{
    ...
    /// V22 recovery enters the route only through this runtime's fenced writer.
    #[cfg(feature = "production")]
    pub(crate) fn record_xmr_compensation_v22(
        &mut self,
        report: crate::production_xmr_recovery_pump_v22::XmrCompensationReportV22,
    ) -> Result<(), settlement_coordinator::ChildAuthorityRefusalV1> {
        report
            .driver
            .record_compensation_with_funding(
                &mut self.supervisor,
                report.leg,
                report.funding,
                &report.observed,
    ...
    #[cfg_attr(
        ...
        expect(dead_code, reason = "Used by the production XMR recovery pump")
    )]
