...
impl<R: EvmRpcV1, C: ProductionEvmChildClockV1> ProductionEvmChildPortV1<R, C> {
    ...
    pub(crate) fn new_materializing(
        ...
        if observation_valid_for_ms == 0
            || remote_custody_lease_duration_ms == 0
            || lease_renewal_ms_v12.is_some_and(|duration| duration == 0 || duration > 3_600_000)
    ...
    for ProductionEvmChildPortV1<R, C>
{
    ...
    > {
        ...
        self.actuator
            .retained_open_transaction_v20(
                authority.route_id,
                self.deployment,
                &authority.opening_call,
            )
            .map(|id| id.map(crate::production_child_router::ProductionRetainedFundingIdV20::Evm))
            .map_err(map_actuator_error)
    }
    ...
    fn renew_actuator_lease_v12(&mut self) -> Result<(), ChildAuthorityRefusalV1> {
        ...
        let now = self.clock.now_unix_ms()?;
        for slot in [&mut self.funder_lease, &mut self.beneficiary_lease] {
            let Some(lease) = *slot else {
                continue;
            };
            let remaining = lease
                .lease_until_unix_ms()
...
fn renew_evm_lease_v12(
    actuator: &mut DurableEvmActuatorV1,
    lease: EvmActuatorLeaseV1,
    now: u64,
    duration: u64,
) -> Result<EvmActuatorLeaseV1, ChildAuthorityRefusalV1> {
