...
//!
//! The production entrypoint reads its credential stream once and dispatches
//! explicitly to the byte-frozen V3/V10 recovery graph or the V4/V11 universal
//! graph. The latter initializes only its two selected external positions.
//! Both paths retain native DOM/Contracts/Relay, signer and custody owners.
//!
//! Per-family startup is not a substitute for a claim/funding authorization.
//! Every child still requires its native readiness and post-anchor authority;
//! [`PRODUCTION_KNOWN_LIMITS_V1`] identifies integration work not yet closed.

#[path = "production_run_universal.rs"]
...
use crate::production_chain_services::ProductionChainClientsV1;
...
use crate::production_child_evm::{
    ProductionEvmMaterializationScopeV1, ProductionEvmMaterializingPortInputV1,
...
use crate::production_materializer::{
    ProductionCustodiedFirstExposureClaimAuthorityV1, ProductionSettlementMaterializationOwnerV1,
};
use crate::production_materializing_children::{
    compose_materializing_route_children_v7, ProductionBitcoinChildInputV7,
    ProductionCounterpartyChildInputV7,
};
...
pub use f6_stage_failure_v25::{DomActuatorCauseV25, ProductionF6StageFailureV25};
...
pub enum ProductionRunErrorV1 {
    ...
    Secrets,
    /// Configuration fallback when this boundary has no typed check to report.
    #[error("production configuration refused")]
    Configuration,
    /// The canonical configuration was refused, and this says by which check.
    ///
    /// Naming the class widens nothing: `ProductionConfigErrorV1` is redacted
    /// by construction — no variant holds a path, endpoint, input byte or
    /// nested error — so the operator learns "a managed leaf was already
    /// present" rather than which leaf, or "an input artifact was
    /// unavailable" rather than which file.
    ///
    /// Without it, assembling a state directory of a hundred-odd fields tells
    /// the operator only that it was refused, which turns provisioning into
    /// blind trial and error. Refusing precisely is only useful if the
    /// refusal can be acted on.
    ///
    /// The secret stream deliberately does NOT do this — see `Secrets` above.
    /// Its fields are the secrets themselves, so naming the failing field
    /// would describe material this boundary must not describe.
    #[error("production configuration refused: {0:?}: {0}")]
    ConfigurationDetail(ProductionConfigErrorV1),
    ...
    ChainServices,
    /// A valid route has no complete signer/refund/post-anchor resource graph
    /// in this version of the process bootstrap. The legacy graph still needs
    /// exactly one EVM and one Bitcoin leg.
    #[error("production route admitted but execution authority graph is incomplete")]
    ExecutionAuthoritiesUnavailable,
    /// The compensated XMR graph has no native bilateral ready/funding
    /// authority in this root yet. Plain legacy DOM refunds cannot authorize it.
    #[error("production XMR compensated funding authority unavailable")]
    XmrCompensatedFundingAuthorityUnavailable,
...
pub const PRODUCTION_KNOWN_LIMITS_V1: &[&str] = &[
    "RouteServicesV8: the schema and topology validator represent all 16 BTC/EVM/SOL/XMR pairs with DOM as the common center and independent services per position, including same-family routes on one network with distinct settlements. Topology acceptance is not proof of complete admission/funding/claim/recovery for every pair. V2 settings migrate only their old EVM+BTC resource layout; use the V8 document for other pairs",
    "ExecutionGraph: V4 credentials select the V11 independent per-position bootstrap; V3 credentials retain their V10 EVM+BTC recovery layout. Family-specific startup is connected, but complete execution remains subject to native F7/readiness/claim capabilities",
    "BitcoinClaim: V11 loads same-Store bilateral readiness and calls the native late M.8 pump between route steps, using per-position participant sockets. The resulting Contracts authority still needs the DOM claim-signing producer; full swaps remain unverified",
    "SolanaExecution: V11 composes per-position signers, durable custody, escrow refund verification and native children; end-to-end completion still requires the general F7/Contracts consumer",
    "MoneroExecution: native sweep and selected startup exist; the compensated recovery graph must be authenticated and retained before funding. Upstream revelation requires a DOM source; a Monero spend cannot reveal the scalar as VerifiedCounterpartyClaim",
    "F7AnchorFamily: BTC uses its native M.8 validator. EVM/SOL/XMR have family-specific V12 observers, and V19 schedules native bilateral 0x17 readiness in the composite loop. Initial gate construction, funding signing and the complete claim producer are not yet connected across all families",
];
...
pub fn run_production_v1(options: &ProductionRunOptionsV1) -> Result<(), ProductionRunErrorV1> {
    ...
    let secrets = crate::production_node::read_production_run_secrets_v11_from_stdin()
        .map_err(|_| ProductionRunErrorV1::Secrets)?;
    match secrets {
        crate::production_node::ProductionRunSecretsV11::LegacyV3(secrets) => {
            run_legacy_production_v3(options, secrets)
        }
        crate::production_node::ProductionRunSecretsV11::UniversalV4(secrets) => {
            universal::run(options, secrets)
        }
    }
}

fn run_legacy_production_v3(
    options: &ProductionRunOptionsV1,
    secrets: crate::production_node::ProductionSecretsV3,
    ...
    let xmr_graph_vault_key_v23 = secrets.xmr_graph_vault_key_v23;
    ...
    let bootstrap = load_bootstrap(options).map_err(ProductionRunErrorV1::ConfigurationDetail)?;
    ...
    // Admission and endpoint ownership are now independent of the family pair.
    // The remaining V10 signer/Contracts driver has only this resource graph;
    // missing execution authorities are never fabricated from endpoint settings.
    let (evm_deployment, _) = selected_services
        .legacy_deployments()
        .map_err(|_| ProductionRunErrorV1::ExecutionAuthoritiesUnavailable)?;
    ...
    let mut chain_clients = selected_services
        .into_legacy_clients()
        .map_err(|_| ProductionRunErrorV1::ChainServices)?;
    ...
    let bitcoin_payout = bitcoin_prebroadcast_owner
        .take_payout_face()
        .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
    let [upstream_payout, downstream_payout] = match chain_signers
        .bitcoin_leg()
        .map_err(|_| ProductionRunErrorV1::BitcoinChildAuthority)?
    {
        LegIdV1::Upstream => [Some(bitcoin_payout), None],
        LegIdV1::Downstream => [None, Some(bitcoin_payout)],
    };
    ...
    let mut f6_pair_factory =
        ProductionF6PairAuthoritiesFactoryV7::new(ProductionF6PairFactoryRequestV7 {
            ...
            native_xmr_inventory: None,
            native_xmr_inventory_required: false,
    ...
    let evm_source_leg = match chain_signers
        .bitcoin_leg()
        .map_err(|_| ProductionRunErrorV1::BitcoinChildAuthority)?
    {
    ...
    let evm_secret_source = evm_refund
        .public_secret_source_v4(&inputs, evm_source_leg)
        .map_err(|_| ProductionRunErrorV1::PlanSource)?;
    let (upstream_counterparty_refund, downstream_counterparty_refund) = match chain_signers
        .bitcoin_leg()
        .map_err(|_| ProductionRunErrorV1::BitcoinChildAuthority)?
    {
        LegIdV1::Upstream => (
    ...
    let (role_plan, upstream_source_scope, downstream_source_scope) = f6_final_claim_plan
        .into_parts()
    ...
    let remote_evm_transport = relay_stage12_owner
        ...
        .map_err(|_| ProductionRunErrorV1::SettlementChildAuthority)?;
    let evm_child =
        ProductionCounterpartyChildInputV7::Evm(Box::new(ProductionEvmMaterializingPortInputV1 {
    ...
    // ------------------------------------------------------------------
    // Stage 14 — retained Bitcoin funding/refund authorities, new plans closed.
    //
    // F6 consumed the payout proof, but the root still lacks the bilateral-ready
    // and post-anchor network driver. V3's authenticated frames and participant
    // bridge alone do not authorize funding. `new` leaves BOTH claim and the
    // materialization scope absent: every new plan is refused. Do not replace
    // this with `new_materializing` until the full readiness/recovery graph is
    // driven through the retained Stage-8 and Stage-12 owners.
    // ------------------------------------------------------------------
    let bitcoin_leg = chain_signers
        .bitcoin_leg()
        .map_err(|_| ProductionRunErrorV1::BitcoinChildAuthority)?;
    ...
    let bitcoin_lease = bitcoin_actuator_store
        ...
        .map_err(|_| ProductionRunErrorV1::BitcoinChildAuthority)?;
    let bitcoin_child =
        ProductionCounterpartyChildInputV7::BitcoinRetainedOnly(ProductionBitcoinChildInputV7 {
            actuator: bitcoin_actuator_store,
            rpc: bitcoin_chain_client,
            lease: bitcoin_lease,
            funding: bitcoin_funding,
            lease_renewal_ms_v12: None,
        });
    ...
    // single-face route.
    // ------------------------------------------------------------------
    let (upstream_child, downstream_child) = match bitcoin_leg {
        LegIdV1::Upstream => (bitcoin_child, evm_child),
        LegIdV1::Downstream => (evm_child, bitcoin_child),
    };
    ...
    let materialization_owner = ProductionSettlementMaterializationOwnerV1::authenticate(
        ...
        match bitcoin_leg {
            LegIdV1::Upstream => Some(bitcoin_secret_installer),
            LegIdV1::Downstream => None,
        },
    )
    .map_err(|_| ProductionRunErrorV1::PlanSource)?;
...
/// Classes a DOM actuator refusal for the Stage-11 diagnostic. Reads only the
/// closed variant; never its message, a commitment, a value or a path.
fn dom_payout_failure_v25(error: &dom_actuator::DomActuatorError) -> ProductionRunErrorV1 {
    use dom_actuator::DomActuatorError as Refusal;
    let cause = match error {
        Refusal::LinuxRequired
        | Refusal::StorageUnavailable
        | Refusal::InvalidStorageAuthority
        | Refusal::DatabasePresent
        | Refusal::DatabaseMissing
        | Refusal::CreationIncomplete
        | Refusal::UnsupportedFormat
        | Refusal::ProcessLocked => DomActuatorCauseV25::Storage,
        ...
        _ => DomActuatorCauseV25::Other,
    };
    ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomPayouts(cause))
}

fn required_f6_v4_path(
    bootstrap: &ValidatedProductionBootstrapV1,
    role: ProductionF6PathRoleV4,
) -> Result<&Path, ProductionRunErrorV1> {
    bootstrap
        .layout()
        .f6_path_v4(role)
        .ok_or(ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::ActivationPaths))
}
...
fn authenticate_dom_f6_payouts(
    ...
    mut private_bootstrap: Option<
        &mut crate::production_contracts_bootstrap::producer_v13::MountedBootstrapV13,
    >,
...
> {
    ...
    let lease = store
        ...
        .map_err(|_| ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomLease))?;
    ...
    store
        ...
        .map_err(|_| ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomSession))?;
    store
        .bind_session(lease, downstream_binding, now_unix_ms)
        .map_err(|_| ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomSession))?;

    // Bootstrap the actual DOM payer's wallet before the early/BP round.
    // The native store retains the reservation and the encrypted wallet owns
    // its change opening. Later construction reopens that exact reservation;
    // there is no transient root-owned input list to lose on a crash. The
    // complementary participant needs neither inputs nor a balance here.
    // Legacy externally prepared sessions keep their original recovery path;
    // only the mounted native bootstrap owner opts into input construction.
    if let Some(private) = private_bootstrap.as_deref_mut() {
        for (index, leg, terms) in [
            (0, LegIdV1::Upstream, inputs.composition().upstream()),
            (1, LegIdV1::Downstream, inputs.composition().downstream()),
        ] {
                ...
                == kaystra_core::types::LockMechanism::CrossCurveSharedSpend
            {
                ...
                let kinds = if chain_signers
                    .dom_binding(leg)
                    .participant()
                    .participant_id()
                    == cancelled.policy.policy().dom_funder
                {
                ...
                for kind in kinds {
                    ...
                    let offer = chain_signers
                        ...
                        .prepare_xmr_payout_proof_v22(
                            store,
                            lease,
                            dom_actuator::DomXmrPayoutProofRequestV22 {
                                terms,
                                policy: &cancelled.policy,
                                kind,
                                shared: &material.capability,
                    ...
                    let bytes = offer
                        ...
                        .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                    material
                        .retain_runtime_public_v16(key, &bytes)
                        .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                }
                let cancelled_material = private._cancelled_shares[index]
                    .as_ref()
                    .ok_or(ProductionRunErrorV1::F6Authorities)?;
                let shares = chain_signers
                    .dom_authority(leg)
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?
                    .wallet()
                    .prepare_xmr_graph_shares_v22(
                        store,
                        lease,
                        dom_actuator::DomXmrGraphSharesRequestV22 {
                            terms,
                            policy: &cancelled.policy,
                            collateral: &material.capability,
                            cancelled: &cancelled_material.capability,
                            now_unix_ms,
                        },
                    )
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                material
                    .retain_runtime_public_v16(
                        b"xmr-graph-keys-v22",
                        &shares.public_commitment_v22(),
                    )
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                let retained_funding = material
                    .runtime_public_record_v16(b"xmr-funding-offer-v22")
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                let funding_offer = chain_signers
                    .dom_authority(leg)
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?
                    .wallet()
                ...
                material
                    ...
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                let retained_proofs = material
                    .runtime_public_record_v16(b"xmr-graph-proofs-v22")
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                let proofs = shares
                    .prove_public_commitment_v22(retained_proofs.as_deref())
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
                material
                    .retain_runtime_public_v16(b"xmr-graph-proofs-v22", &proofs)
                    .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            ...
            let _ = chain_signers
                .dom_authority(leg)
                .map_err(|_| ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomFunding))?
                .wallet()
                .prepare_funding_inputs_v16(store, lease, terms, now_unix_ms)
                .map_err(|_| ProductionRunErrorV1::F6AuthoritiesDetail(ProductionF6StageFailureV25::DomFunding))?;
        }
    }
    ...
    let upstream = if let Some(owner) = native_owners[0].take() {
        ...
        ProductionF6DomTermsOwnerV25::Wallet(
            ...
            .map_err(|error| dom_payout_failure_v25(&error))?,
        )
    };
    ...
    if let Some(private) = private_bootstrap {
        for (index, leg, terms, payout) in [
            (
                0,
                LegIdV1::Upstream,
            ...
            // XMR's five-edge graph uses independent C/D signing shares and
            // policy-bound payouts. Never manufacture the ordinary three-edge
            // wallet offer for this session; its amounts and refunds differ.
            if terms.counterparty_leg.mechanism
                == kaystra_core::types::LockMechanism::CrossCurveSharedSpend
            {
                continue;
            ...
            let material = &mut private._shares[index];
            let retained = material
                .runtime_public_record_v16(b"wallet-offer-v17")
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            let retained = retained
                .as_deref()
                .map(dom_adaptor::DomBootstrapOfferV17::from_bytes)
                .transpose()
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            let (offer, signing_shares) = chain_signers
                .dom_authority(leg)
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?
                .wallet()
                .prepare_bootstrap_offer_v17(
                    store,
                    lease,
                    terms,
                    &material.capability,
            ...
            let bytes = offer
                ...
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            material
                .retain_runtime_public_v16(b"wallet-offer-v17", &bytes)
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            let retained = material
            ...
            let retained = retained
                .as_deref()
                .map(dom_adaptor::DomBootstrapProvenOfferV18::from_bytes)
                .transpose()
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            let proven = signing_shares
                .prove_offer_v18(&offer, &material.capability, retained.as_ref())
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            material
                .retain_runtime_public_v16(
                ...
                .map_err(|_| ProductionRunErrorV1::F6Authorities)?;
            let (funding, claim, refund) = signing_shares.into_parts();
            material.funding_share_v18 = Some(funding);
            material.claim_share_v18 = Some(claim);
...
enum PreparedContractsStoreProvisioningV1<'a> {
    Pristine {
        ...
        parent: Arc<Dir>,
        root_name: &'a str,
        policy: Box<BudgetPolicyV1>,
        creation_binding: [u8; 32],
    },
    CompleteBound(Box<PreparedContractsSessionStoreOpenV1>),
}
    ...
    fn finish(self) -> Result<ContractsSessionStoreV1, ProductionRunErrorV1> {
        match self {
            Self::Pristine {
                ...
                policy,
                creation_binding,
            } => ContractsSessionStoreV1::resume_create_production_with_trusted_chain_v23(
                parent,
                root_name,
                *policy,
                creation_binding,
...
fn prepare_contracts_store_provisioning<'a>(
    ...
    match ContractsSessionStoreV1::preflight_resume_create_production(
        ...
        Err(_) if allow_complete_bound => {
            let prepared =
                ContractsSessionStoreV1::prepare_open_resumed_production_with_trusted_chain_v23(
                    parent,
                    root_name,
                    policy,
                    creation_binding,
...
fn open_contracts_store_pair(
    ...
    match (mode, stage) {
        ...
        (ProductionRunModeV1::Create, ProductionProvisioningStageStateV1::Complete)
        | (ProductionRunModeV1::ReopenExisting, ProductionProvisioningStageStateV1::Complete) => {
            let upstream = ContractsSessionStoreV1::prepare_open_production_with_trusted_chain_v23(
                Arc::clone(&parent),
                upstream_root,
                policy.clone(),
                chain,
            ...
            .map_err(|_| ProductionRunErrorV1::ContractsStores)?;
            let downstream =
                ContractsSessionStoreV1::prepare_open_production_with_trusted_chain_v23(
                    parent,
                    downstream_root,
                    policy,
                    chain,
