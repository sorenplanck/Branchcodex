...
pub(crate) struct ProductionTimeGuardedPlanPersistenceV2<A> {
    ...
    evidence_inbox: Option<crate::production_time_inbox::ProductionTimeEvidenceInboxV5>,
}
    ...
    A: SettlementPlanAuthorityV1,
{
    ...
    fn accept_time(&mut self, now_ms: u64) -> Result<(), AuthorityRefusalV1> {
        if now_ms == 0 || now_ms < self.last_accepted_unix_ms {
            return Err(AuthorityRefusalV1::Inconsistent);
        }
        self.last_accepted_unix_ms = now_ms;
        Ok(())
    }
    ...
    pub(crate) fn with_evidence_inbox_v5(
        mut self,
        inbox: crate::production_time_inbox::ProductionTimeEvidenceInboxV5,
    ) -> Self {
        self.evidence_inbox = Some(inbox);
        self
    }

    fn refresh_before_new_funding_v5(
        &mut self,
        now_seconds: u64,
    ) -> Result<(), AuthorityRefusalV1> {
        let candidate = match self.evidence_inbox.as_ref() {
            Some(inbox) => inbox.read_candidate()?,
            None => None,
        };
        if let Some(signed) = candidate {
            self.install_time_evidence(&signed, now_seconds)
    ...
    fn install_new_funding(
        ...
        self.refresh_before_new_funding_v5(trusted_now_seconds)?;
        let expected = plan.clone();
