#!/usr/bin/env python3
"""Install one public peer wallet offer with V18 knowledge proofs atomically; never execute a swap.
...
MAX_BYTES = 16_384 + 207
MAGIC = b'DWPO18\0\x01'
...
def install(source: Path, directory: Path, scope: tuple[bytes, bytes, bytes, bytes]) -> dict:
    ...
    if (data[:8] != MAGIC or size > 16384 or size + 207 != len(data)
        or data[12:20] != b'DWO17\0\0\x01'
        or tuple(data[20+i*32:52+i*32] for i in range(4)) != scope):
        raise ValueError('wrong offer format, chain, session, signed terms or peer participant')
    destination = directory / f'dom-wallet-offer-v18-{scope[1].hex()}.peer'
    lock_path = directory / f'dom-wallet-offer-v18-{scope[1].hex()}.import-lock'
    ...
    with os.fdopen(lock_fd, 'rb') as lock:
        ...
        if existing is not None:
            ...
            fd, temporary_name = tempfile.mkstemp(prefix='dom-wallet-offer-v18-', suffix='.pending', dir=directory)
