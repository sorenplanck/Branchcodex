#!/usr/bin/env python3
"""Test daemon and XMR RPC changes and record evidence. Never executes a live swap."""
import argparse
...
def main():
    ...
    parser.add_argument("--format", action="store_true", help="format the daemon and XMR RPC packages before Rust tests")
    args = parser.parse_args()
    if args.offline and args.format:
        parser.error("--format requires Rust mode")
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    evidence = ROOT / "artifacts" / "daemon-v8"
    evidence.mkdir(parents=True, exist_ok=True)
    out = Path(tempfile.mkdtemp(prefix=stamp + "-", dir=evidence))
    report = {"schema": 8, "status": "incomplete", "mode": "offline" if args.offline else "daemon",
    ...
    if not args.offline:
        if args.format:
            commands.append(("format-daemon", ["cargo", "fmt", "-p", "dom-interopd", "-p", "xmr-rpc-broadcast-blocking"], {}))
        commands.extend([
