#!/usr/bin/env python3
"""Validate the V11 candidate: native components, daemon build and artifact checks. Never executes a live swap."""
import argparse
...
def main():
    ...
    evidence = ROOT / "artifacts" / "daemon-v11"
    evidence.mkdir(parents=True, exist_ok=True)
    out = Path(tempfile.mkdtemp(prefix=stamp + "-", dir=evidence))
    report = {"schema": 11, "status": "incomplete", "mode": "offline" if args.offline else "daemon",
              ...
              "limits": ["Passing this runner does not prove live chain swaps or 10/10 security.",
                         ...
                         "The root calls the BTC M.8 pump; DOM claim signing and the XMR V11 funding authority remain incomplete.",
