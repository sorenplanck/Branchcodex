#!/usr/bin/env python3
"""Validate the V16 development checkpoint: native components, daemon build and artifact checks. Never executes a live swap."""
import argparse
...
def main():
    ...
    parser.add_argument("--focus-v16", action="store_true", help="Run the new native V16 regressions before building; not a live-swap campaign")
    parser.add_argument("--offline", action="store_true", help="Python verifier tests only; no Rust validation")
    args = parser.parse_args()
    if args.offline and args.focus_v16:
        parser.error("--offline and --focus-v16 are mutually exclusive")
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    evidence = ROOT / "artifacts" / "daemon-v16"
    evidence.mkdir(parents=True, exist_ok=True)
    out = Path(tempfile.mkdtemp(prefix=stamp + "-", dir=evidence))
    report = {"schema": 16, "status": "incomplete", "mode": "offline" if args.offline else "focus-v16" if args.focus_v16 else "daemon",
              ...
              "limits": ["Passing this runner does not prove live chain swaps or 10/10 security.",
                         ...
                         "V13 adds a native bootstrap ceremony and Stage-10/12 private custody; full daemon initialization is not verified.",
                         "V13 blocks new unconditional compensation signing; old signed transactions remain spendable and a safe functional replacement is not implemented.",
                         "V16 schedules early/BP bootstrap, heals native BP publication prefixes, and defers verified 0x12 until observation. Templates/refunds/funding bootstrap, F7 sender scheduling and functional XMR compensation remain incomplete.",
    ...
    if not args.offline:
        commands.extend([
            ...
            ("artifact-bootstrap-help", ["cargo", "run", "--locked", "--release", "-p", "dom-interopd", "--no-default-features", "--features", "production", "--bin", "dom-interopd", "--", "bootstrap-v13", "--help"], {}),
    ...
    if args.focus_v16:
        commands = commands[:3] + [
            ("rust-v16-wallet-bootstrap", ["cargo", "test", "--locked", "-p", "dom-actuator", "--lib", "bootstrap_v16_"], {}),
            ("rust-v16-native-custody", ["cargo", "test", "--locked", "-p", "dom-scriptless-store", "--features", "evidence-only", "--lib", "v16_"], {}),
            ("rust-v16-daemon-runtime", ["cargo", "test", "--locked", "-p", "dom-interopd", "--no-default-features", "--features", "production", "--lib", "v16_"], {}),
            ("build-v16-executable", [sys.executable, "scripts/build_daemon_v14.py", "--output", "dist/v16/dom-interopd", "--report-dir", "artifacts/daemon-v16-build"], {}),
        ]
