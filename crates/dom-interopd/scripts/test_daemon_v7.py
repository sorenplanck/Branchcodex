#!/usr/bin/env python3
"""Build/test only dom-interopd and record evidence. Never executes a live swap."""
import argparse
...
def main():
    ...
    parser.add_argument("--format", action="store_true", help="format only the dom-interopd package before Rust tests")
    args = parser.parse_args()
    if args.offline and args.format:
        parser.error("--format requires Rust mode")
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    evidence = ROOT / "artifacts" / "daemon-v7"
    evidence.mkdir(parents=True, exist_ok=True)
    out = Path(tempfile.mkdtemp(prefix=stamp + "-", dir=evidence))
    report = {"schema": 7, "status": "incomplete", "mode": "offline" if args.offline else "daemon",
              ...
              "limits": ["Passing this runner does not prove live chain swaps or 10/10 security.",
                         ...
                         "Rust signer oracle uses public unit fixtures, not a validator."]}
    ...
    if missing:
        ...
        return 2
    commands = [("python-signer-oracle", [sys.executable, "-m", "unittest", "discover", "-s", SCRIPTS + "/tests", "-v"], {})]
    if not args.offline:
        if args.format:
            commands.append(("format-daemon", ["cargo", "fmt", "-p", "dom-interopd"], {}))
        commands.extend([
