#!/usr/bin/env python3
"""Test V10 DOM/XMR execution components and daemon compatibility. Never executes a live swap."""
import argparse
...
def main():
    ...
    evidence = ROOT / "artifacts" / "daemon-v10"
    evidence.mkdir(parents=True, exist_ok=True)
    out = Path(tempfile.mkdtemp(prefix=stamp + "-", dir=evidence))
    report = {"schema": 10, "status": "incomplete", "mode": "offline" if args.offline else "daemon",
              ...
              "limits": ["Passing this runner does not prove live chain swaps or 10/10 security.",
                         ...
                         "V11 bootstrap and V4 credentials are separate components; run still uses the legacy EVM+BTC root.",
    ...
    if not args.offline:
        commands.extend([
            ...
            ("rust-dom-refund-crypto", ["cargo", "test", "--locked", "-p", "dom-scriptless-crypto", "--test", "refund_adaptor_round_v1"], {}),
