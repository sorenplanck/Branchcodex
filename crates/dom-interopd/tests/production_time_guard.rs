...
#[cfg(feature = "production")]
#[path = "../src/production_time_inbox.rs"]
mod production_time_inbox;
#[cfg(feature = "production")]
#[path = "support/production_time_inbox_v5.rs"]
mod time_inbox_v5;
...
fn adapter_from(
    ...
    ProductionTimeGuardedPlanPersistenceV2::new(guard, TestBasePlanAuthority { state }, 1).unwrap()
}
...
#[cfg(feature = "production")]
#[test]
fn persistence_rejects_zero_and_regressive_clock_without_calling_plan_authority() {
    let harness = admitted_harness_with_evidence_expiry(EVIDENCE_TIME + 15);
    let AdmittedHarness {
    ...
    let state = Rc::new(RefCell::new(BasePlanAuthorityState::default()));
    let mut persistence = adapter_from(time_store, &admission, &context, Rc::clone(&state));
    let (_directory, path) = coordinator_state_path();
    let mut coordinator = create_coordinator(&path);
    // Expired funding evidence must not disable an independently authorized exit.
    let now = (EVIDENCE_TIME + 20) * 1_000;
    let event = [0xc1; 32];
    let refund = plan_for_event(
        SettlementActionV1::Refund,
        LegIdV1::Upstream,
        1,
        event,
    ...
    persistence
        .install_new_plan(&mut coordinator, refund, event, now)
        .unwrap();
    assert_eq!(state.borrow().calls, 1);
    for refused_now in [0, now - 1] {
        let event = [0xc2; 32];
        let refund = plan_for_event(
            SettlementActionV1::Refund,
            LegIdV1::Downstream,
        ...
        assert_eq!(
            persistence
                .install_new_plan(&mut coordinator, refund, event, refused_now)
                .unwrap_err(),
            AuthorityRefusalV1::Inconsistent
        );
        assert_eq!(
            state.borrow().calls,
            1,
            "clock refusal must precede plan authorization"
        );
    }
    let event = [0xc3; 32];
    let refund = plan_for_event(
