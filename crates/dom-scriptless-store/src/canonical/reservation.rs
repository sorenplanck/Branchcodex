...
fn parse_derivation_base_context(
    ...
    match (purpose, context[244], context.len()) {
        (PurposeV1::ClaimAdaptor | PurposeV1::RefundAdaptor, 1, 278)
            if compressed_point_shape(&context[245..278]) => {}
...
pub(crate) mod tests {
    ...
    fn binding_with_session(purpose: PurposeV1, session_byte: u8) -> Vec<u8> {
        let with_adaptor = matches!(purpose, PurposeV1::ClaimAdaptor | PurposeV1::RefundAdaptor);
    ...
    #[test]
    fn refund_adaptor_reservation_requires_its_own_purpose_and_point(
    ) -> Result<(), CanonicalCodecError> {
        let refund = binding(PurposeV1::RefundAdaptor);
        let claim = binding(PurposeV1::ClaimAdaptor);
        let parsed = ReservationContextBindingV1::from_bytes(&refund)?;
        assert_eq!(parsed.purpose(), PurposeV1::RefundAdaptor);
        assert_ne!(
            parsed.context_binding_digest(),
            ReservationContextBindingV1::from_bytes(&claim)?.context_binding_digest()
        );
        let context = &refund[16..294];
        assert!(parse_derivation_base_context(context).is_ok());
        for purpose in [PurposeV1::Funding, PurposeV1::Refund, PurposeV1::Sponsor] {
            let mut wrong = context.to_vec();
            wrong[66] = purpose.to_byte();
            assert!(parse_derivation_base_context(&wrong).is_err());
        }
        let mut absent = context[..245].to_vec();
        absent[244] = 0;
        assert!(parse_derivation_base_context(&absent).is_err());
        let mut malformed = context.to_vec();
        malformed[245] = 0;
        assert!(parse_derivation_base_context(&malformed).is_err());
        for end in 0..refund.len() {
            assert!(ReservationContextBindingV1::from_bytes(&refund[..end]).is_err());
        }
        let mut trailing = refund;
        trailing.push(0);
        assert!(ReservationContextBindingV1::from_bytes(&trailing).is_err());
        Ok(())
    }
