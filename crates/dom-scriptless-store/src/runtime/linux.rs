...
impl RetainedDirectory {
    ...
    fn sync(&self) -> Result<(), LinuxCapabilityError> {
        self.revalidate()?;
        fsync_capability_dir(&self.descriptor).map_err(|error| map_errno("fsync-directory", error))
    }
    ...
    fn create_immutable_file(
        ...
        fsync_capability_dir(&self.descriptor)
            .map_err(|error| map_errno("fsync-parent-directory", error))?;
        #[cfg(test)]
    ...
    fn publish_restore_staging_no_replace(
        ...
        fsync_capability_dir(&self.descriptor)
            .map_err(|error| map_errno("fsync-restore-root", error))?;
    ...
    fn activate_restore_generation_no_replace(
        ...
        fsync_capability_dir(&pending.descriptor)
    ...
    fn activate_restore_pointer(
        ...
        fsync_capability_dir(&activation.descriptor)
...
fn require_synchronizable_directory(directory: &Dir) -> Result<(), LinuxCapabilityError> {
    fsync_capability_dir(directory).map_err(|error| map_errno("fsync-directory-preflight", error))
}
...
fn classify_registered_component(value: &str) -> Option<ExpectedNodeType> {
    if xmr_recovery::is_registered_v22_component(value) {
    ...
    let static_type = match value {
        ...
        | "xmr-recovery-scope-v11.bin"
        | "xmr-recovery-lock-v11.bin"
        | "xmr-recovery-archive-v11.bin"
        | ".xmr-recovery-archive-v11.staging"
        ...
        | "xmr-exit-observed-v12.bin"
        | ".xmr-exit-observed-v12.staging"
        | "xmr-funding-attempt-v12.bin"
        | ".xmr-funding-attempt-v12.staging" => Some(ExpectedNodeType::RegularFile),
        "restore-pending"
        | "journal"
...
fn classify_dynamic_component(value: &str) -> Option<ExpectedNodeType> {
    ...
    if exact_wrapped_hex(value, "", 64, ".authority")
        ...
        || exact_wrapped_hex(value, "", 64, ".xmr-graph-evidence-v23")
        || exact_wrapped_hex(value, ".", 64, ".xmr-graph-evidence-v23.staging")
        || exact_wrapped_hex(value, "", 64, ".xmr-graph-pin-v23")
        || exact_wrapped_hex(value, ".", 64, ".xmr-graph-pin-v23.staging")
        || exact_wrapped_hex(value, "", 64, ".xmr-graph-candidate-v22")
        || exact_wrapped_hex(value, ".", 64, ".xmr-graph-candidate-v22.staging")
...
mod tests {
    ...
    fn component_registry_rejects_unknown_and_noncanonical_names() {
        ...
        for suffix in [
            ".bootstrap-wallet-keys-v18",
            ".xmr-graph-candidate-v22",
            ".xmr-graph-pin-v23",
            ".xmr-graph-evidence-v23",
            ".xmr-graph-commit-context-v23",
            ".f7-v12-funding-signing-v20",
            ".f7-v12-claim-exposure-v14",
            ".f7-v12-claim-admission-v14",
            ".f7-v12-claim-observation-v15",
