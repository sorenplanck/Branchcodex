...
impl CollaborativeBpNonceVaultV1 for ContractsNonceVaultV1 {
    ...
    fn seal_fresh_material(
        ...
        self.store.revalidate_authority()?;
        self.ensure_collaborative_namespace()?;
        if self.collaborative_bp_custody_v16(binding)? != CollaborativeBpCustodyV16::Absent {
            return Err(InventoryError::RestoreQuarantined);
        }
    ...
    fn open_persisted_material(
        ...
        self.store.revalidate_authority()?;
        self.ensure_collaborative_namespace()?;
        if self.collaborative_bp_custody_v16(binding)? != CollaborativeBpCustodyV16::Nonce {
            return Err(InventoryError::RestoreQuarantined);
        }
        let record = self
            .store
    ...
    fn open_persisted_round2(
        ...
        self.ensure_collaborative_namespace()?;
        if !matches!(
            self.collaborative_bp_custody_v16(binding)?,
    ...
    fn open_persisted_finalize_continuation(
        ...
        self.store.revalidate_authority()?;
        self.ensure_collaborative_namespace()?;
        if self.collaborative_bp_custody_v16(binding.nonce_binding())?
            != CollaborativeBpCustodyV16::Round2
        {
            return Err(InventoryError::RestoreQuarantined);
        }
        require_absent(
    ...
    fn open_persisted_proof(
        ...
        self.ensure_collaborative_namespace()?;
        if self.collaborative_bp_custody_v16(binding.nonce_binding())?
            != CollaborativeBpCustodyV16::Proof
        {
            return Err(InventoryError::RestoreQuarantined);
        }
        require_absent(
...
mod tests {
    ...
    #[test]
    fn v16_native_bp_reopens_and_reconciles_both_publication_crash_prefixes(
    ) -> Result<(), Box<dyn Error>> {
        use dom_adaptor::{AggregateBpRound1, AggregateBpRound2, CollaborativeRangeProof};
        let a = TestDirectory::create()?;
        let b = TestDirectory::create()?;
        let mut va = create_vault(&a)?;
        let mut vb = create_vault(&b)?;
        let chain = chain();
        let capsule = capsule()?;
        let ca = contribute_vault_backed_blinding_share_v1(
            &chain,
            [0x71; 32],
            &roster(),
        ...
        .bind_recovery_capsule_restartable_v1(&capsule, &mut va)?;
        let cb = contribute_vault_backed_blinding_share_v1(
            &chain,
            [0x71; 32],
            &roster(),
        ...
        .bind_recovery_capsule_restartable_v1(&capsule, &mut vb)?;
        let points = vec![ca.public_key().clone(), cb.public_key().clone()];
        let commitment = BpStatementV1::aggregate_commitment_from_shares(&points, 100)?;
        let statement = BpStatementV1::new(
            ...
            points,
            commitment,
            Some(*dom_crypto::blake2b_256(capsule.as_bytes()).as_bytes()),
        )?;
        let binding = CollaborativeBpNonceBindingV1::from_statement(&statement, 0)?;
        assert_eq!(
            va.collaborative_bp_custody_v16(&binding)?,
            CollaborativeBpCustodyV16::Absent
        );
        let (pending_a, commit_a) = ca.begin_collaborative_range_proof_v1(&statement, &mut va)?;
        let original_reveal = pending_a.reveal_bytes();
        let old_nonce = va
            .store
            .load_collaborative_state(binding.binding_digest_v1(), StateKind::BpNonce)?;
        drop(pending_a);
        drop(va);
        let mut va = open_vault(&a)?;
        let (pending_a, recovered_commit) =
            ca.resume_collaborative_range_proof_v16(&statement, &mut va)?;
        assert_eq!(commit_a, recovered_commit);
        assert_eq!(original_reveal[..], pending_a.reveal_bytes()[..]);
        assert!(ca
            .begin_collaborative_range_proof_v1(&statement, &mut va)
            .is_err());
        let (pending_b, commit_b) = cb.begin_collaborative_range_proof_v1(&statement, &mut vb)?;
        let reveals = vec![pending_a.reveal_bytes(), pending_b.reveal_bytes()];
        let local_a = pending_a.finish(&statement, &[commit_a, commit_b], reveals.clone())?;
        let local_b = pending_b.finish(&statement, &[commit_a, commit_b], reveals)?;
        let da = ca.bind_collaborative_range_proof_v1(&statement, capsule.as_bytes().to_vec())?;
        let db = cb.bind_collaborative_range_proof_v1(&statement, capsule.as_bytes().to_vec())?;
        let r1a = da.round1(&statement, &local_a)?;
        let r1b = db.round1(&statement, &local_b)?;
        ...
        let r2a = da
            .round2_vault_backed_v1(&statement, &local_a, &r1, &mut va)?
            .into_zeroizing_bytes();
        let r2b = db
            .round2_vault_backed_v1(&statement, &local_b, &r1, &mut vb)?
            .into_zeroizing_bytes();
        // Reconstruct the exact on-disk prefix after successor publication
        // and before deletion. Only test code may publish this old record.
        va.store.publish_collaborative_state(&old_nonce)?;
        drop(va);
        let mut va = open_vault(&a)?;
        assert_eq!(
            va.collaborative_bp_custody_v16(&binding)?,
        ...
        assert!(ca
            .resume_collaborative_range_proof_v16(&statement, &mut va)
            .is_err());
        assert!(ca
            .begin_collaborative_range_proof_v1(&statement, &mut va)
            .is_err());
        assert_eq!(
            da.resume_persisted_round2_v1(&statement, 0, &mut va)?
                .into_zeroizing_bytes()
                .as_ref(),
            r2a.as_ref()
        );
        let old_finalizer = load_round2_by_nonce(&va.store, &binding)?;
        let r2 = AggregateBpRound2::new(
            &statement,
            vec![
                BpRound2ShareV1::from_bytes(r2a.as_ref(), &statement)?,
                BpRound2ShareV1::from_bytes(r2b.as_ref(), &statement)?,
            ],
        )?;
        let proof = da
            .finalize_vault_backed_v1(&statement, 0, &r1, &r2, &mut va)?
            .into_proof();
        va.store.publish_collaborative_state(&old_finalizer)?;
        drop(va);
        let mut va = open_vault(&a)?;
        assert_eq!(
            va.collaborative_bp_custody_v16(&binding)?,
        ...
        let resumed = da
            .resume_persisted_proof_v1(&statement, 0, &r1, &mut va)?
            .into_proof();
        assert_eq!(proof.as_bytes(), resumed.as_bytes());
        da.verify_final(&statement, &resumed)?;
        assert_eq!(
            da.resume_persisted_round2_v1(&statement, 0, &mut va)?
                .into_zeroizing_bytes()
                .as_ref(),
        ...
        // Proof + live nonce is not a publication prefix and must never be
        // silently healed. It could otherwise conceal rollback/replacement.
        va.store.publish_collaborative_state(&old_nonce)?;
        assert!(va.collaborative_bp_custody_v16(&binding).is_err());
        Ok(())
    }

    #[test]
    fn v16_early_reveal_remains_bound_to_context_chain_roster_and_share(
    ) -> Result<(), Box<dyn Error>> {
        use dom_adaptor::EarlyShareRevealV1;
        let directory = TestDirectory::create()?;
        let mut vault = create_vault(&directory)?;
        let chain = chain();
        let share = contribute_vault_backed_blinding_share_v1(
            &chain,
            [0x75; 32],
            &roster(),
            DirectionV1::Initiator,
        ...
        let context = [0x77; 32];
        let bytes = share.early_transport_reveal_v16(context)?.to_bytes();
        assert!(EarlyShareRevealV1::from_bytes(&bytes, &chain, &roster(), &context).is_ok());
        assert!(EarlyShareRevealV1::from_bytes(&bytes, &chain, &roster(), &[0x78; 32]).is_err());
        let other_chain =
            TrustedChainIdV1::from_authenticated_genesis(2, &Hash256::from_bytes([3; 32]));
        assert!(EarlyShareRevealV1::from_bytes(&bytes, &other_chain, &roster(), &context).is_err());
        ...
        for index in 0..bytes.len() {
            ...
            bad[index] ^= 1;
            assert!(
                EarlyShareRevealV1::from_bytes(&bad, &chain, &roster(), &context).is_err(),
                "single-bit mutation at byte {index} was accepted"
            );
        }
        Ok(())
    }
...
/// Authenticated public stage of one native BP nonce lineage.
/// This exposes neither ciphertext nor scalar material and mints no signing permission.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CollaborativeBpCustodyV16 {
    /// The fully audited namespace has no state for this binding.
    Absent,
    /// The original encrypted nonce remains live.
    Nonce,
    /// Round two and its finalizer are durable; the nonce was consumed.
    Round2,
    /// The final proof and retained round two are durable.
    Proof,
}

impl ContractsNonceVaultV1 {
    /// Audit the whole namespace and reconcile only authenticated publication
    /// prefixes: round two before nonce retirement, or proof before finalizer
    /// retirement. This never reactivates a nonce or returns secret material.
    /// Mixed finalizer bindings and a proof overlapping a live nonce are hard
    /// errors. The consumer still opens/verifies the native sealed payload.
    pub fn collaborative_bp_custody_v16(
        &mut self,
        nonce: &CollaborativeBpNonceBindingV1,
    ) -> Result<CollaborativeBpCustodyV16, InventoryError> {
        self.store.revalidate_authority()?;
        self.ensure_collaborative_namespace()?;
        audit_namespace(&self.store)?;
        let aad = nonce.to_aad_bytes();
        let mut records: [Option<StateRecord>; 3] = [None, None, None];
        self.store
            .collaborative_secrets()?
            .scan_lexicographic(|name, node| {
                if node.node_type != ExpectedNodeType::RegularFile || name.starts_with('.') {
                ...
                let record = StateRecord::from_bytes(&bytes)
                    .map_err(|_| LinuxCapabilityError::ExactBytesMismatch)?;
                if record.kind == StateKind::TerminalSession
                    && record.binding.get(10..42) == Some(nonce.session_id().as_slice())
                {
                    return Err(LinuxCapabilityError::ExactBytesMismatch);
                }
                let slot = match record.kind {
                    StateKind::BpNonce if record.binding == aad => Some(0),
                    StateKind::BpRound2Finalize
                        if record.binding.get(10..150) == Some(aad.as_slice()) =>
                    {
                        Some(1)
                    }
                    StateKind::BpProof if record.binding.get(10..150) == Some(aad.as_slice()) => {
                        Some(2)
                    }
                    _ => None,
                };
                if let Some(slot) = slot {
                    if records[slot].replace(record).is_some() {
                        return Err(LinuxCapabilityError::ExactBytesMismatch);
                    }
                }
                Ok(())
            })?;
        match (&records[0], &records[1], &records[2]) {
            (None, None, None) => Ok(CollaborativeBpCustodyV16::Absent),
            (Some(_), None, None) => Ok(CollaborativeBpCustodyV16::Nonce),
            (None, Some(_), None) => Ok(CollaborativeBpCustodyV16::Round2),
            (None, None, Some(_)) => Ok(CollaborativeBpCustodyV16::Proof),
            (Some(old), Some(_), None) => {
                // The authenticated successor was durably published. Returning
                // Nonce here would make an already consumed nonce usable again.
                self.store
                    .delete_collaborative_state(old.binding_digest, StateKind::BpNonce)?;
                Ok(CollaborativeBpCustodyV16::Round2)
            }
            (None, Some(old), Some(proof))
                if old.binding == proof.binding
                    && old.envelopes[0].as_bytes() == proof.envelopes[1].as_bytes() =>
            {
                self.store
                    .delete_collaborative_state(old.binding_digest, StateKind::BpRound2Finalize)?;
                Ok(CollaborativeBpCustodyV16::Proof)
            }
            _ => Err(InventoryError::RestoreQuarantined),
        }
    }
