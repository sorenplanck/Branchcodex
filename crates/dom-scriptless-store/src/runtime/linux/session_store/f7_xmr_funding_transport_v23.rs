//! Native bounded Funding transport; consumes ordinary identity-signing requests.
use super::*;
impl ContractsSessionStoreV1 {
    ...
    pub fn prepare_xmr_bounded_funding_dsc1_request_v23(
        &self,
        ...
        self.prepare_xmr_bounded_funding_dsc1_request_locked_v23(chain, target, payload)
    }

    pub(in super::super::super) fn prepare_xmr_bounded_funding_dsc1_request_locked_v23(
        &self,
        chain: TrustedChainIdV1,
        target: [u8; 32],
        payload: &[u8],
    ) -> Result<Option<PreparedDsc1SigningRequestV1>, SessionStoreError> {
        let binding = self.authenticate_xmr_bounded_funding_binding_v23(target)?;
        if chain != binding.origin.chain {
            return Err(SessionStoreError::Conflict);
        }
        let current = self.load_session_locked(target)?;
        let round = self.audit_xmr_bounded_funding_round_v23(&binding)?;
        ...
        if position > 6
            ...
            || (position != 0 && current.phase() != SessionPhaseV1::FundingAuthorized)
        {
    ...
    pub fn accept_xmr_bounded_funding_transport_v23(
        &self,
        chain: TrustedChainIdV1,
        target: [u8; 32],
        signed_bytes: &[u8],
        ...
        let _guard = self.operation_lock()?;
        self.audit_transport()?;
        self.accept_xmr_bounded_funding_transport_locked_v23(chain, target, signed_bytes)
    }

    pub(in super::super::super) fn accept_xmr_bounded_funding_transport_locked_v23(
        &self,
        chain: TrustedChainIdV1,
        target: [u8; 32],
        signed_bytes: &[u8],
