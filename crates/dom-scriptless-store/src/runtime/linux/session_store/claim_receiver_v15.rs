...
impl ContractsSessionStoreV1 {
    ...
    /// The final claim receiver has already persisted its observation, so
    /// `f7_final_claim_awaits_observation_v16` no longer holds the sender's
    /// final claim back — but the ingress authority that accepts it lives only
    /// in the relay's memory and is installed by `step_f7_claim_receiver_v15`
    /// on the next native round. A fresh process after reopen, or a relay turn
    /// that runs ahead of that round, therefore meets an unseen 0x12 with no
    /// authority. Answer true exactly when the acceptance edge
    /// (`require_f7_final_claim_receiver_edge_v15`) will take this message once
    /// the authority exists, and false otherwise, so a message that edge would
    /// never accept still fails closed as before.
    pub fn f7_final_claim_awaits_ingress_handoff_v29(
        &self,
        session: [u8; 32],
        local_participant: [u8; 32],
        signed_bytes: &[u8],
        ...
        let _guard = self.operation_lock()?;
        let envelope = ParsedTransportEnvelopeV1::parse(signed_bytes)?;
        if envelope.session_id != session || envelope.message_type != 0x12 {
            return Ok(false);
        }
        self.audit_f7_artifact_inventory_v12()?;
        let (sessions, _, _) = self.census_f7_artifacts_v12()?;
        ...
        if signer.participant_id != local_participant
            ...
            || current.phase() != SessionPhaseV1::FundingConfirmed
            || !current.irreversible().adaptor_secret_exposed
            || current.revision() < record.successor.revision()
        {
            return Ok(false);
        }
        let pre = self.load_f7_pre_v12(session)?;
        ...
        if envelope.verify(&sender.identity_key).is_err()
        ...
        if canonical_transaction_hash_v1(payload)
            .map_err(|_| SessionStoreError::InvalidDomTransaction)?
            != record.hashes[6]
        {
            return Ok(false);
        ...
        Ok(self
            .verify_f7_final_claim_bytes_v14(&gate, &issued, &pre, payload, record.height)
            .is_ok())
    }
