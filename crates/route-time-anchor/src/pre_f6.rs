...
impl PreF6TimePolicyV2 {
    ...
    pub fn from_registry(
        ...
        Self::from_registry_profile_v23(scope, registry, limits, false)
    }

    /// Mainnet DOM finalized-height authority for pre-F6 negotiation only.
    /// Registry scope and signed evidence remain mandatory; no economic grant.
    pub fn from_registry_dom_mainnet_v23(
        scope: PreF6TimeScopeV2,
        registry: &ResolvedRegistryV1,
        limits: PreF6TimePolicyLimitsV2,
    ) -> Result<Self> {
        Self::from_registry_profile_v23(scope, registry, limits, true)
    }
    ...
    fn from_registry_profile_v23(
        ...
        if !native_dom_mainnet_v23 && manifest.dom.runtime_identity.network == DomNetworkV1::Mainnet
        {
    ...
    pub fn canonical_bytes(self) -> Result<Vec<u8>> {
        ...
        out.extend_from_slice(if self.native_dom_mainnet_v23 {
            b"DOMPF6P3"
        } else {
            POLICY_MAGIC_V2
        });
    ...
    pub fn policy_digest(self) -> Result<Digest32> {
        digest_parts(
            if self.native_dom_mainnet_v23 {
                b"DOM-INTEROP/PRE-F6-TIME/POLICY/V23\0"
            } else {
                POLICY_DOMAIN_V2
            },
