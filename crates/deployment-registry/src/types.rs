...
///
/// The testnet and stagenet values are independently corroborated by monero-project's own
...
///
/// Mainnet uses the upstream genesis transaction and nonce 10000, independently
/// derived by the regression below. See also the Cuprate implementation book:
/// https://monero-book.cuprate.org/consensus_rules/genesis_block.html
const RATIFIED_MONERO_GENESIS: &[(MoneroNetworkV1, Digest32)] = &[
    (
        MoneroNetworkV1::Mainnet,
        hex_digest(b"418015bb9ae982a1975da7d79277c2705727a56894ba0fb246adaabb1f4632e3"),
    ),
    (
...
mod monero_genesis_tests {
    ...
    fn the_ratified_genesis_is_the_hash_derived_from_the_genesis_transaction() {
        ...
        assert_eq!(
            ratified_monero_genesis(MoneroNetworkV1::Mainnet),
            Some(derive_genesis(TESTNET_GENESIS_TX, 10000))
        );
        assert_eq!(
    ...
    fn every_profileable_monero_network_has_a_ratified_genesis() {
        ...
        for network in [
            ...
            match network {
                ...
                MoneroNetworkV1::Mainnet | MoneroNetworkV1::Stagenet | MoneroNetworkV1::Testnet => {
                }
            }
        }
    }
    ...
    #[test]
    fn the_three_networks_do_not_share_a_genesis() {
        // A shared genesis would let a profile for one network accept evidence
        // observed on another. Check Mainnet against both laboratory networks.
        let networks = [
            MoneroNetworkV1::Mainnet,
            MoneroNetworkV1::Stagenet,
            MoneroNetworkV1::Testnet,
        ...
        for (index, network) in networks.iter().enumerate() {
            for other in &networks[index + 1..] {
                assert_ne!(
                    ratified_monero_genesis(*network),
                    ratified_monero_genesis(*other)
                );
            }
        }
    }
