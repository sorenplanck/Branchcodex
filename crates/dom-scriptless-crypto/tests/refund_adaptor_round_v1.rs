...
#[test]
fn v10_public_refund_observer_restores_and_extracts_without_private_witness() -> TestResult {
    let (session, round, partials) = run(1001)?;
    let public = round
        .aggregate_pre_signature_v1(&partials)
        .map_err(|e| e.to_string())?;
    let bytes = public.to_bytes();
    let final_bytes = public
        .adapt(&session.refund_secret)
        .map_err(|e| e.to_string())?;
    drop(public);
    // New verifier uses only frozen public round data and the durable wire record.
    let reopened = round
        .restore_pre_signature_v1(&bytes)
        .map_err(|e| e.to_string())?;
    drop(session);
    let recovered = reopened.extract(&final_bytes).map_err(|e| e.to_string())?;
    let witness = AdaptorSecret::from_be_bytes(*recovered).map_err(|e| e.to_string())?;
    assert_eq!(
        witness
            .public_point()
            .map_err(|e| e.to_string())?
            .to_compressed_bytes(),
        reopened.refund_adaptor_point()
    );
    for index in [0, 1, 32, 33, 64] {
        let mut corrupted = final_bytes;
        corrupted[index] ^= 1;
        assert!(reopened.extract(&corrupted).is_err());
    }
    Ok(())
}

#[test]
fn v10_refund_restore_rejects_mutation_relabeling_and_another_round() -> TestResult {
    let (session, round, partials) = run(1002)?;
    let public = round
        .aggregate_pre_signature_v1(&partials)
        .map_err(|e| e.to_string())?;
    let bytes = public.to_bytes();
    for index in 0..bytes.len() {
        ...
        corrupted[index] ^= 1;
        assert!(round.restore_pre_signature_v1(&corrupted).is_err());
    }
    assert!(round.restore_pre_signature_v1(&bytes[..161]).is_err());
    let mut extended = bytes.to_vec();
    extended.push(0);
    assert!(round.restore_pre_signature_v1(&extended).is_err());
    let (_, other, _) = run(1003)?;
    assert!(other.restore_pre_signature_v1(&bytes).is_err());
    let mut inputs = session.inputs();
    inputs.transcript_hash[0] ^= 1;
    let relabeled = begin_refund_adaptor_round_v1(&inputs).map_err(|e| e.to_string())?;
    assert!(relabeled.restore_pre_signature_v1(&bytes).is_err());
    Ok(())
}
