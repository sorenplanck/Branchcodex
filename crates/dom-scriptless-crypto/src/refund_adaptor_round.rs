...
//! ```
//!
//! Either completion teaches the waiting party the share they lacked. The
//! surrounding transaction graph and timing rules must prevent conflicting
//! economic outcomes; the equations alone establish neither atomic settlement
//! nor liveness if the witness owner disappears.
//! A public pre-signature plus any final signature reveals U immediately,
//! including off chain or in the mempool. Recovery requires separately
//! authenticated custody, a safe transaction graph, and reveal windows.
//!
...
pub fn begin_refund_adaptor_round_v1(
    ...
    Ok(RefundAdaptorRoundV1 {
        ...
        session_id: inputs.binding_context.session_id,
...
impl RefundAdaptorRoundV1 {
    ...
    /// Aggregates verified refund partials without requiring the refund witness.
    /// Persist the returned public bytes before funding; restore them against
    /// this same authenticated public round after a process restart.
    pub fn aggregate_pre_signature_v1(
        &self,
        partials: &[PartialSignatureV1],
        ...
        self.restore_pre_signature_v1(&pre.to_bytes())
    }

    /// Revalidates persisted public bytes against the exact refund round.
    /// This checks cryptographic consistency, not Store provenance or finality.
    pub fn restore_pre_signature_v1(
        &self,
        bytes: &[u8],
    ) -> Result<VerifiedRefundPreSignatureV1, RefundAdaptorRoundError> {
    ...
    /// Complete the local cryptographic cycle using the same public verifier
    /// used by the observer. This result alone grants no execution authority.
    pub fn complete_cycle_v1(
        &self,
        partials: &[PartialSignatureV1],
        adaptor_secret: &AdaptorSecret,
    ) -> Result<CompletedRefundAdaptorCycleV1, RefundAdaptorRoundError> {
        let pre = self.aggregate_pre_signature_v1(partials)?;
        let final_signature = pre.adapt(adaptor_secret)?;
        let revealed = pre.extract(&final_signature)?;
...
/// Public refund verifier with private, checked cryptographic bindings.
/// No signing secret or secret nonce is retained. A caller must separately
/// authenticate the round's Store provenance and the final transaction.
pub struct VerifiedRefundPreSignatureV1 {
    pre: AdaptorPreSignatureV1,
    // Public verification context must survive recovery. Retaining only the
    // final equation would lose the session-dependent binding factor.
    participants: Vec<ParticipantPublicNoncesV1>,
    aggregate_signing_key: PublicKey,
    chain_id: [u8; 32],
    session_id: [u8; 32],
...
impl VerifiedRefundPreSignatureV1 {
    /// Published nonce roster whose binding factor was checked by the native
    /// refund round. These are public points, never secret nonce material.
    pub fn public_nonce_roster_v11(&self) -> &[ParticipantPublicNoncesV1] {
        &self.participants
    }
    /// Exact public pre-signature bytes, suitable for the authenticated journal.
    pub fn to_bytes(&self) -> [u8; REFUND_ADAPTOR_PRE_SIGNATURE_LEN] {
        self.pre.to_bytes()
    }
    /// Network over which the signature equation is verified.
    pub const fn chain_id(&self) -> &[u8; 32] {
        &self.chain_id
    }
    /// Session used by the verified two-nonce binding-factor transcript.
    pub const fn session_id(&self) -> &[u8; 32] {
        &self.session_id
    }
    /// Exact signature-omitting refund template commitment.
    pub const fn template_hash(&self) -> &[u8; 32] {
        &self.template_hash
    }
    /// Pinned transcript commitment of the public round.
    pub const fn transcript_hash(&self) -> &[u8; 32] {
        &self.transcript_hash
    }
    /// Public refund witness point U.
    pub fn refund_adaptor_point(&self) -> [u8; 33] {
        self.pre.adaptor_point().to_compressed_bytes()
    }

    /// Aggregate key verified by the refund equation, for a native transaction
    /// builder to require equality with the exact kernel excess.
    pub fn aggregate_signing_key(&self) -> [u8; 33] {
        self.aggregate_signing_key.to_compressed_bytes()
    }

    /// Native kernel message digest verified by the refund equation.
    pub const fn kernel_message_digest(&self) -> &[u8; 32] {
        &self.kernel_message_digest
    }

    /// Complete this pre-signature with its witness and verify native DOM bytes.
    /// Publication still requires the separate persisted refund authority.
    pub fn adapt(&self, witness: &AdaptorSecret) -> Result<[u8; 65], RefundAdaptorRoundError> {
        let signature = self
            .pre
            .adapt(
                witness,
    ...
    fn verify_final(&self, signature: &SchnorrSignature) -> Result<(), RefundAdaptorRoundError> {
        match dom_scriptless_primitives::scriptless_verify_final_signature(
            ...
            Ok(true) => Ok(()),
            Ok(false) => Err(RefundAdaptorRoundError::FinalSignatureRejected),
            Err(_) => Err(RefundAdaptorRoundError::BackendRefused),
        }
    }

    /// Recover u from an externally observed signature, without the witness.
    /// Requires the native final-signature equation and u*G == U. This method
    /// does not treat a valid signature as evidence of chain publication.
    pub fn extract(
        &self,
        bytes: &[u8; 65],
    ) -> Result<zeroize::Zeroizing<[u8; 32]>, RefundAdaptorRoundError> {
        let signature = SchnorrSignature::from_bytes(bytes)
            .map_err(|_| RefundAdaptorRoundError::FinalSignatureRejected)?;
        if signature.to_bytes() != *bytes {
            return Err(RefundAdaptorRoundError::FinalSignatureRejected);
        }
        self.verify_final(&signature)?;
        let extracted = self
            .pre
