...
use dom_rpc::{
    ...
    MempoolTxInfo, NodeHandle, PeerInfo, RegtestMineFuture, RegtestMineRequestV1,
    ...
    WalletSlateOfferV1, WalletSlateSubmitRequestV1, WalletSlateSubmittedV1, MAX_ANCESTRY_STEPS,
    MAX_SCAN_RANGE,
};
    ...
    fn scan_chain_full_v1_with_budget(
        ...
        if request.start_height <= end_height {
            for height in request.start_height..=end_height {
                let Some(projected) =
                    crate::wallet_core_api::EmbeddedWalletCoreApi::load_scan_projection_locked(
                        &chain, &identity, height, None,
                    )
...
mod tests {
    ...
    use dom_rpc::{
        AncestryRequest, RegtestMineRequestV1, RpcError, SpendRequest, TxAdmissionState,
        WalletSlateCreateRequestV1, WalletSlateFinalizeRequestV1, WalletSlateSubmitRequestV1,
    };
    use dom_serialization::{DomDeserialize, DomSerialize};
