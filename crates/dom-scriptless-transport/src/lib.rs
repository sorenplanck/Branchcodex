...
mod xmr_graph_template_commit_v22;
pub use xmr_graph_template_commit_v22::{
    XmrGraphTemplateCommitPayloadV22, XMR_GRAPH_TEMPLATE_COMMIT_PAYLOAD_LEN_V22,
};
...
/// Exact public readiness commitment carried by the V12 F7 extension (`0x17`).
pub const UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12: usize = 32;
...
/// Closed DSC1 registry: Master section 8.3 and explicitly versioned extensions.
...
pub enum MessageTypeV1 {
    ...
    /// Bilateral acknowledgement of one Store-authenticated F7 recovery gate.
    UniversalReadyToFundV12 = 0x17,
    /// Identity-signed commitment to the independently reconstructed XMR graph.
    XmrGraphTemplateCommitV22 = 0x18,
    /// Public request for one remote native-XMR claim or refund sweep.
    XmrRemoteSweepRequestV23 = 0x19,
    /// Exact raw transaction and proof bundle answering the request.
    XmrRemoteSweepResponseV23 = 0x1a,
}
...
impl MessageTypeV1 {
    ...
    pub const fn payload_cap(self) -> usize {
        match self {
            ...
            Self::UniversalReadyToFundV12 => UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12,
            Self::XmrGraphTemplateCommitV22 => XMR_GRAPH_TEMPLATE_COMMIT_PAYLOAD_LEN_V22,
            Self::XmrRemoteSweepRequestV23 => {
                xmr_remote_sweep_wire::MAX_REMOTE_SWEEP_REQUEST_BYTES_V23
            }
            Self::XmrRemoteSweepResponseV23 => {
                xmr_remote_sweep_wire::MAX_REMOTE_SWEEP_RESPONSE_BYTES_V23
            }
            Self::ShareReveal
            | Self::BpRound1
            | Self::BpRound2
    ...
    fn accepts_phase(self, phase: SessionPhaseV1) -> bool {
        match self {
            ...
            Self::TxTemplateCommit | Self::XmrGraphTemplateCommitV22 => {
            ...
            Self::UniversalReadyToFundV12 => matches!(
                phase,
                SessionPhaseV1::TemplatesCommitted | SessionPhaseV1::RefundSigned
            ),
            ...
            Self::XmrRemoteSweepRequestV23 | Self::XmrRemoteSweepResponseV23 => matches!(
                phase,
                SessionPhaseV1::FundingConfirmed
                    | SessionPhaseV1::ClaimBroadcast
                    | SessionPhaseV1::RefundEligible
...
impl TryFrom<u8> for MessageTypeV1 {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        Ok(match value {
            ...
            0x17 => Self::UniversalReadyToFundV12,
            0x18 => Self::XmrGraphTemplateCommitV22,
            0x19 => Self::XmrRemoteSweepRequestV23,
            0x1a => Self::XmrRemoteSweepResponseV23,
            _ => return Err(TransportError::UnknownMessageType),
        })
    }
}

/// Public acknowledgement of one exact native F7 readiness commitment.
///
/// This is wire data, never a funding capability. The receiving Store must
/// reconstruct the commitment from authenticated terms, role, recovery and
/// local custody before accepting either roster participant's vote. The DSC1
/// signature also binds chain, session, sender, sequence and transcript.
/// No final adaptor signature or secret is carried by this message.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct UniversalReadyToFundPayloadV12 {
    readiness_digest: [u8; UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12],
}

impl UniversalReadyToFundPayloadV12 {
    /// Encodes a nonzero readiness commitment; this grants no execution right.
    pub fn new(
        readiness_digest: [u8; UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12],
    ) -> Result<Self, TransportError> {
        if readiness_digest == [0; UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12] {
            return Err(TransportError::ZeroPayloadCommitment);
        }
        Ok(Self { readiness_digest })
    }

    /// Rejects all truncations, trailing data and empty commitments.
    pub fn decode_exact(payload: &[u8]) -> Result<Self, TransportError> {
        if payload.len() != UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12 {
            return Err(TransportError::NonCanonicalLength);
        }
        Self::new(array_32(payload)?)
    }

    /// Exact digest that both authenticated participants must acknowledge.
    #[must_use]
    pub const fn readiness_digest(&self) -> &[u8; UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12] {
        &self.readiness_digest
    }

    /// Returns the exact fixed-size wire payload.
    #[must_use]
    pub const fn into_bytes(self) -> [u8; UNIVERSAL_READY_TO_FUND_PAYLOAD_LEN_V12] {
        self.readiness_digest
    }
}
...
impl UnsignedMessageV1 {
    ...
    /// Constructs the public, identity-signed V12 F7 readiness vote.
    pub fn new_universal_ready_to_fund_v12(
        chain_id: [u8; 32],
        session_id: [u8; 32],
        sender_id: [u8; 32],
        sequence: u64,
        previous_transcript_hash: [u8; 32],
        payload: UniversalReadyToFundPayloadV12,
    ) -> Result<Self, TransportError> {
        Self::new(
            MessageTypeV1::UniversalReadyToFundV12,
            chain_id,
            session_id,
            sender_id,
            sequence,
...
fn validate_registered_payload(kind: MessageTypeV1, payload: &[u8]) -> Result<(), TransportError> {
    match kind {
        ...
        MessageTypeV1::UniversalReadyToFundV12 => {
            UniversalReadyToFundPayloadV12::decode_exact(payload)?;
        }
        MessageTypeV1::XmrGraphTemplateCommitV22 => {
            XmrGraphTemplateCommitPayloadV22::decode_exact(payload)?;
        }
        MessageTypeV1::XmrRemoteSweepRequestV23 => {
            xmr_remote_sweep_wire::RemoteSweepRequestV23::decode_exact(payload)
                .map_err(|_| TransportError::InvalidTypedPayload)?;
        }
        MessageTypeV1::XmrRemoteSweepResponseV23 => {
...
mod tests {
    ...
    #[test]
    fn universal_ready_v12_exact_codec_and_closed_registry() -> Result<(), Box<dyn Error>> {
        let key = identity(21)?;
        let payload = UniversalReadyToFundPayloadV12::new([0x71; 32])?;
        let message = SignedMessageV1::sign(
            UnsignedMessageV1::new_universal_ready_to_fund_v12(
                [0x11; 32], [0x22; 32], [0xa5; 32], 0, [0x43; 32], payload,
            )?,
            &key,
        )?;
        ...
        assert_eq!(
            decoded.unsigned().kind(),
            MessageTypeV1::UniversalReadyToFundV12
        );
        assert_eq!(decoded.unsigned().payload(), payload.readiness_digest());
        assert_eq!(MessageTypeV1::UniversalReadyToFundV12.payload_cap(), 32);
        assert_eq!(
            MessageTypeV1::try_from(0x17)?,
            MessageTypeV1::UniversalReadyToFundV12
        );
        assert_eq!(
            MessageTypeV1::try_from(0x1b),
            Err(TransportError::UnknownMessageType)
        );
        for len in 0..=33 {
            if len != 32 {
                assert_eq!(
                    UniversalReadyToFundPayloadV12::decode_exact(&vec![0x71; len]),
                    Err(TransportError::NonCanonicalLength),
                );
                assert!(UnsignedMessageV1::new(
                    MessageTypeV1::UniversalReadyToFundV12,
                    [0x11; 32],
                    [0x22; 32],
                    [0xa5; 32],
        ...
        assert_eq!(
            UniversalReadyToFundPayloadV12::decode_exact(&[0; 32]),
            Err(TransportError::ZeroPayloadCommitment),
        );
        // Canonical envelope decoding enforces the typed payload independently
        // of whether a signature could verify afterwards.
        let mut zeroed = message.as_bytes().to_vec();
        zeroed[UNSIGNED_PREFIX_LEN_V1..UNSIGNED_PREFIX_LEN_V1 + 32].fill(0);
        assert_eq!(
            SignedMessageV1::decode_exact(&zeroed).err(),
    ...
    #[test]
    fn universal_ready_v12_signature_binds_type_scope_and_commitment() -> Result<(), Box<dyn Error>>
    {
        let key = identity(21)?;
        let message = signed(
            &key,
        ...
        // Type substitution into legacy readiness, chain, session, participant,
        // sequence, transcript and economic commitment all invalidate identity.
        for offset in [6, 8, 40, 72, 104, 112, UNSIGNED_PREFIX_LEN_V1] {
            let mut bytes = message.as_bytes().to_vec();
            if offset == 6 {
                bytes[offset] = MessageTypeV1::ReadyToFund as u8;
            } else {
                bytes[offset] ^= 1;
            }
            let altered = SignedMessageV1::decode_exact(&bytes)?;
            assert_eq!(
                altered.verify_identity(&key.public_key()),
        ...
        let transplanted = SignedMessageV1::sign(
            UnsignedMessageV1::new_universal_ready_to_fund_v12(
                ...
                0,
                [0x43; 32],
                UniversalReadyToFundPayloadV12::new([0x71; 32])?,
            )?,
            &key,
        )?;
        let mut receiver = abort_receiver(SessionPhaseV1::TemplatesCommitted, [0x43; 32])?;
        assert_eq!(
            receiver.accept(transplanted.as_bytes(), SessionPhaseV1::TemplatesCommitted),
            Err(TransportError::ContextMismatch)
        );
        assert_eq!(receiver.transcript_hash(), &[0x43; 32]);
        Ok(())
    }

    #[test]
    fn universal_ready_v12_keeps_phase_and_replay_is_exact() -> Result<(), Box<dyn Error>> {
        let key = identity(21)?;
        for phase in [
            SessionPhaseV1::TemplatesCommitted,
            SessionPhaseV1::RefundSigned,
            ...
            assert_eq!(
                receiver.accept(message.as_bytes(), SessionPhaseV1::FundingAuthorized),
                Err(TransportError::PhaseMismatch)
            );
            assert_eq!(receiver.transcript_hash(), &initial);
            let receipt = receiver.accept(message.as_bytes(), phase)?;
            assert_eq!(receiver.phase(), phase);
            assert_ne!(receipt.transcript_hash, initial);
            assert!(receiver.accept(message.as_bytes(), phase)?.duplicate);
            ...
            assert_eq!(
                receiver.accept(conflicting.as_bytes(), phase),
        ...
        for phase in [
            ...
            assert!(!MessageTypeV1::UniversalReadyToFundV12.accepts_phase(phase));
        }
        assert!(!valid_phase_movement(
            SessionPhaseV1::TemplatesCommitted,
            SessionPhaseV1::RefundSigned,
            MessageTypeV1::UniversalReadyToFundV12
        ));
