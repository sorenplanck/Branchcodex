...
CREATE TABLE settlement_children (
    ...
    face_tag INTEGER NOT NULL CHECK(face_tag BETWEEN 1 AND 5),
...
impl DurableSettlementCoordinatorV1 {
    ...
    pub fn resume_create_production(
        ...
        match state {
            ...
            ResumableCreationStateV1::LegacyV3 => {
                migration_v19::migrate(&mut connection, coordinator_id, plan_authority_id, true)?
            }
        }
    ...
    /// Open an existing authority. Exact V3 layouts migrate transactionally
    /// to V4 under the same process lock; unrelated schemas are refused.
    pub fn open_existing(
        ...
        if state == ResumableCreationStateV1::LegacyV3 {
            migration_v19::migrate(&mut connection, coordinator_id, plan_authority_id, false)?;
        }
        let store = Self {
            connection,
            path: path.to_path_buf(),
    ...
    F: FnOnce() -> Result<()>,
{
    ...
    transaction.execute_batch(SCHEMA_V4).map_err(storage)?;
    transaction
        .execute(
...
fn preflight_resumable_creation_state(
    ...
    if version == 3 {
        migration_v19::require_exact_v3(&connection)?;
        validate_open_file_identity(database_authority, path)?;
        return Ok(ResumableCreationStateV1::LegacyV3);
    }
    if version != SCHEMA_VERSION {
        return Err(CoordinatorErrorV1::UnsupportedFormat);
    }
    let reference = Connection::open_in_memory().map_err(storage)?;
    reference.execute_batch(SCHEMA_V4).map_err(storage)?;
