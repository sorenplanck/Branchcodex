...
// V19: these tests cover coordinator persistence/migration, not chain swaps.
fn economic_rows_v19(path: &Path) -> Vec<(String, Vec<Vec<rusqlite::types::Value>>)> {
    let connection = rusqlite::Connection::open(path).expect("snapshot open");
    [
...
#[test]
fn v19_migrates_validated_v18_without_changing_economic_rows_or_fences() {
    let (_root, path) = state_path();
    let mut store = create_store(&path);
    let (plan_id, lease) = install(&mut store, plan(SettlementActionV1::Funding, 7, 2));
    let before_view = store.load_plan(plan_id).expect("before view");
    drop(store);
    {
        // The validated V18 already had all five face tags, but still V3.
        let connection = rusqlite::Connection::open(&path).expect("V18 fixture");
        connection
            .pragma_update(None, "user_version", 3)
            .expect("V3 version");
    }
    let before_rows = economic_rows_v19(&path);
    let store = open_store(&path);
    assert_eq!(
        store.load_plan(plan_id).expect("migrated view"),
        before_view
    );
    drop(store);
    assert_eq!(economic_rows_v19(&path), before_rows);
    let connection = rusqlite::Connection::open(&path).expect("version check");
    let version: i64 = connection
        .query_row("PRAGMA user_version", [], |row| row.get(0))
        .expect("V4 version");
    assert_eq!(version, 4);
    drop(connection);
    // A second open follows the strict V4 path, with the original lease intact.
    let mut store = open_store(&path);
    let mut authority = ChildAuthority {
        dispatch: [
            DispatchMode::Externalized(30),
    ...
    store
        .drive_one(lease, &mut authority, 1_003)
        .expect("existing fence still drives");
}

#[test]
fn v19_all_sixteen_ordered_family_pairs_persist_with_dom_in_each_leg() {
    let families = [
    ...
    for upstream in families {
        for downstream in families {
            let (_root, path) = state_path();
            let mut store = create_store(&path);
            let mut retained = Vec::new();
            for (index, (leg, family)) in [
                (SettlementLegV1::Upstream, upstream),
                (SettlementLegV1::Downstream, downstream),
            ]
            .into_iter()
            .enumerate()
            {
                let base = plan(SettlementActionV1::Funding, 1, 20 + index as u8);
                let mut bindings = base.bindings().clone();
                bindings.leg = leg;
                bindings.settlement_id = digest(60 + index as u8);
                let mut children = base.materialized_children().expect("children").clone();
                children[0].face = family;
                assert_eq!(children[1].face, SettlementFaceV1::Dom);
                ...
                .expect("independent leg");
                let view = store
                    .install_plan(&mut PlanAuthority::new(), plan, 1_001)
                    .expect("persist selected external family plus DOM");
                retained.push(view);
            }
            drop(store);
            let store = open_store(&path);
            for view in retained {
                assert_eq!(store.load_plan(view.plan_id).expect("reopen leg"), view);
            }
        }
    }
}
