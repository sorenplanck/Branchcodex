...
pub mod economic_graph;
pub mod funding_offer_v22;
pub mod graph_builder;
pub mod graph_contribution_digest_v22;
pub mod graph_key_proofs_v22;
pub mod graph_offer_v22;
pub mod graph_public_material_v23;
pub mod graph_signing_keys_v22;
pub mod payout_offer_v22;
...
impl ValidatedRefundPolicy {
    /// Bind a separately verified role-2 proof to the point admitted by the
    /// concrete refund executor. Laboratory admission has no such point.
    pub fn require_refund_point(&self, point: &[u8; 33]) -> Result<(), RefundPolicyError> {
        if self.refund_point.as_ref() != Some(point) {
            return Err(RefundPolicyError::ScopeMismatch);
        }
        Ok(())
    }
    /// Require the exact settlement and complete frozen economic terms that
    /// were checked at admission. A token from another session cannot arm
    /// this session, even if both sessions use the same adaptor point.
    pub fn require_scope(
        &self,
        settlement_id: &[u8; 32],
        terms_hash: &[u8; 32],
    ) -> Result<(), RefundPolicyError> {
        self.require_pre_funding()?;
        if settlement_id != &self.settlement_id || terms_hash != &self.terms_hash {
            return Err(RefundPolicyError::ScopeMismatch);
        }
        Ok(())
    }
...
pub enum RefundPolicyError {
    /// The admitted policy belongs to another settlement or economic terms.
    #[error("XMR refund policy scope mismatch")]
    ScopeMismatch,
...
pub fn admit_refund_policy(
    ...
    terms.validate()?;
    let settlement_id = terms.settlement_id.0;
    ...
    match mode {
        ...
        XmrRefundModeV1::AdaptorRefundRequired => {
            ...
            Ok(ValidatedRefundPolicy {
                ...
                settlement_id,
                terms_hash,
                refund_point: Some(artifact.adaptor_point_sec1),
            })
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn v11_refund_admission_cannot_be_transplanted_to_other_terms() {
        let original = terms();
        let point = original.adaptor_point_sec1;
        let policy = admit_refund_policy(
            &original,
            XmrNetwork::Stagenet,
            XmrRefundModeV1::AdaptorRefundRequired,
        ...
        policy
            .require_scope(
                &original.settlement_id.0,
                &original.terms_hash().expect("terms"),
            )
            .expect("same scope");
        policy.require_refund_point(&point).expect("same point");
        assert_eq!(
            policy.require_refund_point(&[3; 33]),
            Err(RefundPolicyError::ScopeMismatch)
        );
        for field in 0..4 {
            let mut changed = original.clone();
            match field {
                0 => changed.settlement_id.0[0] ^= 1,
                1 => changed.session_id.0[0] ^= 1,
                2 => changed.counterparty_leg.amount += 1,
                _ => changed.metadata.push(1),
            }
            assert_eq!(
                policy.require_scope(
                    &changed.settlement_id.0,
                    &changed.terms_hash().expect("changed valid terms")
                ),
                Err(RefundPolicyError::ScopeMismatch)
            );
        }
