...
impl RealDomRpcRuntimeV1 {
    ...
    fn f7_claim_finality_from_snapshot_v26(
        ...
        let minimum_confirmations = facts.minimum_confirmations();
        validate_finality_policy(minimum_confirmations, max_reorg_depth)?;
        if expected_tx_hash == [0; 32]
            || expected_template_hash == [0; 32]
            || expected_shared_output_commitment == [0; 33]
            || facts.chain_id() != self.adapter.expected_identity().chain_id
        {
            return Err(RealDomError::InvalidEvidence);
        }
        if snapshot.transaction.tx_hash() != expected_tx_hash
        ...
        // Prove the exact retained pre-signature opens the frozen adaptor point
        // without extracting the scalar. Inclusion and tip are one snapshot.
        let _verified_exposed_claim = super::f7_claim_receiver_v15::verify_f7_observation_v15(
            facts,
            &ProvedClaimObservationEvidenceV1::sealed(
                snapshot.transaction.clone(),
                snapshot.identity.tip_height,
                snapshot.identity.tip_hash,
            ),
        )?;
        // This opening proof gates finality without extracting a scalar.
        // The actuator separately authenticates durable exposure before it
        // persists the returned finality checkpoint.
        let evidence_digest = digest_parts(
            CLAIM_FINALITY_DOMAIN,
            &[
                &expected_tx_hash,
...
/// Public pins for extracting a refund witness from the retained transaction.
/// All pins must come from the admitted settlement and authenticated round.
pub struct DomRefundExtractionRequestV10<'a> {
    /// Chain authenticated by the configured DOM genesis authority.
    pub trusted_chain_id: TrustedChainIdV1,
    /// Exact Contracts session, not the other leg of a composed route.
    pub session_id: [u8; 32],
    /// Exact observed transaction location; no caller-supplied signature.
    pub evidence: &'a EvidenceRefV1,
    /// Public verifier rebuilt from the retained RefundAdaptor round.
    pub pre_signature: &'a dom_scriptless_crypto::VerifiedRefundPreSignatureV1,
    /// Role-2 DLEQ point authenticated by the XMR settlement.
    pub expected_refund_point: [u8; 33],
    /// Kernel position committed by the refund template.
    pub kernel_index: u32,
    /// Admitted confirmation policy.
    pub minimum_confirmations: u32,
    /// Admitted recovery policy.
    pub max_reorg_depth: u32,
}

/// Read-only proof that the refund committed by one retained Contracts session
/// is on the walked canonical chain. The local process need not have been its
/// broadcaster. This proof grants neither a Store transition nor signing power.
pub struct VerifiedDomRefundRevealFinalityV10 {
    session_id: [u8; 32],
    checkpoint: TerminalCheckpointV1,
    checkpoint_bytes: Vec<u8>,
}
finality_accessors!(VerifiedDomRefundRevealFinalityV10);
impl VerifiedDomRefundRevealFinalityV10 {
    /// Exact retained Contracts session that committed this refund.
    pub const fn session_id(&self) -> [u8; 32] {
        self.session_id
    }
}

/// Refund witness extracted from a canonical, final, Store-authenticated DOM
/// refund. No raw constructor, deserializer, clone or secret formatter exists.
pub struct VerifiedDomRefundSecretV10 {
    scalar: zeroize::Zeroizing<[u8; 32]>,
    finality: VerifiedDomRefundRevealFinalityV10,
    chain_id: [u8; 32],
    template_hash: [u8; 32],
    refund_point: [u8; 33],
}

impl VerifiedDomRefundSecretV10 {
    /// Exact Contracts session that owns the published refund.
    pub fn session_id(&self) -> [u8; 32] {
        self.finality.session_id()
    }
    /// Canonical DOM network identity.
    pub const fn chain_id(&self) -> [u8; 32] {
        self.chain_id
    }
    /// Exact template proved before signature extraction.
    pub const fn template_hash(&self) -> [u8; 32] {
        self.template_hash
    }
    /// Refund witness point U.
    pub const fn refund_point(&self) -> [u8; 33] {
        self.refund_point
    }
    /// Public finality evidence, including the exact refund transaction.
    pub const fn finality(&self) -> &VerifiedDomRefundRevealFinalityV10 {
        &self.finality
    }
    /// Closure-only access for cross-curve conversion. The buffer wipes on drop.
...
impl RealDomRpcRuntimeV1 {
    /// Extract the refund witness only after the actual scanner and the same
    /// Contracts Store authenticate the exact persisted transaction and depth.
    /// A missing transaction remains a scanner error; an unrelated valid refund
    /// or signature is permanently rejected before any scalar is returned.
    pub fn verified_refund_adaptor_secret_v10(
        &self,
        store: &ContractsSessionStoreV1,
        request: DomRefundExtractionRequestV10<'_>,
    ) -> Result<VerifiedDomRefundSecretV10, RealDomError> {
        let pre = request.pre_signature;
        if pre.chain_id() != &self.adapter.expected_identity().chain_id
            || request.trusted_chain_id.as_bytes() != pre.chain_id()
            || pre.session_id() != &request.session_id
            || pre.refund_adaptor_point() != request.expected_refund_point
        {
            return Err(RealDomError::InvalidEvidence);
        }
        let facts =
            store.real_dom_refund_facts_v11(request.trusted_chain_id, request.session_id)?;
        // Observation is distinct from the local broadcast state machine:
        // requiring RefundBroadcast here would exclude a peer's valid refund.
        // The Store authenticates the committed funding ancestry and exact
        // refund hash without requiring the later post-anchor claim issuance.
        // The scanner authenticates the actual publication.
        validate_finality_policy(request.minimum_confirmations, request.max_reorg_depth)?;
        let snapshot = self.canonical_terminal_snapshot(request.evidence)?;
        let canonical = &snapshot.transaction;
        if facts.chain_id() != pre.chain_id()
            || canonical.template_hash()? != *pre.template_hash()
            || !canonical.spends_commitment(facts.shared_output_commitment())
            || canonical.tx_hash() != *facts.refund_tx_hash()
        {
            return Err(RealDomError::InvalidEvidence);
        }
        let evidence_digest = digest_parts(
            b"DOM-INTEROP/DOM-REFUND-REVELATION/V10\0",
            &[
                &request.session_id,
                facts.chain_id(),
                ...
                pre.transcript_hash(),
                &pre.to_bytes(),
                &request.kernel_index.to_be_bytes(),
                &snapshot.identity.tip_hash,
                &snapshot.identity.tip_height.to_be_bytes(),
            ],
        );
        let (checkpoint, checkpoint_bytes) = self.terminal_checkpoint(
            ...
            request.max_reorg_depth,
            evidence_digest,
        )?;
        let finality = VerifiedDomRefundRevealFinalityV10 {
            session_id: request.session_id,
            checkpoint,
            checkpoint_bytes,
        };
        let index =
            usize::try_from(request.kernel_index).map_err(|_| RealDomError::BoundsExceeded)?;
        let signature = canonical.kernel_signature(index)?;
        let scalar = pre
            .extract(signature)
            .map_err(|_| RealDomError::InvalidEvidence)?;
        Ok(VerifiedDomRefundSecretV10 {
            scalar,
            finality,
