...
pub use xmr_recovery_execution_v12::{
    DomXmrRecoveryProgressV12, VerifiedDomXmrFundingPrerequisiteV12,
};
...
pub use terminal_finality::{
    DomRefundExtractionRequestV10, VerifiedDomClaimFinalityV1, VerifiedDomClaimReorgV1,
    VerifiedDomFundingFinalityV1, VerifiedDomFundingReorgV1, VerifiedDomRefundFinalityV1,
    VerifiedDomRefundReorgV1, VerifiedDomRefundRevealFinalityV10, VerifiedDomRefundSecretV10,
};
...
impl RealDomRpcRuntimeV1 {
    ...
    /// Verify the selected EVM/Solana anchors against this retained DOM client.
    /// The external evidence is a closed native verifier result, not a supplied
    /// receipt or a claim that a caller selected transaction has been observed.
    pub fn verify_f7_anchor_authority_v12(
        &self,
    ...
    > {
        f7_anchor_authority::families_v11::verify_f7_anchor_authorization_v12(
            &self.adapter,
            role,
    ...
    /// Verify exact DOM/XMR funding, view-only output ownership and the native
    /// recovery graph through this retained scanner. No spend share is sent.
    pub async fn verify_f7_xmr_anchor_authority_v12(
        &self,
    ...
    /// Native bounded XMR anchors use the actual graph, never synthetic M.8.
    pub fn verify_f7_xmr_bounded_anchor_authority_v23(
        &self,
    ...
    pub fn submit_persisted_f7_funding_v20(
        ...
        let receipt = self
            .adapter
            .submit_canonical_transaction(prepared.canonical_bytes())
            .map_err(RealDomError::Chain)?;
        if receipt.tx_hash() != prepared.tx_hash() || !receipt.is_economically_admitted() {
            return Err(RealDomError::InvalidEvidence);
        }
    ...
    /// Submit only a durably exposed universal F7 claim on its bound DOM chain.
    pub fn submit_persisted_f7_final_claim_v14(
        &self,
        prepared: &dom_scriptless_store::PreparedF7FinalClaimSubmissionV14,
    ) -> Result<SubmissionReceiptV1, RealDomError> {
        if prepared.chain_id() != self.adapter.expected_identity().chain_id {
            return Err(RealDomError::InvalidEvidence);
        }
