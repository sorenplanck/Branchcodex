...
impl EncryptedSqliteSecretStore {
    ...
    /// Opens an existing recovery store without creating a file or schema.
    /// Missing local material must never be replaced by a newly empty wallet.
    /// The caller retains responsibility for an owner-only parent directory.
    pub fn open_existing(
        path: impl AsRef<Path>,
        master_key: SecretStoreMasterKey,
    ) -> Result<Self, SecretStoreError> {
        let metadata =
            std::fs::symlink_metadata(path.as_ref()).map_err(|_| SecretStoreError::Unavailable)?;
        ...
        .map_err(|_| SecretStoreError::Unavailable)?;
        // Preparing the exact projection validates the retained schema; no DDL
        // or INSERT runs on recovery. AEAD authenticates the selected row.
        connection.prepare("SELECT settlement_id,terms_hash,nonce,ciphertext FROM xmr_secret_material_v2 LIMIT 0")
...
mod tests {
    ...
    #[test]
    fn recovery_never_creates_a_missing_store_or_schema() {
        let directory = tempfile::tempdir().expect("tempdir");
        let missing = directory.path().join("missing.sqlite");
        assert!(EncryptedSqliteSecretStore::open_existing(
            &missing,
            SecretStoreMasterKey::new([0x44; 32]).expect("key")
        )
        .is_err());
        assert!(!missing.exists());
        let empty = directory.path().join("unrelated.sqlite");
        Connection::open(&empty).expect("empty sqlite");
        assert!(EncryptedSqliteSecretStore::open_existing(
            &empty,
            SecretStoreMasterKey::new([0x44; 32]).expect("key")
        )
        .is_err());
        let count: u32 = Connection::open(&empty)
            .expect("open")
            .query_row(
    ...
    #[test]
    fn recovery_authenticates_the_same_row_and_refuses_a_different_master_key() {
        let directory = tempfile::tempdir().expect("tempdir");
        ...
        secrets
            ...
            .expect("insert");
        drop(secrets);
        let path = directory.path().join("secrets.sqlite");
        ...
        drop(reopened);
        let wrong = EncryptedSqliteSecretStore::open_existing(
            &path,
            SecretStoreMasterKey::new([0x45; 32]).expect("key"),
        )
        .expect("schema still valid");
        assert!(matches!(
            wrong.load(&SETTLEMENT, &TERMS),
    ...
    #[test]
    fn identical_insert_is_idempotent_for_restart_preparation() {
        let directory = tempfile::tempdir().expect("tempdir");
        let secrets = store(&directory, [0x44; 32]);
        ...
        secrets
            .insert(SETTLEMENT, TERMS, &material, &mut rand::thread_rng())
            .expect("same authenticated row resumes");
        secrets
            .load(&SETTLEMENT, &TERMS)
            .expect("retained row still authenticates");
    }
