...
// No copying or formatting: the C keypair contains secret scalar material.
struct SecretKeypair(secp256k1_keypair);

impl SecretKeypair {
    fn empty() -> Self {
        Self(secp256k1_keypair { data: [0; 96] })
    }
}
...
impl SecpContext {
    ...
    pub fn xonly_public_key(&self, secret32: &[u8; 32]) -> Result<[u8; 32], BtcCryptoError> {
        let mut keypair = SecretKeypair::empty();
        let mut secret = *secret32;
        // SAFETY: ctx valid; keypair is owned here; secret is exactly 32 bytes.
        let created =
            unsafe { secp256k1_keypair_create(self.as_ptr(), &mut keypair.0, secret.as_ptr()) };
        secret.zeroize();
        if created != 1 {
            keypair.0.data.zeroize();
        ...
        let got_pub = unsafe { secp256k1_keypair_pub(self.as_ptr(), &mut pubkey, &keypair.0) };
        keypair.0.data.zeroize();
        if got_pub != 1 {
            return Err(BtcCryptoError::NonCanonicalScalar);
        }
    ...
    pub fn partial_sign(
        ...
        let ok = unsafe {
            secp256k1_keypair_create(self.as_ptr(), &mut keypair.0, signer_secret.as_ptr())
        };
        if ok != 1 {
            return Err(BtcCryptoError::NonCanonicalScalar);
        }
