...
/// Legacy V1 only. V2 deployments refuse destructive close and retain rent.
...
/// V2 deployment only: sweep surplus to the frozen funder, retaining state/rent.
/// No arbitrary amount or destination owner is accepted.
pub fn sweep_terminal_excess_v2(
    setup: &ValidatedSolanaSetup,
    funder_token_account: Option<SolanaPubkey>,
) -> Result<SolanaInstruction, ClientError> {
    let accounts = match setup.asset() {
        SolanaAssetV1::NativeSol => {
            if funder_token_account.is_some() {
                return Err(ClientError::UnexpectedAccount);
            }
            vec![
                rw(setup.state_pda()),
        ...
        SolanaAssetV1::LegacySpl { mint, .. } => vec![
            rw(setup.state_pda()),
            ro(setup.vault_authority()),
            rw(setup.vault_pda()),
            ro(setup.funder()),
            rw(funder_token_account.ok_or(ClientError::MissingAccount)?),
            ro(mint),
            ro(LEGACY_TOKEN_PROGRAM_ID),
        ],
    };
    Ok(SolanaInstruction {
        program_id: setup.program_id(),
        accounts,
        data: solana_escrow_wire::EscrowMaintenanceInstructionV2::SweepTerminalExcess.encode(),
    })
}
