...
use solana_kaystra_source::{
    SolanaFeedChangesV2, SolanaSlotAnchor, VerifiedFeedError, VerifiedSolanaEvent,
    VerifiedSolanaEventKind, VerifiedSolanaFeed,
};
...
impl SqliteVerifiedSolanaFeed {
    ...
    pub fn open(
        ...
        let mut connection =
            Connection::open(path).map_err(|_| ObservationStoreError::Unavailable)?;
        connection
            .execute_batch(
                "PRAGMA journal_mode=WAL;
                 PRAGMA synchronous=FULL;
                 CREATE TABLE IF NOT EXISTS solana_feed_binding_v2(
                 ...
                 CREATE TABLE IF NOT EXISTS solana_feed_changes_v2(
                   revision INTEGER PRIMARY KEY AUTOINCREMENT,
                   chain_id BLOB NOT NULL CHECK(length(chain_id)=32),
                   slot INTEGER NOT NULL CHECK(slot>=0),
                   old_anchor BLOB CHECK(old_anchor IS NULL OR length(old_anchor)=32)
                 );
        ...
        // Persist the one-settlement identity, including when migrating a V1 DB.
        // A second connection cannot reinterpret this database under new terms.
        let transaction = connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(|_| ObservationStoreError::Unavailable)?;
        let mismatched: i64 = transaction
            .query_row(
                "SELECT (SELECT COUNT(*) FROM solana_verified_events_v1
              WHERE chain_id!=?1 OR settlement_id!=?2 OR terms_hash!=?3)
              + (SELECT COUNT(*) FROM solana_canonical_slots_v1 WHERE chain_id!=?1)
              + (SELECT COUNT(*) FROM solana_scan_tip_v1 WHERE chain_id!=?1)",
                params![
                    chain_id.0.as_slice(),
        ...
        let ambiguous: i64 = transaction
            .query_row(
                "SELECT COUNT(*) FROM (SELECT 1 FROM solana_verified_events_v1
             GROUP BY settlement_id,tx_id,instruction_index HAVING COUNT(*)>1)",
                [],
                |row| row.get(0),
            )
            .map_err(|_| ObservationStoreError::Unavailable)?;
        ...
        transaction
            .execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS solana_event_identity_v2
             ON solana_verified_events_v1(settlement_id,tx_id,instruction_index)",
                [],
            )
            .map_err(|_| ObservationStoreError::Unavailable)?;
        let inserted = transaction.execute(
            "INSERT OR IGNORE INTO solana_feed_binding_v2(singleton,chain_id,settlement_id,terms_hash)
             VALUES(1,?1,?2,?3)",
            params![chain_id.0.as_slice(), settlement_id.as_slice(), terms_hash.as_slice()],
        ).map_err(|_| ObservationStoreError::Unavailable)?;
        let binding: (Vec<u8>, Vec<u8>, Vec<u8>) = transaction.query_row(
            "SELECT chain_id,settlement_id,terms_hash FROM solana_feed_binding_v2 WHERE singleton=1",
            [], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        ).map_err(|_| ObservationStoreError::Unavailable)?;
        if binding.0.as_slice() != chain_id.0
            || binding.1.as_slice() != settlement_id
            || binding.2.as_slice() != terms_hash
        {
            return Err(ObservationStoreError::Conflict);
        }
        if inserted != 0 {
            transaction
                .execute(
                    "INSERT INTO solana_feed_changes_v2(chain_id,slot)
                 SELECT ?1,MIN(slot) FROM solana_canonical_slots_v1 WHERE chain_id=?1
                 HAVING COUNT(*)>0",
                    params![chain_id.0.as_slice()],
                )
                .map_err(|_| ObservationStoreError::Unavailable)?;
        }
        transaction
    ...
    pub fn replace_canonical_suffix(
        ...
        let transaction = connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(|_| ObservationStoreError::Unavailable)?;
        let old_anchor: Option<Vec<u8>> = transaction
            .query_row(
                "SELECT blockhash FROM solana_canonical_slots_v1 WHERE chain_id=?1 AND slot>=?2
    ...
    /// Insert an already-anchored event, atomically checking canonical identity.
    pub fn insert_event(&self, event: &VerifiedSolanaEvent) -> Result<(), ObservationStoreError> {
        ...
        let transaction = connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(|_| ObservationStoreError::Unavailable)?;
        if self.insert_event_in(&transaction, event)? {
            record_change(
                &transaction,
                self.chain_id,
                event.evidence.block_height,
                None,
            )?;
        }
        transaction
    ...
    /// Append one verified observation without deleting history or lowering tip.
    /// Anchor, event and change watermark commit together; a conflict rolls back all three.
    pub fn persist_observation(
        &self,
        event: &VerifiedSolanaEvent,
    ) -> Result<(), ObservationStoreError> {
        self.validate_event_scope(event)?;
        let slot = to_i64(event.evidence.block_height)?;
        let mut connection = self
            .connection
            .lock()
            .map_err(|_| ObservationStoreError::Unavailable)?;
        let transaction = connection
            ...
            .map_err(|_| ObservationStoreError::Unavailable)?;
        transaction.execute(
            "INSERT OR IGNORE INTO solana_canonical_slots_v1(chain_id,slot,blockhash) VALUES(?1,?2,?3)",
            params![self.chain_id.0.as_slice(), slot, event.evidence.block_anchor.as_slice()],
        ).map_err(|_| ObservationStoreError::Unavailable)?;
        if self.insert_event_in(&transaction, event)? {
            record_change(
                &transaction,
        ...
        transaction.execute(
            "INSERT INTO solana_scan_tip_v1(chain_id,finalized_slot) VALUES(?1,?2)
             ON CONFLICT(chain_id) DO UPDATE SET finalized_slot=MAX(finalized_slot,excluded.finalized_slot)",
            params![self.chain_id.0.as_slice(), slot],
        ).map_err(|_| ObservationStoreError::Unavailable)?;
        transaction
            .commit()
            .map_err(|_| ObservationStoreError::Unavailable)
    ...
    fn insert_event_in(
        ...
        let existing: Option<(Vec<u8>, Vec<u8>, i64, u8)> = connection
            .query_row(
                "SELECT terms_hash,blockhash,slot,kind FROM solana_verified_events_v1
                 WHERE settlement_id=?1 AND tx_id=?2 AND instruction_index=?3",
                params![
            ...
            .optional()
            .map_err(|_| ObservationStoreError::Unavailable)?;
        if let Some((terms, blockhash, slot, kind)) = existing {
            if kind != event.kind as u8
                || terms.as_slice() != self.terms_hash
...
impl VerifiedSolanaFeed for SqliteVerifiedSolanaFeed {
    ...
    fn changes_since(&self, revision: u64) -> Result<SolanaFeedChangesV2, VerifiedFeedError> {
        ...
        // One SQL statement gives the revision and earliest invalidation from
        // the same snapshot even when another process is the observer writer.
        let (latest, first, reorg_slot, old_anchor): (
            i64,
            Option<i64>,
            Option<i64>,
            Option<Vec<u8>>,
            ...
            .query_row(
                "SELECT COALESCE(MAX(revision),0),MIN(CASE WHEN revision>?2 THEN slot END),
             (SELECT slot FROM solana_feed_changes_v2 WHERE chain_id=?1 AND revision>?2
               AND old_anchor IS NOT NULL ORDER BY slot,revision LIMIT 1),
             (SELECT old_anchor FROM solana_feed_changes_v2 WHERE chain_id=?1 AND revision>?2
               AND old_anchor IS NOT NULL ORDER BY slot,revision LIMIT 1)
             FROM solana_feed_changes_v2 WHERE chain_id=?1",
                params![self.chain_id.0.as_slice(), to_i64_feed(revision)?],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
            )
            .map_err(|_| VerifiedFeedError::Unavailable)?;
        let latest = from_i64(latest)?;
        if revision > latest {
            return Err(VerifiedFeedError::InvalidEvidence);
        }
        let first_reorg = match (reorg_slot, old_anchor) {
            (Some(slot), Some(blockhash)) => Some(SolanaSlotAnchor {
                slot: from_i64(slot)?,
                blockhash: vec32(blockhash)?,
            }),
            (None, None) => None,
            _ => return Err(VerifiedFeedError::InvalidEvidence),
        };
        Ok(SolanaFeedChangesV2 {
            revision: latest,
            earliest_slot: first.map(from_i64).transpose()?,
            first_reorg,
        })
    }
...
fn record_change(
    ...
    chain_id: ChainId,
    slot: u64,
    old_anchor: Option<&[u8]>,
) -> Result<(), ObservationStoreError> {
    connection
        .execute(
            "INSERT INTO solana_feed_changes_v2(chain_id,slot,old_anchor) VALUES(?1,?2,?3)",
            params![chain_id.0.as_slice(), to_i64(slot)?, old_anchor],
        )
        .map_err(|_| ObservationStoreError::Unavailable)?;
    Ok(())
}
