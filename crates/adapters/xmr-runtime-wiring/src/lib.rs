...
pub enum RuntimeWiringError {
    /// The admitted policy does not bind the selected settlement and terms.
    #[error("XMR refund policy binding: {0}")]
    RefundBinding(#[from] RefundPolicyError),
...
pub fn attach_xmr_consumer(
    ...
    // Initialization owns creation. A running/recovering consumer must find
    // the exact encrypted share row before creating delivery state or clients.
    // A missing database is lost recovery material, never an empty new wallet.
    ...
    let retained = secrets.load(&setup.settlement_id(), &setup.terms_hash())?;
    drop(retained);
