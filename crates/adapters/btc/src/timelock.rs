...
pub struct AnchoredCrossChainWindowV1 {
    ...
    /// Exact Bitcoin funding transaction whose anchor passed M.8.
    bitcoin_funding_txid: [u8; 32],
    /// Network selected by the validated Bitcoin finality policy.
    bitcoin_network: BitcoinNetworkV1,
}
...
pub enum TimelockError {
    ...
    TermsHashMismatch,
    /// The claim does not spend the Bitcoin contract described by M.8.
    #[error("M.8 Bitcoin claim binding mismatch")]
...
impl AnchoredCrossChainWindowV1 {
    /// Checks that a claim uses the funding, network and native CSV delay
    /// whose cross-chain timing was validated. This must also run on replay:
    /// a cached nonce or partial is not a substitute for timing authorization.
    /// Funding inclusion/finality is still the caller's evidence boundary.
    pub fn validate_bitcoin_claim(
        &self,
        terms_hash: [u8; 32],
        network: BitcoinNetworkV1,
        funding_txid: [u8; 32],
        refund_delay: BitcoinCsvDelayV1,
    ) -> Result<(), TimelockError> {
        if terms_hash != self.settlement_terms_hash {
            return Err(TimelockError::TermsHashMismatch);
        }
        let delay = match (self.window.first_refund, self.window.second_refund) {
            (TimelockSpecV1::BtcBlocks { delta_blocks, .. }, TimelockSpecV1::DomHeight { .. })
            | (TimelockSpecV1::DomHeight { .. }, TimelockSpecV1::BtcBlocks { delta_blocks, .. }) => {
                BitcoinCsvDelayV1::Blocks(delta_blocks)
            }
            (TimelockSpecV1::BtcTime512s { units, .. }, TimelockSpecV1::DomHeight { .. })
            | (TimelockSpecV1::DomHeight { .. }, TimelockSpecV1::BtcTime512s { units, .. }) => {
                BitcoinCsvDelayV1::Time512s(units)
            }
            _ => return Err(TimelockError::InvalidCrossChainTopology),
        };
        if network != self.bitcoin_network
            || funding_txid != self.bitcoin_funding_txid
            || refund_delay != delay
        {
            return Err(TimelockError::ClaimBindingMismatch);
        }
        Ok(())
    }
...
pub fn bind_and_validate_funding_anchors(
    ...
    Ok(AnchoredCrossChainWindowV1 {
        ...
        bitcoin_funding_txid: anchors.bitcoin.funding_txid,
        bitcoin_network: policy.bitcoin_finality.network,
    })
}
