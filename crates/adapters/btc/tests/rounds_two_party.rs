...
fn two_party_claim_produces_valid_pre_signature() {
    ...
    let mut received_partial = p2;
    r1.verify_counterparty_partial(&received_partial).unwrap();
    received_partial[0] ^= 0xff;
    assert_ne!(
        received_partial, p2,
        "caller buffer changed after verification"
    );
    r2.verify_counterparty_partial(&p1).unwrap();

    // Aggregate the pre-signature (backend also proves it is NOT final).
    let pre1 = r1.aggregate_pre_signature().unwrap();
    let pre2 = r2.aggregate_pre_signature().unwrap();
...
fn tampered_counterparty_partial_blocks_pre_signature() {
    ...
    assert!(r1.aggregate_pre_signature().is_err());
