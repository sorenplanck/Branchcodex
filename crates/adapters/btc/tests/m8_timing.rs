...
#[test]
fn claim_binding_checks_network_funding_terms_and_native_csv_in_both_orders() {
    for bitcoin_first in [true, false] {
        for delay in [
            BitcoinCsvDelayV1::Blocks(12),
            BitcoinCsvDelayV1::Time512s(12),
        ] {
            let bitcoin = match delay {
                BitcoinCsvDelayV1::Blocks(delta_blocks) => {
                    TimelockOffsetV1::BtcBlocks { delta_blocks }
                }
                BitcoinCsvDelayV1::Time512s(units) => TimelockOffsetV1::BtcTime512s { units },
            };
            let mut candidate = policy();
            // Use explicit bounds with no MTP uncertainty for a Bitcoin-second window.
            candidate.dom_bounds = ChainTimingBoundsV1 {
                min_block_seconds: 1,
                max_block_seconds: 1,
                max_reorg_seconds: 0,
            ...
            candidate.btc_bounds = candidate.dom_bounds;
            candidate.safety_margin_seconds = 1;
            if bitcoin_first {
                candidate.first_refund = bitcoin;
                candidate.second_refund = TimelockOffsetV1::DomBlocks {
                    delta_blocks: 10_000,
                };
            } else {
                candidate.first_refund = TimelockOffsetV1::DomBlocks { delta_blocks: 1 };
                candidate.second_refund = bitcoin;
            }
            let anchored = anchors(&candidate);
            let auth = bind_and_validate_funding_anchors(&candidate, &anchored).unwrap();
            let check =
                |terms, network, txid, csv| auth.validate_bitcoin_claim(terms, network, txid, csv);
            assert_eq!(
                check(
                    candidate.settlement_terms_hash,
                    BitcoinNetworkV1::Regtest,
                    anchored.bitcoin.funding_txid,
                    delay
                ),
                Ok(())
            );
            ...
            assert_eq!(
                ...
                Err(TimelockError::ClaimBindingMismatch)
            );
            assert_eq!(
                check(
                    candidate.settlement_terms_hash,
            ...
            let wrong_unit = match delay {
                BitcoinCsvDelayV1::Blocks(n) => BitcoinCsvDelayV1::Time512s(n),
                BitcoinCsvDelayV1::Time512s(n) => BitcoinCsvDelayV1::Blocks(n),
            };
            assert_eq!(
                check(
                    candidate.settlement_terms_hash,
