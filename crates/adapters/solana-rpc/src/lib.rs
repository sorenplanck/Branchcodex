...
impl HttpSolanaRpc {
    /// Read the cluster genesis identity without accepting a caller-provided
    /// `SolanaRpc` implementation as a network identity authority.
    pub fn genesis_hash(&self) -> Result<SolanaHash, RpcError> {
        let result = self.call("getGenesisHash", json!([]))?;
        let encoded = result.as_str().ok_or(RpcError::InvalidResponse)?;
        SolanaHash::from_base58(encoded).map_err(|_| RpcError::InvalidResponse)
    }
...
impl SolanaRpc for HttpSolanaRpc {
    ...
    fn get_transaction(
        ...
        let returned_signature = exact_transaction_signature(&result, signature)?;
        let slot = result
            .get("slot")
            .and_then(Value::as_u64)
            .ok_or(RpcError::InvalidResponse)?;
...
// Never label returned transaction contents with the requested signature.
// A non-null transaction must carry its actual primary signature; a different
// answer is invalid evidence, while a null result remains legitimate absence.
fn exact_transaction_signature(
    result: &Value,
    requested: SolanaSignature,
) -> Result<SolanaSignature, RpcError> {
    let signatures = result
        .get("transaction")
        .and_then(|transaction| transaction.get("signatures"))
        .and_then(Value::as_array)
        .ok_or(RpcError::InvalidResponse)?;
    if signatures.is_empty() || signatures.len() > MAX_TRANSACTION_ACCOUNTS {
        return Err(RpcError::InvalidResponse);
    }
    let first = signatures
        ...
        .and_then(Value::as_str)
        .ok_or(RpcError::InvalidResponse)?;
    let returned = SolanaSignature::from_base58(first).map_err(|_| RpcError::InvalidResponse)?;
...
#[cfg(test)]
mod identity_tests_v11 {
    use super::*;

    #[test]
    fn returned_primary_signature_must_be_the_one_requested() {
        let requested = SolanaSignature([1; 64]);
        let other = SolanaSignature([2; 64]);
        let exact = json!({"transaction":{"signatures":[requested.to_base58()]}});
        let swapped = json!({"transaction":{"signatures":[other.to_base58()]}});
        assert_eq!(
            exact_transaction_signature(&exact, requested),
            Ok(requested)
        );
        assert_eq!(
            exact_transaction_signature(&swapped, requested),
            Err(RpcError::InvalidResponse)
        );
    }

    #[test]
    fn missing_empty_malformed_and_wrong_length_signatures_are_errors() {
        let requested = SolanaSignature([1; 64]);
        for answer in [
            json!({"transaction":{}}),
            json!({"transaction":{"signatures":[]}}),
            json!({"transaction":{"signatures":[null]}}),
            json!({"transaction":{"signatures":["not-base58!"]}}),
            json!({"transaction":{"signatures":[SolanaHash([1; 32]).to_base58()]}}),
        ] {
            assert_eq!(
                exact_transaction_signature(&answer, requested),
                Err(RpcError::InvalidResponse)
            );
        }
    }
