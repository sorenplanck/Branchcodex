...
/// Additive V2 maintenance instruction. V1 decoders reject this namespace.
/// V2 deployments retain the V1 state layout and reject destructive Close.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EscrowMaintenanceInstructionV2 {
    SweepTerminalExcess,
}

impl EscrowMaintenanceInstructionV2 {
    pub const MAGIC: &'static [u8; 8] = b"DOMSLIX2";
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, WireError> {
        ...
        if bytes[8..10] != 2u16.to_be_bytes() {
            return Err(WireError::InvalidVersion);
        }
        if bytes[10] != 1 {
            return Err(WireError::UnknownTag);
        }
        Ok(Self::SweepTerminalExcess)
    }
}
...
mod maintenance_tests {
    use super::*;
    #[test]
    fn maintenance_namespace_is_canonical_and_separate_from_v1() {
        let ix = EscrowMaintenanceInstructionV2::SweepTerminalExcess;
        let bytes = ix.encode();
        assert_eq!(EscrowMaintenanceInstructionV2::decode(&bytes), Ok(ix));
        assert!(EscrowInstructionV1::decode(&bytes).is_err());
        assert!(
            EscrowMaintenanceInstructionV2::decode(&EscrowInstructionV1::Close.encode()).is_err()
        );
        for index in 0..bytes.len() {
            let mut bad = bytes.clone();
            bad[index] ^= 0xff;
            assert!(EscrowMaintenanceInstructionV2::decode(&bad).is_err());
        }
        let mut extra = bytes;
        extra.push(0);
        assert!(EscrowMaintenanceInstructionV2::decode(&extra).is_err());
    }
}
