...
use crate::signer::{
    aggregate_pre_signature, expose_local_pubnonce, produce_local_partial, require_claim_timing,
...
impl DurableBitcoinActuatorV1 {
    ...
    pub fn reconcile_takeover<R: BitcoinRpcV1, F: FnOnce() -> Result<u64>>(
        ...
        // An exact claim and its participant transcript share one fencing
        // epoch. Re-fence both in this same transaction: otherwise a finalized
        // participant claim can never pass the relational audit after restart.
        if new_scope.action() == BitcoinActionV1::Claim {
            transaction.execute(
                "UPDATE claim_transcripts SET fence_epoch=?1,updated_at_ms=?2
                 WHERE effect_id=?3 AND route_id=?4 AND fence_epoch=?5",
                params![
                    u64_blob(new_scope.fence_epoch()),
                    u64_blob(post_rpc_now_ms),
                    new_scope.effect_id().as_slice(),
                    new_scope.route_id().as_slice(),
                    u64_blob(old_fence)
                ],
            )?;
        }
    ...
    pub fn expose_claim_pubnonce(
        ...
        require_claim_timing(&authorization, session)?;
        self.audit_storage()?;
    ...
    /// Recover public extraction from an already retained exact claim and its
    /// verified durable transcript. No adaptor scalar is requested or restored.
    /// The current signing scope must have been re-fenced together with the
    /// terminal operation by `reconcile_takeover` when ownership changed.
    pub fn recover_claim_extraction_v3(
        &mut self,
        context: BitcoinClaimSigningContextV1<'_>,
    ) -> Result<crate::BitcoinClaimExtractionContextV1> {
        require_claim_timing(&context.authorization, context.session)?;
        self.audit_storage()?;
        require_claim_session(context.scope, context.authority, context.session)?;
        let transaction = self
            .connection
            .transaction_with_behavior(TransactionBehavior::Deferred)?;
        require_clock_not_regressed(&transaction, context.now_ms)?;
        require_scope_lease(
            &transaction,
            &self.owner_digest,
            context.scope,
        ...
        let row: Option<(Vec<u8>, Vec<u8>, Vec<u8>)> = transaction
            .query_row(
                "SELECT c.remote_pubnonce,c.remote_partial,o.raw_transaction
             FROM claim_transcripts c JOIN operations o ON o.effect_id=c.effect_id
             WHERE c.effect_id=?1 AND c.route_id=?2 AND c.fence_epoch=?3
               AND c.session_digest=?4 AND c.participant_id=?5 AND c.authority_digest=?6
               AND c.verified_remote_partial=1 AND o.action=2",
                params![
                    context.scope.effect_id().as_slice(),
                    context.scope.route_id().as_slice(),
                    u64_blob(context.scope.fence_epoch()),
                    context.session.session_digest()?.as_slice(),
                    context.authority.participant_id().as_slice(),
                    context.authority.authority_digest().as_slice()
                ],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
            )
            .optional()?;
        let (nonce, partial, canonical_claim) = row.ok_or(BitcoinActuatorErrorV1::InvalidState)?;
        let nonce = array_66(nonce)?;
        let partial = array_32(partial)?;
        audit_runtime_state(&transaction, &self.owner_digest)?;
        transaction.commit()?;
        self.audit_storage()?;
        let presignature = self.aggregate_claim_pre_signature(context, nonce, partial)?;
        presignature.into_extraction_context(&canonical_claim)
    }

    /// Read-only V3 transcript check before authenticated peer aggregation.
    pub(crate) fn claim_transcript_for_peer_v3(
        &mut self,
        context: &BitcoinClaimSigningContextV1<'_>,
        remote_nonce: [u8; 66],
    ) -> Result<[u8; 32]> {
        ...
        type AuthenticatedTranscriptRowV3 = (
            Vec<u8>,
            Vec<u8>,
            Vec<u8>,
            Option<Vec<u8>>,
        ...
        let row: Option<AuthenticatedTranscriptRowV3> = transaction.query_row(
            "SELECT session_digest,participant_id,authority_digest,remote_pubnonce,transcript_digest,local_partial
             FROM claim_transcripts WHERE effect_id=?1 AND fence_epoch=?2",
            params![context.scope.effect_id().as_slice(), u64_blob(context.scope.fence_epoch())],
            |row| Ok((row.get(0)?,row.get(1)?,row.get(2)?,row.get(3)?,row.get(4)?,row.get(5)?)),
        ).optional()?;
        let (stored_session, participant, authority, nonce, transcript, partial) =
            row.ok_or(BitcoinActuatorErrorV1::InvalidState)?;
        if stored_session.as_slice() != session_digest
        ...
        let result = array_32(transcript.ok_or(BitcoinActuatorErrorV1::InvalidState)?)?;
        audit_runtime_state(&transaction, &self.owner_digest)?;
        transaction.commit()?;
        self.audit_storage()?;
        Ok(result)
...
fn require_claim_session(
    ...
    if scope.action() != BitcoinActionV1::Claim
        ...
        || scope.terms_digest() != session.actuation_terms_digest_v11()
