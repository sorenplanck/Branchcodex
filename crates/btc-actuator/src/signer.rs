...
const CLAIM_SESSION_DOMAIN_V11: &[u8] = b"DOM-INTEROP/BTC-ACTUATOR/CLAIM-SESSION/V11\0";
...
impl BitcoinParticipantClaimAuthorityV1 {
    /// Internal fixed-protocol message signing, never a public signing API.
    pub(crate) fn sign_protocol_message_v3(
        &self,
        session: &BitcoinClaimSessionV1,
        message: [u8; 32],
    ) -> Result<[u8; 64]> {
        require_authority(self, session)?;
        let crypto = SecpContext::new(&fresh_entropy()?);
        // Deterministic BIP340 signatures make a crash/retry reproduce the
        // exact transport bytes. This is separate from MuSig2 nonce custody.
        let (signature, public) = crypto
            .sign_bip340(&self.secret, &message, &[0; 32])
            .map_err(|_| BitcoinActuatorErrorV1::ClaimCryptography)?;
        if public.as_slice() != &self.public_key[1..] {
...
pub struct BitcoinClaimSessionV1 {
    ...
    /// Frozen settlement terms digest used by F7 and the native nonce permit.
    pub terms_digest: [u8; 32],
    /// V11 composed-route terms commitment used by the actuator scope. `None`
    /// preserves the complete legacy V1 session digest and scope semantics.
    /// `Some` selects the V11 digest domain and commits both hashes; it cannot
    /// reopen a retained V1 nonce round under silently changed semantics.
    pub route_terms_digest_v11: Option<[u8; 32]>,
...
impl BitcoinClaimSessionV1 {
    /// Computes the exact signature-independent Bitcoin transaction identity
    /// after rebuilding its authenticated public Taproot template. This is a
    /// public-data calculation and grants no signing authority.
    pub fn expected_claim_txid_v11(&self) -> Result<[u8; 32]> {
        Ok(build_context(self)?
            .transaction
            .compute_txid()
            .to_raw_hash()
            .to_byte_array())
    ...
    /// Terms identity expected by the route actuator. Native M.8 continues
    /// using `terms_digest`, which identifies this one economic settlement.
    pub const fn actuation_terms_digest_v11(&self) -> [u8; 32] {
        match self.route_terms_digest_v11 {
            Some(route) => route,
            None => self.terms_digest,
        }
    }
    ...
    pub fn session_digest(&self) -> Result<[u8; 32]> {
        ...
        if let Some(route) = self.route_terms_digest_v11 {
            if route == [0; 32] {
                return Err(BitcoinActuatorErrorV1::ClaimAuthorityMismatch);
            }
        ...
        digest(
            if self.route_terms_digest_v11.is_some() {
                CLAIM_SESSION_DOMAIN_V11
            } else {
                CLAIM_SESSION_DOMAIN
            },
            &bytes,
        )
    }
...
impl BitcoinPreSignatureV1 {
    /// Finalize the exact claim and preserve its public extraction authority.
    /// The caller must persist both before permitting broadcast. No adaptor
    /// scalar is retained in the returned extraction context.
    pub fn finalize_claim_with_extraction(
        self,
        secret: BitcoinAdaptorSecretV1,
    ) -> Result<(
        ExactBitcoinTransactionV1,
        crate::BitcoinClaimExtractionContextV1,
    )> {
        ...
        transaction.input[0].witness = Witness::from_slice(&[signature]);
        let bytes = bitcoin::consensus::serialize(&transaction);
        let extraction = self.into_extraction_context(&bytes)?;
        let exact = ExactBitcoinTransactionV1::from_consensus_bytes(bytes)?;
        Ok((exact, extraction))
    }
...
/// Validate the exact Bitcoin face before any durable signing transition,
/// including paths returning a cached nonce or partial after restart.
pub(crate) fn require_claim_timing(
    authorization: &AnchoredCrossChainWindowV1,
    session: &BitcoinClaimSessionV1,
) -> Result<()> {
    authorization
        .validate_bitcoin_claim(
