...
pub struct LegSnapshotV1 {
    ...
    /// Separate catastrophic DOM payout for an XMR leg. This is never an XMR
    /// refund transaction and never asserts that either share became public.
    pub dom_compensation_v12: Option<DomCompensationRecordV12>,
}

/// Public, replayable facts of a natively observed catastrophic DOM payout.
/// Encoding these facts does not mint native finality or an acceptance grant.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DomCompensationRecordV12 {
    /// Native DOM chain identity.
    pub dom_chain_id: Digest32,
    /// Exact selected signing session.
    pub session_id: Digest32,
    /// Per-settlement terms committed by the route admission checkpoint.
    pub terms_digest: Digest32,
    /// Exact authenticated recovery graph.
    pub graph_digest: Digest32,
    /// Actual DOM collateral funding transaction.
    pub funding_transaction_id: Digest32,
    /// Actual plain DOM compensation transaction.
    pub transaction_id: Digest32,
    /// Canonical finality evidence for the compensation.
    pub evidence_digest: Digest32,
    /// Signed frozen quote, margin, recipients, fees and deadlines.
    pub policy_hash: Digest32,
    /// Exact encrypted recovery custody identity.
    pub custody_id: Digest32,
    /// XMR funder paid by the compensation output.
    pub recipient: Digest32,
    /// Exact native payout, including the signed positive volatility margin.
    pub payout_noms: u64,
}

impl DomCompensationRecordV12 {
...
impl LegSnapshotV1 {
    ...
    pub fn is_terminal(&self) -> bool {
        self.claim.progress() == ActionProgressV1::Final
            ...
            || self.dom_compensation_v12.is_some()
    }
...
pub enum RouteEventV1 {
    /// Native-only catastrophic compensation. The general Store event method
    /// refuses this event; only the concrete native-observation consumer may
    /// journal it. Public decoding exists solely for authenticated replay.
    DomCompensatedV12 {
        /// Ordered route leg whose XMR funder received the DOM payout.
        leg: LegIdV1,
        /// Distinct public economic outcome, without an XMR-refund assertion.
        compensation: DomCompensationRecordV12,
    },
...
pub(crate) fn validate_event(event: &RouteEventV1) -> Result<(), CodecErrorV1> {
    match event {
        RouteEventV1::DomCompensatedV12 { compensation, .. } => compensation.validate(),
...
fn validate_leg(leg: &LegSnapshotV1) -> Result<(), CodecErrorV1> {
    ...
    if let Some(compensation) = &leg.dom_compensation_v12 {
        compensation.validate()?;
        if leg.funding.progress() == ActionProgressV1::NotPrepared
