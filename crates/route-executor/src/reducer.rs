...
pub fn reduce_route_v1(
    ...
    match event {
        RouteEventV1::DomCompensatedV12 { leg, compensation } => {
            let prior = current.leg(*leg);
            if current.aborted_unfunded
                || current.refunds.is_none()
                || prior.funding.progress() == ActionProgressV1::NotPrepared
                || prior.is_terminal()
            {
                return Err(ReduceErrorV1::InvalidTransition);
            }
            next.leg_mut(*leg).dom_compensation_v12 = Some(compensation.clone());
        ...
        RouteEventV1::SetHealth { target, .. } => {
            if *target == HealthStateV1::Running
                && ((current.health.restricts_to_recovery() && current.has_open_funds())
                    || current.upstream.dom_compensation_v12.is_some()
                    || current.downstream.dom_compensation_v12.is_some())
            {
...
fn require_new_work_allowed(snapshot: &RouteSnapshotV1) -> Result<(), ReduceErrorV1> {
    if snapshot.health.restricts_to_recovery()
        || snapshot.upstream.dom_compensation_v12.is_some()
        || snapshot.downstream.dom_compensation_v12.is_some()
    {
...
fn validate_action_preconditions(
    ...
    if snapshot.leg(leg).dom_compensation_v12.is_some() {
        return Err(ReduceErrorV1::InvalidTransition);
    }
