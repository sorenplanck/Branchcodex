...
use crate::model::{
    ...
    ActionKindV1, ActionStateV1, CoordinationPhaseV1, Digest32, DomCompensationRecordV12,
    EffectDispatchV1, EffectPriorityV1, EffectReferenceV1, ExposureSourceV1, FrozenBindingsV1,
    FrozenRouteAdmissionCheckpointV2, FrozenRouteTimeFactsV2, HealthStateV1, LegIdV1,
    LegSnapshotV1, PublicExposureV1, RefundBindingsV1, RouteEffectV1, RouteEventV1,
    RouteSnapshotV1, RouteTimerV1, SecretVisibilityV1, TimerKindV1, MAX_EFFECT_PAYLOAD_BYTES_V1,
};
...
const COMPENSATED_SNAPSHOT_MAGIC_V12: &[u8; 4] = b"DRS2";
...
impl CanonicalCodecV1 for RouteSnapshotV1 {
    fn encode_canonical(&self) -> Result<Vec<u8>, CodecErrorV1> {
        self.validate()?;
        let has_compensation = self.upstream.dom_compensation_v12.is_some()
            || self.downstream.dom_compensation_v12.is_some();
        let mut writer = Writer::new(if has_compensation {
            COMPENSATED_SNAPSHOT_MAGIC_V12
        } else {
            SNAPSHOT_MAGIC
        });
        ...
        if has_compensation {
            writer.optional(&self.upstream.dom_compensation_v12, encode_compensation_v12)?;
            writer.optional(
                &self.downstream.dom_compensation_v12,
                encode_compensation_v12,
            )?;
        }
        Ok(writer.finish())
    }
    ...
    fn decode_canonical(bytes: &[u8]) -> Result<Self, CodecErrorV1> {
        let has_compensation = bytes.starts_with(COMPENSATED_SNAPSHOT_MAGIC_V12);
        ...
        if has_compensation {
            snapshot.upstream.dom_compensation_v12 = reader.optional(decode_compensation_v12)?;
            snapshot.downstream.dom_compensation_v12 = reader.optional(decode_compensation_v12)?;
            if snapshot.upstream.dom_compensation_v12.is_none()
                && snapshot.downstream.dom_compensation_v12.is_none()
            {
...
impl CanonicalCodecV1 for RouteEventV1 {
    fn encode_canonical(&self) -> Result<Vec<u8>, CodecErrorV1> {
        ...
        match self {
            Self::DomCompensatedV12 { leg, compensation } => {
                writer.u8(15)?;
                encode_leg_id(&mut writer, *leg)?;
                encode_compensation_v12(&mut writer, compensation)?;
            }
    ...
    fn decode_canonical(bytes: &[u8]) -> Result<Self, CodecErrorV1> {
        ...
        let event = match reader.u8()? {
            ...
            15 => Self::DomCompensatedV12 {
                leg: decode_leg_id(&mut reader)?,
                compensation: decode_compensation_v12(&mut reader)?,
            },
...
fn encode_compensation_v12(
    writer: &mut Writer,
    value: &DomCompensationRecordV12,
) -> Result<(), CodecErrorV1> {
    value.validate()?;
    for bytes in [
        &value.dom_chain_id,
    ...
    writer.u64(value.payout_noms)
}

fn decode_compensation_v12(
    reader: &mut Reader<'_>,
) -> Result<DomCompensationRecordV12, CodecErrorV1> {
    let value = DomCompensationRecordV12 {
        dom_chain_id: reader.digest()?,
        session_id: reader.digest()?,
        terms_digest: reader.digest()?,
        graph_digest: reader.digest()?,
        funding_transaction_id: reader.digest()?,
        transaction_id: reader.digest()?,
        evidence_digest: reader.digest()?,
        policy_hash: reader.digest()?,
        custody_id: reader.digest()?,
        recipient: reader.digest()?,
        payout_noms: reader.u64()?,
    };
    value.validate()?;
    Ok(value)
}
