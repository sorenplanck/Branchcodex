...
#[cfg(test)]
mod compensation_journal_v12_tests;
#[cfg(all(target_os = "linux", feature = "native-xmr-compensation"))]
mod xmr_compensation_v12;
...
impl DurableRouteStoreV1 {
    ...
    pub fn apply_event(
        ...
        if matches!(event, RouteEventV1::DomCompensatedV12 { .. }) {
    ...
    // Private to this concrete Store. The optional native XMR consumer is the
    // only productive caller for compensation; the public raw-event path
    // above always refuses that tag, including after decoding journal bytes.
    fn apply_event_from_native_owner_v12(
        &mut self,
        lease: RouteLeaseV1,
        expected_revision: u64,
        event_id: EventIdV1,
        ...
        if let RouteEventV1::DomCompensatedV12 { leg, .. } = event {
            // Cancel only still-pending work owned by this exact leg. Retain
            // its full historical action states and bytes for audit/replay.
            let previous = current.leg(*leg);
            for action in [&previous.funding, &previous.claim, &previous.refund] {
                if let Some(effect) = action.effect() {
                    transaction.execute(
                        "UPDATE route_outbox SET status_tag=?3, completed_at_unix_ms=?4,
...
fn effect_is_dispatchable(snapshot: &RouteSnapshotV1, effect: &RouteEffectV1) -> bool {
    ...
    if snapshot.aborted_unfunded || snapshot.leg(effect.leg).dom_compensation_v12.is_some() {
        return false;
    }
