...
pub fn full_scan_block_wire_weight_v1(block: &ScanBlock) -> usize {
    ...
    let coinbase = block.coinbase.as_ref().map_or(0, |coinbase| {
        coinbase
...
struct FullScanBlockDtoV1 {
    ...
    coinbase: Option<FullScanCoinbaseDtoV1>,
    total_fees_noms: u64,
    protocol_version: u32,
    range_proof_serialization_version: u8,
}
...
fn full_scan_block_dto_v1(block: ScanBlock) -> FullScanBlockDtoV1 {
    FullScanBlockDtoV1 {
        ...
        transactions: block
            ...
            .collect(),
        coinbase: block.coinbase.map(|coinbase| FullScanCoinbaseDtoV1 {
            output_commitment: hex::encode(coinbase.output_commitment),
            explicit_value: coinbase.explicit_value,
            kernel_excess: hex::encode(coinbase.kernel_excess),
            kernel_features: coinbase.kernel_features,
            kernel_excess_signature: hex::encode(coinbase.kernel_excess_signature),
            offset: hex::encode(coinbase.offset),
            output_proof_envelope: hex::encode(coinbase.output_proof_envelope),
        }),
...
mod tests {
    ...
    fn canned_full_scan_v1() -> ChainScanFullV1 {
        ...
        let block = ScanBlock {
            ...
            coinbase: Some(CoinbaseScanMetadata {
