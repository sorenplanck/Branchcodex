...
pub enum FinalClaimSecretSourceV1 {
    ...
    /// A verified DOM claim in the downstream settlement of this exact route.
    /// This is a public-reveal dependency, never authority to read local T.
    VerifiedDownstreamDomClaimV23 = 0x03,
}
...
impl TryFrom<u8> for FinalClaimSecretSourceV1 {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0x03 => Ok(Self::VerifiedDownstreamDomClaimV23),
            _ => Err(FinalClaimBindingError::UnknownTag),
        }
    }
}
...
impl ComposedFinalClaimRolePlanV1 {
    ...
    pub fn bind(
        ...
        downstream_dom_source_v23::validate_cross_leg_source_v23(
            &plan,
            upstream_selection.source_scope(),
            ComposedSettlementLegV1::Upstream,
            upstream_terms,
        )?;
        downstream_dom_source_v23::validate_cross_leg_source_v23(
            &plan,
            downstream_selection.source_scope(),
            ComposedSettlementLegV1::Downstream,
            downstream_terms,
        )?;
        Ok(plan)
...
fn validate_mode_source_roles(
    ...
    match (reveal_mode, secret_source) {
        ...
        (
            ...
            // The original XMR T owner may also be the upstream sender.
            // This source still requires a verified downstream reveal before
            // any upstream signing share; it never grants local-secret access.
        }
...
fn validate_bilateral_role_relation(
    ...
    if secret_source == FinalClaimSecretSourceV1::VerifiedDownstreamDomClaimV23 {
        return if origin == sender || origin == receiver {
            Ok(())
        } else {
            Err(FinalClaimBindingError::InvalidRoleRelation)
        };
    }
    match reveal_mode {
...
fn validate_selection_for_terms(
    ...
    if scope.route_id() != route_id
        ...
        || (selection.secret_source != FinalClaimSecretSourceV1::VerifiedDownstreamDomClaimV23
    ...
    let expected_source_chain = match selection.secret_source {
        ...
        | FinalClaimSecretSourceV1::VerifiedDownstreamDomClaimV23 => terms.dom_leg.chain_id,
