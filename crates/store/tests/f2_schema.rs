...
#[cfg(target_os = "linux")]
#[test]
fn production_consumer_binding_rejects_foreign_and_development_journals() {
    use std::os::unix::fs::PermissionsExt;
    let dir = tempdir().expect("production root");
    std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o700))
        .expect("owner-only root");
    let expected = ProductionStoreBindingV1::new([0x81; 32]).expect("binding");
    let foreign = ProductionStoreBindingV1::new([0x82; 32]).expect("foreign binding");
    let path = dir.path().join("bound.sqlite3");
    let mut journal = Store::create_production(&path, expected).expect("journal");
    journal
        .require_production_binding(expected)
        .expect("exact scope");
    assert!(matches!(
        journal.require_production_binding(foreign),
        Err(StoreError::InvalidStorageAuthority)
    ));
    journal
        .require_empty_production()
        .expect("refusal left journal empty");
    drop(journal);
    let reopened = Store::open_production(&path, expected).expect("reopened");
    reopened
        .require_production_binding(expected)
        .expect("retained exact scope");
    assert!(matches!(
        reopened.require_production_binding(foreign),
        Err(StoreError::InvalidStorageAuthority)
    ));
    let development = Store::open(&dir.path().join("development.sqlite")).expect("development");
    assert!(matches!(
        development.require_production_binding(expected),
        Err(StoreError::InvalidStorageAuthority)
    ));
}
