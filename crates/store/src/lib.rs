...
impl Store {
    ...
    /// Authenticate this live production opening against its consumer's scope.
    /// A development store or a differently bound production journal is refused.
    /// This checks physical custody and persisted metadata without changing rows.
    pub fn require_production_binding(&self, expected: ProductionStoreBindingV1) -> Result<()> {
        if self.production_binding() != Some(expected) {
            return Err(StoreError::InvalidStorageAuthority);
        }
        self.audit_production_authority()
    }
