...
//! - for a Monero chain: the explicit network (`MoneroNetworkV1`), including
//!   Mainnet; deployment admission separately authenticates its genesis;
...
pub enum ChainKindV1 {
    ...
    /// A Monero network, including Mainnet for authenticated interop.
    /// This external-chain identity does not alter DOM or Monero consensus.
    Monero {
        /// The Monero network.
...
///
/// Mainnet is represented explicitly for authenticated production interop.
/// This identity does not change either chain’s consensus rules.
/// The discriminants match `xmr_profile::XmrNetworkId` so the two
/// spellings of "which Monero network" can never drift apart; a test in
/// `xmr-profile` holds them together.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
#[repr(u8)]
pub enum MoneroNetworkV1 {
    /// The public Monero mainnet.
    Mainnet = 0x01,
    /// The public stagenet.
    Stagenet = 0x02,
...
impl MoneroNetworkV1 {
    /// Decode the network discriminant shared with the Monero adapter.
    pub fn from_u8(value: u8) -> Option<Self> {
        match value {
            0x01 => Some(Self::Mainnet),
...
impl ChainProfileV1 {
    ...
    pub fn validate(&self) -> Result<(), ProfileRefusal> {
        ...
        match self.kind {
            ...
            ChainKindV1::Bitcoin { .. } => {}
            // Monero network tags are explicit. Deployment admission, not
            // this timing profile, verifies the ratified genesis identity.
            ChainKindV1::Monero { .. } => {}
