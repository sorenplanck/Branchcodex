...
use chain_profile::{
    ...
    ChainProfileV1, MoneroNetworkV1, ProfileRefusal, SolanaNetworkV1, MAX_ALLOWED_ASSETS,
};
...
#[test]
fn monero_mainnet_is_explicit_and_network_profiles_do_not_alias() {
    let networks = [
    ...
    for (index, network) in networks.into_iter().enumerate() {
        assert_eq!(MoneroNetworkV1::from_u8((index + 1) as u8), Some(network));
        let mut profile = btc_profile();
        profile.kind = ChainKindV1::Monero { network };
        profile
            .validate()
            .expect("explicit Monero network with valid timing");
        digests.push(profile.profile_digest().unwrap());
    }
    for index in 0..digests.len() {
        for other in index + 1..digests.len() {
            assert_ne!(digests[index], digests[other]);
        }
    }
    assert_eq!(MoneroNetworkV1::from_u8(0), None);
    assert_eq!(MoneroNetworkV1::from_u8(4), None);
    assert_eq!(MoneroNetworkV1::from_u8(255), None);
}
