...
#[test]
fn v20_exact_funding_identity_recovers_signed_custody_and_refuses_substitution() {
    let mut fixture = Fixture::new(EVM_NATIVE);
    let (_, call) = fixture.scope(0x10, 0x11, 50);
    assert_eq!(
        fixture
            .store
            .retained_open_transaction_v20(id(0x10), fixture.deployment, &call)
            .unwrap(),
        None
    );
    let (call, prepared) = prepare_native(&mut fixture, 0x10);
    // An actual prepared operation is not yet a signed funding identity.
    assert_eq!(
        fixture
            .store
            .retained_open_transaction_v20(id(0x10), fixture.deployment, &call)
    ...
    let signed = fixture
        ...
        .unwrap()
        .value;
    let expected = signed.transaction_hash.expect("signed identity");
    assert_eq!(
        fixture
            .store
            .retained_open_transaction_v20(id(0x10), fixture.deployment, &call)
    ...
    substituted.binding[0] ^= 1;
    assert!(fixture
        .store
        .retained_open_transaction_v20(id(0x10), fixture.deployment, &substituted)
        .is_err());
    // Reopen reads exact custody without obtaining another signer or issuing
    // an operation. The previous account lease is not a write permission here.
    drop(fixture.store);
    let mut reopened = DurableEvmActuatorV1::open_existing(&fixture.path).unwrap();
    assert_eq!(
        reopened
