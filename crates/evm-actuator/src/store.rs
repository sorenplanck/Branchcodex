...
impl DurableEvmActuatorV1 {
    ...
    fn audit_file_authority(&self) -> Result<()> {
        ...
        return validate_resumable_sidecars(&self.path);
    }
    ...
    /// Read-only, fully audited identity of the unique opening operation for
    /// this route and exact admitted lock. This does not authorize a write or
    /// claim funding inclusion. No caller-supplied transaction hash is accepted.
    /// Ambiguous duplicate operations and corrupt custody are hard refusals.
    pub fn retained_open_transaction_v20(
        &mut self,
        route_id: Digest32,
        deployment: ResolvedEvmDeploymentV1,
        call: &adapter_evm::UnsignedEvmCall,
    ) -> Result<Option<Digest32>> {
        validate_id(route_id)?;
        let lock = crate::transaction::validate_lock_scope(deployment, call)?;
        let transaction = self.deferred()?;
        let ids = {
            let mut query = transaction.prepare(
                "SELECT operation_id FROM evm_operations WHERE route_id=?1 AND lock_id=?2 AND operation_kind=?3 LIMIT 2",
            )?;
            let rows = query.query_map(
                params![
                    route_id.as_slice(),
                    lock.lock_id.as_slice(),
                    EvmOperationKindV1::Open.tag()
                ],
        ...
        // This reader audits the complete retained signed-attempt chain.
        let row = load_operation_row(&transaction, blob32(id)?)?;
        if row.kind != EvmOperationKindV1::Open
            || row.route_id != route_id
            || row.fields.chain_id != call.chain_id
            || row.fields.to != call.to
            || row.fields.value != call.value
            || row.fields.calldata.as_slice() != call.calldata.as_slice()
            || row.registry_digest != deployment.registry_digest()
            || row.profile_digest != deployment.profile_digest()
            || row.asset_digest != deployment.asset_binding_digest()
            || row.lock_id != lock.lock_id
            || row.binding != call.binding
            || row.account != deployment.adapter_config().funder
            || row.secret_exposed
        {
            return Err(EvmActuatorErrorV1::InvalidScope);
        }
