...
impl ContractsTransportIdentityStoreV1 {
    ...
    pub fn create_production(
        ...
        file.sync_all()
            .map_err(|_| IdentityStoreError::Filesystem)?;
        fsync_capability_dir(root).map_err(|_| IdentityStoreError::Filesystem)?;
        ...
        fsync_capability_dir(&parent).map_err(|_| IdentityStoreError::Filesystem)?;
    ...
    fn create(parent: Arc<Dir>) -> Result<Self, IdentityStoreError> {
        ...
        let root = match open_identity_root(parent.as_ref(), &name) {
            ...
            Err(error) => {
                ...
                let _ = fsync_capability_dir(&parent);
                return Err(error);
            }
        };
        let staging = Self {
        ...
        fsync_capability_dir(root).map_err(|_| IdentityStoreError::Filesystem)?;
        fsync_capability_dir(&staging.parent).map_err(|_| IdentityStoreError::Filesystem)?;
...
impl Drop for StagingIdentityRoot {
    fn drop(&mut self) {
        ...
        let _ = fsync_capability_dir(&self.parent);
    }
}
