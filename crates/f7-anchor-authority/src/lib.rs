...
/// Concrete EVM, SOL and XMR funding authentication for the V11 composition.
pub mod families_v11;
...
pub enum F7AnchorAuthorityError {
    ...
    DomFundingMismatch,
    /// A fresh V11 scan legitimately found no occurrence of the requested DOM
    /// funding transaction. Historical V2 error classification is unchanged.
    #[error("exact DOM funding transaction is not yet included")]
    DomFundingAbsent,
...
fn verify_dom_funding_evidence(
    ...
    verify_dom_funding_evidence_inner(
        dom,
...
fn verify_dom_funding_evidence_inner(
    dom: &DomHttpChainAdapterV1,
    expected_funding_txid: [u8; 32],
    expected_shared_output_commitment: [u8; 33],
    expected_funding_template_hash: [u8; 32],
    ...
    verify_dom_funding_evidence_until_v23(
        dom,
        expected_funding_txid,
        expected_shared_output_commitment,
        expected_funding_template_hash,
    ...
    for _ in 0..MAX_F7_DOM_SCAN_PAGES {
        ...
        for block in &page.blocks {
            for transaction in &block.transactions {
                // Historical V2 recovery still reads funding after a terminal
                // spend. A fresh V11 XMR claim gate instead requires C to be
                // unspent throughout this complete authenticated snapshot.
                if require_unspent
        ...
        if page.reached_snapshot_tip {
            let (block_hash, height, block_time_seconds) = found.ok_or(if require_unspent {
                ...
                F7AnchorAuthorityError::DomFundingMismatch
            })?;
            let depth = page
                .identity
                .tip_height
...
fn require_external_scan_deadline_v23(
...
#[cfg(test)]
#[path = "native_scan_deadline_v23_tests.rs"]
mod native_scan_deadline_v23_tests;
