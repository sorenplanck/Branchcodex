...
#[test]
fn permanent_broadcast_rejection_survives_as_error_and_preserves_recoverable_bytes() {
    struct Reject;
    impl ExactBroadcastPort for Reject {
        fn submit_exact(
            &mut self,
            _: [u8; 32],
            _: &[u8],
    ...
    let (directory, actuator) = prepared();
    assert_eq!(
        actuator.broadcast_current(&lease(1), locator(), [0x41; 32], &mut Reject, NOW),
        Err(XmrActuatorErrorV1::BroadcastRejected)
    );
    assert_eq!(
        actuator.view(locator()).expect("retained").stage,
        XmrTxStageV1::SendAttempted
    );
    assert_eq!(actuator.retained(locator()).expect("exact bytes"), raw());
    drop(actuator);
    let reopened = DurableXmrActuatorV1::new(
        XmrOperationStoreV1::open(directory.path().join("xmr-actuator.sqlite")).expect("reopen"),
    );
    assert_eq!(
        reopened.view(locator()).expect("durable attempt").stage,
        XmrTxStageV1::SendAttempted
    );
    assert_eq!(reopened.retained(locator()).expect("durable bytes"), raw());
    let outcome = reopened
        .broadcast_current(
            &lease(1),
            locator(),
            [0x42; 32],
            &mut MockBroadcast::default(),
            NOW,
        )
        .expect("transient result remains ambiguous");
    assert!(!outcome.accepted);
    assert_eq!(outcome.view.stage, XmrTxStageV1::SendAttempted);
}
