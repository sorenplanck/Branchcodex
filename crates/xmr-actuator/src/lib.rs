...
mod finality_observation_v23;
pub mod model;
pub mod store;
mod timed_observation_v23;
...
pub use store::{
    custody_digest_v1, XmrOperationStoreV1, XmrRemoteCustodyDigestsV23, MAX_RAW_TX_BYTES_V1,
};
...
impl DurableXmrActuatorV1 {
    ...
    /// Atomically retain a verified remote sweep's raw bytes and four ancestry commitments.
    #[allow(clippy::too_many_arguments)]
    pub fn prepare_signed_remote_v23(
        ...
        self.store.prepare_signed_remote_v23(
            lease,
            locator,
            tx_hash,
            key_image,
    ...
    /// Require exact remote ancestry and raw custody under the live physical fence.
    pub fn require_remote_custody_v23(
        ...
        self.store
            .require_remote_custody_v23(lease, locator, now_unix_ms)
    }
    ...
    /// Re-read an existing operation under the exact live lease and physical
    /// owner fence. This is read-only and never renews or replaces a lease.
    pub fn checked_view_v23(
        &self,
        lease: &XmrActuatorLeaseV1,
        locator: XmrOperationLocatorV1,
        now_unix_ms: u64,
    ) -> Result<XmrOperationViewV1> {
        self.store.checked_view_v23(lease, locator, now_unix_ms)
    }
    ...
    pub fn broadcast_current(
        ...
        let accepted = match port.submit_exact(view.tx_hash, &raw) {
            Ok(BroadcastAcceptance::Accepted | BroadcastAcceptance::AlreadyKnown) => true,
            Err(SpendPortError::Retryable) => false,
            // Preserve SendAttempted and exact retained bytes: a permanent
            // response error is neither absence nor permission to rebuild.
            Err(SpendPortError::Rejected) => return Err(XmrActuatorErrorV1::BroadcastRejected),
        };
