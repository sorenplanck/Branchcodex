...
#[cfg(test)]
#[path = "model_xmr_bindings_tests.rs"]
mod xmr_bindings_tests;
...
impl DomParticipantSigningShareV1 {
    ...
    /// Public kernel share composed by this participant's authenticated wallet.
    /// This is the purpose-specific excess point, not its shared-output point.
    pub const fn public_key_v12(&self) -> &dom_crypto::PublicKey {
        self.share.public_key()
    }

    /// Prove knowledge of the wallet-composed local excess for an exact native
    /// purpose statement. The profile verifier separately binds the statement's
    /// recovery/context hash to its template, purpose and economic graph; this
    /// method never accepts or exports a signing scalar.
    pub fn prove_native_signing_share_v12(
        &self,
        statement: &dom_adaptor::SharePoPStatementV1,
    ) -> DomActuatorResult<dom_adaptor::ShareProofV1> {
        if statement.chain_id() != self.binding.chain_id()
            || statement.session_id() != self.binding.session_id()
            || statement.terms_hash() != self.binding.terms_digest()
            || statement.participant_id() != self.binding.participant().participant_id()
            || statement.participant_index()
                != u16::from(self.binding.participant().protocol_index())
            || statement.share_point() != *self.share.public_key()
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        dom_adaptor::prove_share_knowledge_v1(statement, &self.share)
...
impl DomSessionBindingV1 {
    /// Private derivation for independently pinned XMR wallet outputs. This
    /// grants no Contracts authority and cannot substitute a caller session.
    pub(crate) fn for_xmr_wallet_payout_v22(
        self,
        policy: &xmr_refund_policy::compensation::ValidatedXmrCompensationPolicyV11,
        kind: crate::DomXmrPayoutKindV22,
    ) -> DomActuatorResult<Self> {
        self.validate()?;
        use crate::DomXmrPayoutKindV22 as Kind;
        let recipient = match kind {
            Kind::ClaimPrincipal | Kind::Compensation => policy.policy().xmr_funder,
            Kind::ClaimChange | Kind::Refund => policy.policy().dom_funder,
        };
        if policy.policy().session_id != self.session_id
            || policy.policy().dom_chain_id != self.chain_id
            || policy.terms_hash() != &self.terms_digest
            || recipient != self.participant.participant_id()
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let mut bytes = b"DOM:wallet-xmr-policy-payout:v22\0".to_vec();
        bytes.extend_from_slice(&self.chain_id);
        bytes.extend_from_slice(&self.session_id);
        bytes.extend_from_slice(&self.terms_digest);
        bytes.push(kind as u8);
        let session_id = *dom_crypto::blake2b_256(&bytes).as_bytes();
        if session_id == self.session_id {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let payout = Self { session_id, ..self };
        payout.validate()?;
        Ok(payout)
    }

    /// Derive the independent shared-output session D under the V12 domain.
    /// All other authority fields remain unchanged. This identifier alone
    /// neither creates a journal nor authenticates an economic recovery graph.
    pub fn for_xmr_cancelled_output_v22(self) -> DomActuatorResult<Self> {
        self.validate()?;
        let session_id = dom_scriptless_crypto::xmr_cancelled_output_session_id_v22(
            &self.chain_id,
            &self.session_id,
            &self.terms_digest,
        );
        ...
        let cancelled = Self { session_id, ..self };
        cancelled.validate()?;
        Ok(cancelled)
    }

    /// Derive an ordinary XMR recovery session from this parent binding.
    ///
    /// All deployment, participant and route fields are preserved. The native
    /// graph identity and exact canonical template hash separate cancellation
    /// from compensation. This public binding grants no signing authority:
    /// the auxiliary Contracts journal, roster and template must still be
    /// authenticated before a retained signer can use it.
    pub fn for_xmr_ordinary_recovery_v22(
        self,
        ...
        self.validate()?;
        if graph.chain_id != self.chain_id
            || graph.session_id != self.session_id
            || graph.terms_hash != self.terms_digest
            || template_hash == [0; 32]
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let session_id =
            dom_scriptless_crypto::xmr_ordinary_recovery_session_v12(graph, kind, template_hash);
        if session_id == self.session_id {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let auxiliary = Self { session_id, ..self };
        auxiliary.validate()?;
        Ok(auxiliary)
    }

    /// Bind the explicit V23 compensation policy to this parent identity.
    /// All other authority fields are preserved; this grants no signing authority.
    pub fn for_xmr_compensation_v23(
        self,
        graph: &dom_scriptless_crypto::XmrRecoveryGraphBindingV11,
        policy: &xmr_refund_policy::compensation::ValidatedXmrCompensationPolicyV11,
        template_hash: Digest32,
        ...
        self.validate()?;
        let local = self.participant.participant_id();
        if graph.chain_id != self.chain_id
            || graph.session_id != self.session_id
            || graph.terms_hash != self.terms_digest
            || (local != policy.policy().dom_funder && local != policy.policy().xmr_funder)
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let session_id = dom_scriptless_crypto::xmr_bounded_compensation_session_v23(
            graph,
            policy,
            template_hash,
        )
