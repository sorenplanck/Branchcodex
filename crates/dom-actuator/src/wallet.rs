...
#[path = "wallet_bootstrap_v16.rs"]
mod bootstrap_v16;
#[path = "wallet_funding_v16.rs"]
mod funding_v16;
#[path = "wallet_xmr_funding_offer_v22.rs"]
mod xmr_funding_offer_v22;
pub use funding_v16::DomBootstrapFundingInputsV16;
#[path = "wallet_templates_v17.rs"]
mod templates_v17;
pub use templates_v17::DomBootstrapSigningSharesV17;
#[path = "wallet_xmr_payouts_v22.rs"]
mod xmr_payouts_v22;
pub use xmr_payouts_v22::DomXmrPayoutKindV22;
#[path = "wallet_xmr_payout_proofs_v22.rs"]
mod xmr_payout_proofs_v22;
pub use xmr_payout_proofs_v22::DomXmrPayoutProofRequestV22;
#[path = "wallet_xmr_graph_shares_v22.rs"]
mod xmr_graph_shares_v22;
pub use xmr_graph_shares_v22::{DomXmrGraphSharesRequestV22, DomXmrGraphSigningSharesV22};
...
pub struct DomParticipantWalletV1 {
    ...
    wallet_file: File,
    process_lock: File,
    // An uncertain encrypted publication must be recovered by reopening the
    // physical wallet, never by continuing with an in-memory-only blinding.
    bootstrap_publication_failed_v16: bool,
}
...
impl DomParticipantWalletV1 {
    ...
    pub fn open_existing(
        ...
        let wallet = Self {
            ...
            wallet_file,
            process_lock,
            bootstrap_publication_failed_v16: false,
        };
    ...
    fn authenticate_payout_face_for_session(
        ...
        let binding = self.require_session(leg)?;
        self.authenticate_payout_face_for_binding_v22(binding, store, lease, request)
    }

    // Only the retained parent session or a policy-validated XMR payout scope
    // may call this internal primitive. No public caller supplies a binding.
    fn authenticate_payout_face_for_binding_v22(
        &mut self,
    ...
    fn prove_payout_blinding_for_session_v12(
        &self,
        leg: DomWalletSessionLegV1,
        store: &mut DomActuatorStoreV1,
        lease: DomLeaseV1,
        payout: &AuthenticatedDomPayoutFaceV1,
        statement: &dom_adaptor::SharePoPStatementV1,
        now_unix_ms: u64,
    ) -> DomActuatorResult<dom_adaptor::ShareProofV1> {
        let binding = self.require_session(leg)?;
        self.audit_physical_authority()?;
        if payout.retained.binding != binding
            || payout.retained.payout_value == 0
            ...
            || statement.session_id() != binding.session_id()
            || statement.terms_hash() != binding.terms_digest()
            || statement.participant_id() != binding.participant().participant_id()
            || statement.participant_index() != u16::from(binding.participant().protocol_index())
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        store.validate_payout_face(lease, &payout.retained, now_unix_ms)?;
        ...
        let share = SigningShareV1::from_be_bytes(*output.blinding)
            .map_err(|_| DomActuatorError::CryptoAuthorityUnavailable)?;
        if statement.share_point() != *share.public_key() {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let proof = dom_adaptor::prove_share_knowledge_v1(statement, &share)
            .map_err(|_| DomActuatorError::CryptoAuthorityUnavailable)?;
        self.audit_physical_authority()?;
        Ok(proof)
    }
    ...
    fn audit_physical_authority(&self) -> DomActuatorResult<()> {
        if self.bootstrap_publication_failed_v16 {
            return Err(DomActuatorError::InvalidStorageAuthority);
        }
        self.authority.validate()?;
...
impl DomParticipantWalletSessionV1<'_> {
    ...
    /// Prove knowledge of the exact wallet-owned payout blinding for a native
    /// value statement. The commitment, amount and economic/session context
    /// are revalidated before proof creation; the blinding never leaves this
    /// encrypted wallet owner. The caller must separately verify the signed
    /// economic policy that selected the statement and payout amount.
    pub fn prove_payout_blinding_v12(
        &self,
        store: &mut DomActuatorStoreV1,
        lease: DomLeaseV1,
        payout: &AuthenticatedDomPayoutFaceV1,
        ...
        self.wallet.prove_payout_blinding_for_session_v12(
            self.leg,
            store,
            lease,
            payout,
...
mod tests {
    ...
    #[test]
    fn bootstrap_v16_creates_both_local_payouts_and_reopens_exact_openings() -> TestResult {
        ...
        let empty = WalletV2State::new(Network::Regtest, upstream.chain_id());
        save_wallet_state(&empty, &wallet_path, "test-password")?;
        ...
        let second = wallet
            ...
            .prepare_unique_payout_face_v16(
                &mut store,
                lease,
                DomPayoutFaceSelectionRequestV1::new(50, 1_002)?,
            )?;
        assert_ne!(first.payout_commitment(), second.payout_commitment());
        assert_eq!(wallet.state.outputs.len(), 2);
        let first_commitment = first.payout_commitment();
        let first_digest = first.evidence_digest();
        let private_blinding = Zeroizing::new(
            *wallet
                .state
                .outputs
                .get(&first_commitment)
                .ok_or(DomActuatorError::WalletUnavailable)?
                .blinding,
        );
        drop(wallet);
        drop(store);
        for path in [&wallet_path, &store_path] {
            let bytes = fs::read(path)?;
            assert!(!bytes
                .windows(32)
                .any(|part| part == private_blinding.as_slice()));
        }
        let mut store = DomActuatorStoreV1::open_existing(&store_path)?;
        let lease = store.acquire_lease(digest(3), digest(20), 1_003, 10_000)?;
        let mut wallet = DomParticipantWalletV1::open_existing(
            &wallet_path,
            Zeroizing::new("test-password".into()),
            authority,
        ...
        let resumed = wallet
            ...
            .prepare_unique_payout_face_v16(
                &mut store,
                lease,
                DomPayoutFaceSelectionRequestV1::new(50, 1_004)?,
            )?;
        assert_eq!(resumed.payout_commitment(), first_commitment);
        assert_eq!(resumed.evidence_digest(), first_digest);
        assert_eq!(wallet.state.outputs.len(), 2);
    ...
    #[test]
    fn bootstrap_v16_recovers_c0_publication_before_payout_preparation() -> TestResult {
        let directory = tempfile::tempdir().test_context("tempdir")?;
        fs::set_permissions(directory.path(), fs::Permissions::from_mode(DIRECTORY_MODE))?;
        let wallet_path = directory.path().join("wallet.v3");
        let store_path = directory.path().join("actuator.sqlite");
        ...
        save_wallet_state(
            &WalletV2State::new(Network::Regtest, upstream.chain_id()),
            &wallet_path,
            "test-password",
        )?;
        fs::set_permissions(&wallet_path, fs::Permissions::from_mode(FILE_MODE))?;
        ...
        wallet.prepare_missing_payout_opening_v16(
        ...
        assert_eq!(wallet.state.outputs.len(), 1);
        Ok(())
    }

    #[test]
    fn bootstrap_v16_expired_lease_and_ambiguous_openings_never_create_another() -> TestResult {
        let directory = tempfile::tempdir().test_context("tempdir")?;
        fs::set_permissions(directory.path(), fs::Permissions::from_mode(DIRECTORY_MODE))?;
        let wallet_path = directory.path().join("wallet.v3");
        let (upstream, _, authority) = bindings()?;
        create_ambiguous_payout_wallet(&wallet_path, upstream)?;
        let mut store = DomActuatorStoreV1::create(&directory.path().join("actuator.sqlite"))?;
        let lease = store.acquire_lease(digest(3), digest(20), 1_000, 10_000)?;
        store.bind_session(lease, upstream, 1_000)?;
        ...
        let expired = wallet
            ...
            .prepare_unique_payout_face_v16(
                &mut store,
                lease,
                DomPayoutFaceSelectionRequestV1::new(51, 20_000)?,
            );
    ...
    fn bootstrap_funding_terms_v16(
        local_funds: bool,
    ) -> TestResult<kaystra_core::SettlementTermsV1> {
        use kaystra_core::types::*;
        let payer = ParticipantId(if local_funds { digest(3) } else { digest(13) });
        let receiver = ParticipantId(if local_funds { digest(13) } else { digest(3) });
        Ok(kaystra_core::SettlementTermsV1 {
            settlement_id: SettlementId(digest(30)),
            session_id: SessionId(digest(2)),
            intent_hash: IntentHash(digest(31)),
            solver_id: SolverId(digest(32)),
            roster: [ParticipantId(digest(3)), ParticipantId(digest(13))],
            dom_leg: LegTermsV1 {
                role: LegRole::Dom,
                chain_id: ChainId(digest(4)),
                asset_id: AssetId(digest(9)),
                amount: 1_000_000,
                ...
                adapter_profile_hash: digest(7),
            },
            counterparty_leg: LegTermsV1 {
                role: LegRole::Counterparty,
                chain_id: ChainId(digest(14)),
                asset_id: AssetId(digest(19)),
                amount: 1_000_000,
                beneficiary: payer,
                refund_to: receiver,
                mechanism: LockMechanism::ConditionLock,
                deadline: TimelockSpec::TimestampSeconds { value: 20_000 },
                finality: FinalityPolicyV1 {
                    min_confirmations: 2,
                    max_reorg_depth: 10,
                },
                adapter_profile_hash: digest(17),
            },
            adaptor_point_sec1: dom_adaptor::SigningShareV1::from_be_bytes(digest(35))?
                .public_key()
                .to_compressed_bytes(),
            fee_limit: FeeLimitV1 {
                dom_max: 1_000_000,
                counterparty_max: 1_000_000,
            },
    ...
    #[test]
    fn bootstrap_v16_reserves_actual_funding_inputs_and_recovers_after_lease_takeover() -> TestResult
    {
        let directory = tempfile::tempdir().test_context("tempdir")?;
        fs::set_permissions(directory.path(), fs::Permissions::from_mode(DIRECTORY_MODE))?;
        let wallet_path = directory.path().join("wallet.v3");
        let store_path = directory.path().join("actuator.sqlite");
        let terms = bootstrap_funding_terms_v16(true)?;
        let (_, downstream, _) = bindings()?;
        let upstream = binding_from(TestBindingPartsV1 {
            route_id: digest(1),
            session_id: digest(2),
            participant_id: digest(3),
            protocol_index: 0,
            terms_digest: terms.terms_hash()?,
            deployment_digest: digest(8),
        })?;
        let authority = DomWalletAuthorityBindingV1::new(upstream, downstream)?;
        let blinding = BlindingFactor::from_bytes(digest(36))?;
        let input_commitment = *Commitment::commit(3_000_000, &blinding).as_bytes();
        let mut state = WalletV2State::new(Network::Regtest, upstream.chain_id());
        state.meta.last_reconciled_tip = 10;
        let mut output = StoredOutput::new_unconfirmed(
            input_commitment,
            3_000_000,
        ...
        save_wallet_state(&state, &wallet_path, "test-password")?;
        fs::set_permissions(&wallet_path, fs::Permissions::from_mode(FILE_MODE))?;
        let mut store = DomActuatorStoreV1::create(&store_path)?;
        let lease = store.acquire_lease(digest(3), digest(20), 1_000, 10_000)?;
        store.bind_session(lease, upstream, 1_000)?;
        ...
        assert_eq!(
            prepared.reservation().outputs()[0].commitment(),
            input_commitment
        );
        let reservation = prepared.reservation().clone();
        let fee = prepared.fee_noms();
        let change = prepared.change_noms();
        let change_commitment = prepared
            .change_commitment()
            .ok_or(DomActuatorError::WalletUnavailable)?;
        let change_output = wallet
            .state
            .outputs
            .get(&change_commitment)
            .ok_or(DomActuatorError::WalletUnavailable)?;
        assert_eq!(change_output.value, change);
        assert_eq!(change_output.status, OutputStatus::Unconfirmed);
        assert!(change_output.payout_for().is_some());
        let change_blinding = Zeroizing::new(*change_output.blinding);
        assert_eq!(wallet.state.outputs.len(), 2);
        drop(wallet);
        drop(store);
        let before = fs::read(&wallet_path)?;
        let mut store = DomActuatorStoreV1::open_existing(&store_path)?;
        let lease = store.acquire_lease(digest(3), digest(21), 20_000, 10_000)?;
        let mut wallet = DomParticipantWalletV1::open_existing(
            &wallet_path,
            Zeroizing::new("test-password".into()),
            authority,
        ...
        let resumed = wallet
            .session(DomWalletSessionLegV1::Upstream)?
            .prepare_funding_inputs_v16(&mut store, lease, &terms, 20_001)?
            .ok_or(DomActuatorError::InsufficientFunds)?;
        assert_eq!(resumed.reservation(), &reservation);
        assert_eq!((resumed.fee_noms(), resumed.change_noms()), (fee, change));
        assert_eq!(resumed.change_commitment(), Some(change_commitment));
        assert_eq!(wallet.state.outputs.len(), 2);
        assert_eq!(fs::read(&wallet_path)?, before);
        assert!(!before
            .windows(32)
            .any(|bytes| bytes == &change_blinding[..]));
        assert!(!fs::read(&store_path)?
            .windows(32)
            .any(|bytes| bytes == &change_blinding[..]));
        let mut wrong_terms = terms;
        wrong_terms.dom_leg.amount += 1;
        assert!(wallet
            .session(DomWalletSessionLegV1::Upstream)?
            .prepare_funding_inputs_v16(&mut store, lease, &wrong_terms, 20_002)
            .is_err());
        assert_eq!(fs::read(&wallet_path)?, before);
        Ok(())
    }
    ...
    #[test]
    fn bootstrap_v16_nonfunding_participant_needs_no_dom_inputs() -> TestResult {
        let directory = tempfile::tempdir().test_context("tempdir")?;
        fs::set_permissions(directory.path(), fs::Permissions::from_mode(DIRECTORY_MODE))?;
        let wallet_path = directory.path().join("wallet.v3");
        let terms = bootstrap_funding_terms_v16(false)?;
        let (_, downstream, _) = bindings()?;
        let upstream = binding_from(TestBindingPartsV1 {
            route_id: digest(1),
            session_id: digest(2),
        ...
        assert!(wallet
            ...
            .is_none());
        assert_eq!(fs::read(&wallet_path)?, before);
        assert_eq!(wallet.state.outputs.len(), 0);
        Ok(())
    }
