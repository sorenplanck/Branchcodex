...
/// Exact wallet-to-signer bridge for a statically selected native nonce vault.
///
/// This is additive to the Contracts-owned V1 constructor. A borrowed vault
/// keeps the daemon's retained physical owner in place across bounded ticks;
/// all nonce and restart operations still enter that owner's native methods.
/// The local signing share remains opaque and is consumed only after both
/// the retained Contracts session and wallet binding have been authenticated.
pub fn participant_vault_signer_v12<'store, Vault: dom_adaptor::NonceVaultV1>(
    nonce_vault: Vault,
    session_store: &'store ContractsSessionStoreV1,
    expected_binding: DomSessionBindingV1,
    trusted_chain_id: TrustedChainIdV1,
...
impl<'store> DomContractsActuatorV1<'store> {
    ...
    pub fn bind_funding_settlement_child(
        ...
        if let Some(native) = self
            ...
            .map_err(|_| DomActuatorError::ContractsAuthorityUnavailable)?
        {
            control.retain_native_f7_funding_v20(lease, request.scope(), &native, now_unix_ms)?;
    ...
    pub fn funding_settlement_child_binding(
        ...
        if let Some(native) = self
            ...
            .map_err(|_| DomActuatorError::ContractsAuthorityUnavailable)?
        {
            return self.require_settlement_child_binding(
                control,
                lease,
                custody_digest,
    ...
    /// Send native F7 funding under the authenticated coordinator/control
    /// locator. None selects legacy only if no F7 gate exists in this Store.
    pub fn dispatch_f7_funding_child_v20(
        &self,
        control: &mut DomActuatorStoreV1,
        lease: DomLeaseV1,
        ...
        let receipt = runtime
            .submit_persisted_f7_funding_v20(&native)
            .map_err(f7_runtime_error_v20)?;
...
fn f7_runtime_error_v20(error: RealDomError) -> DomActuatorError {
    match error {
        ...
        // Wrong txid/chain or malformed evidence is a hard refusal, not absence
        // or an ambiguous transport result eligible for blind retransmission.
...
mod tests {
    ...
    #[test]
    fn f7_v20_malformed_or_substituted_evidence_is_not_a_retryable_rpc_error() {
        assert!(matches!(
            f7_runtime_error_v20(RealDomError::InvalidEvidence),
            DomActuatorError::CapabilityMismatch
        ));
        assert!(matches!(
            f7_runtime_error_v20(RealDomError::Chain(
