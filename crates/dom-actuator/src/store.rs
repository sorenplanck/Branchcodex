...
impl DomLeaseV1 {
    ...
    /// Coordinator owner this lease is fenced to.
...
impl DomActuatorStoreV1 {
    ...
    /// Derive a local change-output pin from an authenticated, active native
    /// input reservation. This is wallet construction provenance only: it is
    /// neither a signing capability nor evidence that funding was broadcast.
    /// The eventual canonical template still fixes the actual commitment.
    pub(crate) fn funding_change_pin_v16(
        &mut self,
        lease: DomLeaseV1,
        binding: DomSessionBindingV1,
        ...
        value: u64,
        now_unix_ms: u64,
    ) -> DomActuatorResult<dom_wallet2::PayoutForV1> {
        if value == 0 || value > dom_crypto::range_proof::MAX_PROVABLE_VALUE {
            return Err(DomActuatorError::InvalidBinding);
        }
        // Both queries authenticate the physical store and its journal. The
        // retained process lock excludes a concurrent native writer between
        // the live lease check and the reservation read.
        self.retained_payout_face_selection(lease, binding, now_unix_ms)?;
        let retained = self
            .reservation_by_digest(reservation_digest)?
            .ok_or(DomActuatorError::CapabilityMismatch)?;
        if retained.route_id != binding.route_id()
            || retained.session_id != binding.session_id()
            || retained.status != RESERVATION_ACTIVE
        {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let mut body = b"DOM:wallet-bootstrap-change:v16\0".to_vec();
        body.extend_from_slice(&binding.chain_id());
        body.extend_from_slice(&binding.session_id());
        body.extend_from_slice(&reservation_digest);
        body.extend_from_slice(&value.to_be_bytes());
        dom_wallet2::PayoutForV1::new(*dom_crypto::blake2b_256(&body).as_bytes())
    ...
    pub fn authorize_action(
        ...
        self.authorize_action_with_owner_v21(
            lease,
            scope,
            evidence_digest,
    ...
    pub(crate) fn authorize_f7_claim_action_v21(
        &mut self,
        lease: DomLeaseV1,
        scope: ScopedDomActionV1,
        evidence_digest: Digest32,
        ...
        self.authorize_action_with_owner_v21(lease, scope, evidence_digest, None, now_unix_ms, true)
    }

    fn authorize_action_with_owner_v21(
        &mut self,
        lease: DomLeaseV1,
        scope: ScopedDomActionV1,
        evidence_digest: Digest32,
        ...
        if retain_claim_owner {
            append_event(
                &transaction,
                scope.binding().session_id(),
                native_f7_claim_v21::claim_prefix_effect_v21(scope),
                native_f7_claim_v21::claim_prefix_digest_v21(
                    scope,
                    lease.owner_id,
                    lease.fencing_epoch,
                    authorization_digest,
    ...
    /// Repair the mirror using an admission reauthenticated by the native F7
    /// Store. This accepts a closed Store proof, not arbitrary receipt facts.
    pub(crate) fn persist_f7_final_claim_admission_mirror_v14(
        &mut self,
        lease: DomLeaseV1,
        binding: DomSessionBindingV1,
        authority: &FinalClaimTransportAuthorityFactsV2,
        ...
        if admitted.session_id() != binding.session_id() {
            return Err(DomActuatorError::CapabilityMismatch);
        }
        let facts = ValidatedSubmissionReceiptFactsV1 {
            tx_hash: admitted.tx_hash(),
            state: admitted.receipt_state(),
            relayed: admitted.was_relayed(),
            receipt_digest: admitted.receipt_digest(),
        };
        facts.validate()?;
        self.persist_final_claim_admission_v2(lease, binding, authority, facts, now_unix_ms)
    }
