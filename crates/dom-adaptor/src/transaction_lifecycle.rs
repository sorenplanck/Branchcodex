...
impl VerifiedSharedOutputV1 {
    /// Revalidate an exact output retained by an authenticated funding owner.
    ///
    /// This checks the mathematical proof and capsule envelope. The caller
    /// must separately authenticate the funding transaction, terms and shared
    /// commitment; this constructor does not mint a funding authorization.
    pub fn from_retained_output_v14(
        output: &TransactionOutput,
...
mod tests {
    ...
    #[test]
    fn retained_shared_output_v14_preserves_native_capsule_and_proof() {
        let shared = shared_fixture(90);
        let retained = VerifiedSharedOutputV1::from_retained_output_v14(
            shared.output.output(),
            shared.output.commitment(),
        )
        .expect("retained collaborative output verifies");
        assert_eq!(retained, shared.output);
    }

    #[test]
    fn retained_shared_output_v14_refuses_commitment_and_proof_substitution() {
        let shared = shared_fixture(90);
        let unrelated = output(91, &blinding(19));
        assert!(VerifiedSharedOutputV1::from_retained_output_v14(
            &unrelated,
            shared.output.commitment(),
        )
        .is_err());
        let mut mutated = shared.output.output().clone();
        mutated.proof[50] ^= 1;
        assert!(VerifiedSharedOutputV1::from_retained_output_v14(
            &mutated,
            shared.output.commitment(),
        ...
        .is_err());
        // Stripping the capsule cannot turn its bound proof into an ordinary
        // output: the verifier's extra-commit domain remains mandatory.
        let mut stripped = shared.output.output().clone();
        stripped.proof = shared
            .output
            .output()
            .range_proof_bytes()
            .expect("proof")
