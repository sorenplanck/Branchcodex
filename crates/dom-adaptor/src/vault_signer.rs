...
impl<Handle> CommitmentExportedV1<Handle> {
    ...
    /// Consume trusted protocol evidence for this exact commitment resend.
    pub fn authorize_resend(
        &self,
        round: &mut ValidatedSigningRoundStateV1,
    ) -> crate::Result<ValidatedResendAuthorizationV1> {
        round.authorize_local_resend(
...
impl<Handle> RevealExportedV1<Handle> {
    ...
    /// Consume trusted protocol evidence for this exact nonce-reveal resend.
    pub fn authorize_resend(
        &self,
        round: &mut ValidatedSigningRoundStateV1,
    ) -> crate::Result<ValidatedResendAuthorizationV1> {
...
impl PartialExportedTerminalV1 {
    ...
    /// Consume trusted protocol evidence for this exact terminal partial resend.
    pub fn authorize_resend(
        &self,
        round: &mut ValidatedSigningRoundStateV1,
    ) -> crate::Result<ValidatedResendAuthorizationV1> {
    ...
    Sessions: OperationalSigningSessionAuthorityV1,
{
    ...
    /// Consume a Store-authenticated U-adaptor recovery session explicitly.
    pub fn begin_graph_refund_signing_round_v23(
        &mut self,
        accepted_session: Sessions::AcceptedSession,
    ) -> core::result::Result<
        ValidatedSigningRoundStateV1,
    ...
    > {
        ...
        ValidatedSigningRoundStateV1::from_graph_refund_session_authority_v23::<Sessions>(
            accepted_session,
            &self.signing_share,
        )
        .map_err(Into::into)
    ...
    ///
    /// Uses the journal predecessor and sender sequence bases from the opaque
    /// handle with the vault-backed consume-before-export path.
...
where
    ...
    Custody: ReservationLookupCustodyV1,
{
    /// End this signer ownership and return the original vault unchanged.
    /// The private signing share and custody handles are dropped. All nonce
    /// consumption and exposure records remain in the same durable vault.
    pub fn into_vault_v18(self) -> Vault {
        self.vault
    }
    ...
    /// Select fresh initialization or exact restart from native custody. A
    /// caller cannot choose the branch. Exact absence permits a single local
    /// pristine derivation; an existing lookup enters strict restart, where a
    /// missing vault occurrence is irreversibly abandoned and never recreated.
    pub fn open_local_reservation_v12(
        &mut self,
        round: &mut ValidatedSigningRoundStateV1,
    ) -> SignerResult<Vault, Custody, RestartedReservationV1<Vault::ReservationHandle>>
    where
        Custody: crate::ReservationLookupInitializationCustodyV12,
    {
        let (context, roster, local_index) = round.local_reservation_context_v12();
        let binding =
            ReservationContextBindingV1::new(context, roster, local_index, &self.signing_share)?;
        let request = ReservationLookupRecoveryRequestV1::new(
            crate::SessionId::from_bytes(*context.session_id())?,
            *binding.digest(),
        )?;
        match self
            .custody
            .classify_local_reservation_v12(&request)
            .map_err(VaultBackedSignerError::Custody)?
        {
            crate::LocalReservationPresenceV12::Present => self.resume_claimed_after_restart(round),
            crate::LocalReservationPresenceV12::Absent => {
                let authority = round.take_local_pristine_derivation_base_v12()?;
                self.claim_fresh(authority)
    ...
    pub fn sign_and_export_partial(
        ...
        Ok((
            PartialExportedTerminalV1 {
                ...
                request_lookup,
                context_binding_digest: state.context_binding_digest,
                session_id: crate::SessionId::from_bytes(*state.context.session_id())?,
    ...
    pub fn resend_commitment(
        ...
        self.resend_bound(authority, ResendProtocolStageV1::Commitment)
    }
    ...
    pub fn resend_reveal(
        ...
        self.resend_bound(authority, ResendProtocolStageV1::Reveal)
    }
    ...
    fn resend_bound(
        ...
        let recovered = self
            .vault
            .recover_spent_artifact(&authority)
            .map_err(VaultBackedSignerError::Vault)?;
        let request = ResendRequestV1::from_recovered(authority, &recovered)?;
...
fn spent_identity_matches(
    spent: &impl VaultSpentArtifactSnapshotV1,
    context_binding_digest: &[u8; 32],
    expected_session_id: &[u8; 32],
    ...
    spent.nonce_identity().bound_digest() == context_binding_digest
        && spent.nonce_identity().session_id().as_bytes() == expected_session_id
        && spent.nonce_identity().participant_id().as_bytes() == expected_participant_id
        && spent.nonce_identity().purpose() == expected_purpose
}

fn validate_resend_state_binding(
    authority: &ValidatedResendAuthorizationV1,
    request_lookup: &ReservationRequestLookupV1,
    context_binding_digest: &[u8; 32],
    session_id: &[u8; 32],
    purpose: crate::PurposeV1,
    required_stage: ResendProtocolStageV1,
) -> core::result::Result<(), AdaptorError> {
    if authority.request_lookup() != request_lookup
        || authority.reservation_context_binding_digest() != context_binding_digest
        || authority.session_id().as_bytes() != session_id
        || authority.purpose() != purpose
        || authority.protocol_stage() != required_stage
    {
...
mod tests {
    ...
    fn live_projection_rejects_wrong_retry_spent_kind_and_identity() {
        ...
        let rejects_identity = |identity: crate::NonceIdentityV1| {
            let mut snapshot = test_snapshot(ReservationLiveStageV1::AfterCommitment, false);
            snapshot
                .commitment
                .as_mut()
                .expect("commitment descriptor")
        ...
        for identity in [
            crate::NonceIdentityV1::new(
                crate::SessionId::from_bytes([0x55; 32]).expect("mutated session"),
                crate::ParticipantId::from_bytes([9; 32]).expect("participant ID"),
                crate::PurposeV1::Refund,
                [3; 32],
                1,
            ...
            crate::NonceIdentityV1::new(
                crate::SessionId::from_bytes([8; 32]).expect("session ID"),
                crate::ParticipantId::from_bytes([0x56; 32]).expect("mutated participant"),
                crate::PurposeV1::Refund,
                [3; 32],
                1,
            )
