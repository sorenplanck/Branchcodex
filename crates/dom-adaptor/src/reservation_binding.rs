...
impl ReservationContextBindingV1 {
    pub(crate) fn new(
        ...
        Self::from_public_context_v23(
            context,
            roster,
            local_protocol_roster_index,
            signing_share.public_key(),
        )
    }

    fn from_public_context_v23(
        context: &SessionContextV1,
        roster: &ParticipantRosterV1,
        local_protocol_roster_index: u16,
        local_public_key: &dom_crypto::PublicKey,
    ) -> Result<Self> {
        ...
        if local.direction() != context.direction()
            ...
            || local.signing_public_key() != local_public_key
...
/// Native custody's result after authenticating the exact local reservation
/// namespace. Absence is distinct from a corrupt, abandoned or unavailable
/// record; those remain errors and never permit a new reservation.
#[derive(Clone, Copy)]
pub enum LocalReservationPresenceV12 {
    /// One exact still-custodied lookup exists.
    Present,
    /// The authenticated namespace has no matching lookup.
    Absent,
}

/// Additive initialization boundary used by a retained distributed signer.
/// A production implementation must audit its real session and complete lookup
/// namespace before reporting absence; it must not create or repair state.
pub trait ReservationLookupInitializationCustodyV12: ReservationLookupRecoveryCustodyV1 {
    /// Classify only the signer-derived exact private context binding.
    fn classify_local_reservation_v12(
        &mut self,
        request: &ReservationLookupRecoveryRequestV1,
    ) -> core::result::Result<LocalReservationPresenceV12, Self::Error>;
}
