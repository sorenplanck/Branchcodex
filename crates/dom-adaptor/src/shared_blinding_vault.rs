...
impl SharedBlindingBindingV1 {
    /// Trusted chain retained when the shared-output binding was authenticated.
...
impl SessionBlindingShareCapabilityV1 {
    ...
    /// Derive this participant's public transaction-offset contribution from
    /// its already retained share and exact bound session. The offset is
    /// deterministic across restart, purpose separated and never an arbitrary
    /// zero allocation supplied by the daemon. Both contributions must still
    /// be aggregated into the jointly committed native transaction template.
    pub fn transaction_offset_contribution_v12(
        &self,
        ...
        let mut context = self.binding.to_aad_bytes();
        context.extend_from_slice(b"DOM/SHARED-OUTPUT/LOCAL-TX-OFFSET/V12\0");
        context.push(purpose.to_byte());
        for attempt in 0u32..1024 {
            let mut candidate_context = context.clone();
            candidate_context.extend_from_slice(&attempt.to_be_bytes());
            let candidate = self.signing_share.decoy_seed_v1(&candidate_context);
            if dom_scriptless_primitives::scalar_bytes_are_canonical(&candidate, false) {
                return Ok(*candidate);
            }
        }
    ...
    /// Reopen the exact BP nonce without exporting the bound blinding share.
    pub fn resume_collaborative_range_proof_v16<Vault: CollaborativeBpNonceVaultV1>(
        &self,
        statement: &crate::BpStatementV1,
        vault: &mut Vault,
    ) -> core::result::Result<
    ...
    > {
        self.require_bound_bp_statement(statement)
            .map_err(CollaborativeBpNonceVaultError::Protocol)?;
        PendingCommonNonce::resume_vault_backed_v1(
            statement,
            self.binding.participant_index(),
            &self.signing_share,
            vault,
    ...
    /// Produce the public early-transport proof from this exact bound share.
    /// The caller must retain the resulting reveal before publishing its hash.
    pub fn early_transport_reveal_v16(
        &self,
        context_commitment: [u8; 32],
    ) -> Result<crate::EarlyShareRevealV1> {
        let b = &self.binding;
        let chain = b.trusted_chain_id();
        let statement = crate::SharePoPStatementV1::new(
            chain,
            *b.session_id(),
            b.roster(),
            b.role(),
        ...
        let proof = crate::prove_share_knowledge_v1(&statement, &self.signing_share)?;
        crate::EarlyShareRevealV1::new(context_commitment, statement, proof)
    }
    ...
    /// Compose one participant's ordinary shared-output transition C -> D:
    /// `x_i = r_D_i - r_C_i - offset_i`. Both capabilities must belong to the
    /// same participant and signed terms, with independent native sessions.
    /// This never reconstructs either aggregate shared-output blinding and
    /// cannot substitute the peer's share for the local contribution.
    pub fn compose_shared_output_transition_signing_share_v12(
        &self,
        destination: &SessionBlindingShareCapabilityV1,
        participant_offset: &[u8; 32],
    ) -> Result<SigningShareV1> {
        let source = &self.binding;
        let target = &destination.binding;
        if source.chain_id() != target.chain_id()
            || source.terms_hash() != target.terms_hash()
            || source.participant_id() != target.participant_id()
            || source.participant_index() != target.participant_index()
            || source.roster() != target.roster()
            || source.role() != target.role()
            || source.session_id() == target.session_id()
            || source.share_point() == target.share_point()
        {
            return Err(AdaptorError::AuthorizationMismatch);
        }
        // Public offsets still need canonical nonzero encoding. The native
        // scalar constructor owns validation and zeroizes its temporary copy.
        let offset = SigningShareV1::from_be_bytes(*participant_offset)?;
        let destination_excess = crate::compose_local_shared_output_spend_signing_share_v1(
            &destination.signing_share,
            &offset,
        )?;
        crate::compose_local_shared_output_spend_signing_share_v1(
            &destination_excess,
...
mod tests {
    ...
    #[test]
    fn v12_shared_transition_composes_only_one_participants_independent_shares() {
        let source = bound_share_fixture([0x71; 32], [0x73; 32]);
        let destination = bound_share_fixture([0x72; 32], [0x73; 32]);
        let offset = scalar(7);
        let transition = source
            .compose_shared_output_transition_signing_share_v12(&destination, &offset)
            .expect("local transition");
        let offset_point = SigningShareV1::from_be_bytes(offset).expect("offset");
        let reconstructed = dom_scriptless_primitives::scriptless_add_public_points(&[
            transition.public_key().clone(),
            source.public_key().clone(),
            offset_point.public_key().clone(),
        ])
        .expect("public equation");
        assert_eq!(&reconstructed, destination.public_key());
        assert!(source
            .compose_shared_output_transition_signing_share_v12(&source, &offset)
            .is_err());
        let foreign_terms = bound_share_fixture([0x74; 32], [0x75; 32]);
        assert!(source
            .compose_shared_output_transition_signing_share_v12(&foreign_terms, &offset)
            .is_err());
        assert!(source
            .compose_shared_output_transition_signing_share_v12(&destination, &[0; 32])
            .is_err());
    }
    ...
    #[test]
    fn v12_transaction_offset_replays_and_is_bound_to_each_signing_purpose() {
        let capsule = deterministic_capsule();
        let first = deterministic_bound_share_capability([0x22; 32], [0x51; 32], &capsule);
        let reopened = deterministic_bound_share_capability([0x22; 32], [0x51; 32], &capsule);
        let purposes = [
            crate::PurposeV1::Funding,
            crate::PurposeV1::ClaimAdaptor,
            crate::PurposeV1::Refund,
            crate::PurposeV1::RefundAdaptor,
        ];
        ...
        for purpose in purposes {
            let offset = first
                ...
                .expect("offset");
            assert!(dom_scriptless_primitives::scalar_bytes_are_canonical(
                &offset, false
            ));
            assert_eq!(
                offset,
        ...
        assert!(first
            .transaction_offset_contribution_v12(crate::PurposeV1::Sponsor)
            .is_err());
        let changed = deterministic_bound_share_capability([0x23; 32], [0x51; 32], &capsule);
        assert_ne!(
            offsets[0],
            changed
                .transaction_offset_contribution_v12(crate::PurposeV1::Funding)
                .expect("other session")
        );
        let changed = deterministic_bound_share_capability([0x22; 32], [0x52; 32], &capsule);
        assert_ne!(
            offsets[0],
            changed
                .transaction_offset_contribution_v12(crate::PurposeV1::Funding)
