...
impl ValidatedAcceptedSessionMessageV1 {
    fn parse(
        ...
        Self::parse_with_graph_refund_profile_v23(bytes, trusted_chain_id, roster, false)
    }

    fn parse_with_graph_refund_profile_v23(
        bytes: &[u8],
        trusted_chain_id: &TrustedChainIdV1,
        roster: &ParticipantRosterV1,
        graph_refund_v23: bool,
    ) -> Result<Self> {
        ...
        if graph_refund_v23 {
            if payload.purpose() != PurposeV1::RefundAdaptor {
...
impl ValidatedSigningRoundBootstrapV1 {
    ...
    fn from_components(
        ...
        let kernel_message_digest =
            dom_scriptless_consensus::scriptless_kernel_message_digest_v1(kernel);
...
impl ValidatedSigningRoundStateV1 {
    ...
    {
        Self::from_operational_with_profile_v23::<Authority>(session, signing_share, false)
    }

    /// Explicit recovery-adaptor entry. Legacy entry and parser stay strict.
    pub fn from_graph_refund_session_authority_v23<Authority>(
        session: Authority::AcceptedSession,
        signing_share: &SigningShareV1,
    ) -> Result<Self>
    where
        Authority: OperationalSigningSessionAuthorityV1,
    {
        if session.purpose() != PurposeV1::RefundAdaptor || session.adaptor_point().is_none() {
            return Err(AdaptorError::AuthorizationMismatch);
        }
        Self::from_operational_with_profile_v23::<Authority>(session, signing_share, true)
    }

    fn from_operational_with_profile_v23<Authority>(
        session: Authority::AcceptedSession,
        signing_share: &SigningShareV1,
        graph_refund_v23: bool,
    ) -> Result<Self>
        ...
        Authority: OperationalSigningSessionAuthorityV1,
    {
        ...
        let mut state = Self::from_bootstrap_with_profile_v23(
            bootstrap,
            signing_share,
            sender_sequence_bases,
            graph_refund_v23,
    ...
    fn from_bootstrap_with_sequences(
        ...
        Self::from_bootstrap_with_profile_v23(
            bootstrap,
            signing_share,
            sender_sequence_bases,
            false,
    ...
    fn from_bootstrap_with_profile_v23(
        bootstrap: ValidatedSigningRoundBootstrapV1,
        signing_share: &SigningShareV1,
        sender_sequence_bases: [u64; 2],
        graph_refund_v23: bool,
        ...
        if graph_refund_v23 {
            if base_context.purpose() != PurposeV1::RefundAdaptor {
                return Err(AdaptorError::AuthorizationMismatch);
            }
        } else {
    ...
    pub fn accept_message(&mut self, bytes: &[u8]) -> Result<AcceptedMessageDispositionV1> {
        ...
        let message = ValidatedAcceptedSessionMessageV1::parse_with_graph_refund_profile_v23(
            bytes,
            &self.trusted_chain_id,
            &self.roster,
            self.graph_refund_v23,
        )?;
    ...
    pub(crate) fn local_reservation_context_v12(
        &self,
    ) -> (&SessionContextV1, &ParticipantRosterV1, u16) {
        (&self.base_context, &self.roster, self.local_protocol_index)
    }

    /// Consume a fresh local derivation authority with at most the earlier
    /// peer's commitment already accepted. This is the distributed V12 entry:
    /// the second participant need not beat the first participant's network
    /// message to start its own independently generated nonce.
    ///
    /// No local commitment, reveal, partial, pending out-of-order message or
    /// previously issued derivation/restart capability may exist. The vault's
    /// lifetime session tombstone remains authoritative against reuse.
    pub fn take_local_pristine_derivation_base_v12(&mut self) -> Result<ValidatedDerivationBaseV1> {
        if self.derivation_authority_issued
            || self.restart_authority_issued
            ...
            || self.commitments.len() > usize::from(self.local_protocol_index)
        {
            return Err(AdaptorError::AuthorizationMismatch);
        }
        self.derivation_authority_issued = true;
...
mod tests {
    ...
    fn pristine_participant_fixture_v12(index: usize) -> CommitmentFixture {
        let mut fixture = commitment_fixture();
        let shares = [
            SigningShareV1::from_be_bytes([0x13; 32]).expect("share"),
            SigningShareV1::from_be_bytes([0x14; 32]).expect("share"),
        ];
        let share = shares
            .iter()
            .find(|share| {
                share.public_key() == fixture.roster.entries()[index].signing_public_key()
            })
            .expect("exact participant share");
        let mut keys: Vec<_> = fixture
            .roster
            .entries()
            .iter()
            .map(|entry| entry.signing_public_key().clone())
            .collect();
        keys.sort_by_key(PublicKey::to_compressed_bytes);
        let request = SigningRoundSessionRequestV1::new(
            [0x32; 32],
            ContractKindV1::WitnessOrTimeout,
            PurposeV1::Refund,
            fixture.roster.clone(),
            transaction_for_signing_keys(&keys),
            0,
            None,
        )
        .expect("native request");
        let bootstrap =
            ValidatedSigningRoundBootstrapV1::from_session_request(fixture.chain, request, share)
                .expect("native bootstrap");
        fixture.state = ValidatedSigningRoundStateV1::from_bootstrap(bootstrap, share)
            .expect("native local state");
        fixture
    }

    #[test]
    fn v12_second_participant_can_start_after_exact_peer_commitment_once() {
        let mut fixture = pristine_participant_fixture_v12(1);
        assert_eq!(
            fixture
                .state
        ...
        let authority = fixture
            .state
            .take_local_pristine_derivation_base_v12()
            .expect("own nonce has not been generated");
        assert_eq!(authority.local_protocol_index, 1);
        assert!(fixture
            .state
            .take_local_pristine_derivation_base_v12()
            .is_err());
        assert!(fixture.state.take_restart_derivation_base().is_err());
    }

    #[test]
    fn v12_pristine_derivation_refuses_own_commitment_and_out_of_order_prefix() {
        for index in 0..2 {
            let mut fixture = pristine_participant_fixture_v12(index);
            for message in &fixture.messages[..=index] {
                fixture
                    .state
                    .accept_message(message)
        ...
        assert_eq!(
            fixture
                .state
                .accept_message(&fixture.messages[1])
                .expect("buffered"),
            AcceptedMessageDispositionV1::Buffered
        );
        assert!(fixture
