...
impl SharePoPStatementV1 {
    ...
    pub fn new(
        ...
        let roster_digest = canonical_roster_digest(participant_roster);
    ...
    /// Check the complete ordered roster retained in this proof context.
    /// Matching only the participant at its index does not authenticate the
    /// other participants. This check issues no chain or signing authority.
    pub fn require_authenticated_roster_v22(&self, participant_roster: &[[u8; 32]]) -> Result<()> {
        let index = self.participant_index();
        validate_roster(participant_roster, index)?;
        if participant_roster[usize::from(index)] != self.participant_id()
            || canonical_roster_digest(participant_roster) != self.roster_digest
        {
    ...
    fn context_digest(&self) -> [u8; 32] {
        let mut context = [0u8; Self::ENCODED_LEN + 32];
        context[..Self::ENCODED_LEN].copy_from_slice(&self.bytes);
        context[Self::ENCODED_LEN..].copy_from_slice(&self.roster_digest);
        *blake2b_256_tagged(CONTEXT_TAG_V1, &context).as_bytes()
    }
}
...
fn canonical_roster_digest(participant_roster: &[[u8; 32]]) -> [u8; 32] {
    let mut bytes = Vec::with_capacity(2 + participant_roster.len() * 32);
    bytes.extend_from_slice(&(participant_roster.len() as u16).to_le_bytes());
    for participant in participant_roster {
        bytes.extend_from_slice(participant);
    }
    *blake2b_256_tagged(ROSTER_TAG_V1, &bytes).as_bytes()
}
