...
/// Read-only recovery of an artifact spent before its signed transcript ACK.
///
/// The request is signer-minted and contains no caller-selected permit or
/// digest. Resolve its unique stage index, then cross-check the authenticated
/// reservation and spent record before returning a non-exporting descriptor.
impl dom_adaptor::RestartArtifactRecoveryVaultV1 for DurableNonceVault {
    fn recover_spent_artifact_for_restart(
        &mut self,
        request: &dom_adaptor::RestartArtifactRecoveryRequestV1,
    ) -> Result<Self::RecoveredSpentArtifact> {
        self.require_operational()?;
        let kind = request.protocol_stage().exposure_kind();
        ...
        let permit_id = self
            .core
            .spent_permit_for_stage(request.request_lookup().as_bytes(), kind.to_byte())?;
        let artifact = self.core.spent_artifact(&permit_id)?;
        if artifact.kind != kind.to_byte()
            || artifact.reservation_id != reservation.identity.reservation_id
            || &artifact.request_lookup != request.request_lookup().as_bytes()
            || &reservation.identity.request_lookup != request.request_lookup().as_bytes()
            || &artifact.session_id != request.session_id().as_bytes()
            || &artifact.participant_id != request.participant_id().as_bytes()
            || artifact.purpose != request.purpose().to_byte()
            || &artifact.bound_digest != request.reservation_context_binding_digest()
            || Self::identity_of(&reservation)? != Self::identity_of_artifact(&artifact)?
        {
            return Err(VaultError::InvalidPermit);
        }
        // An already committed stage must name these exact persisted bytes.
        // A spent record may legitimately be ahead of the reservation journal
        // at a crash boundary; its native immutable stage index remains the
        // authority in that case and the signer revalidates the replay prefix.
        if let Some(retained) = reservation.spent(kind.to_byte()) {
            if retained.permit_id != artifact.permit_id
                || retained.outbound_digest != artifact.outbound_digest
            {
                return Err(VaultError::InvalidPermit);
            }
        }
